...
+-----------------------------------------------------------------------------+
|                                                                             |
|  [ ALERT 0: SILENT SHADOWS / UNWARY ] (0–3 Alert Points)                     |
|  • Orc patrols follow routine routes; drowsy, careless sentries.            |
|  • Stealth rolls are resolved with standard STEALTH rolls against each hero's TN. |
|  • Surprise round automatically granted on ambushing sentries.              |
|                                                                             |
|                                     │                                       |
|                                     ▼                                       |
|  [ ALERT 1: UNEASE & SNIFFERS / SUSPICIOUS ] (4–7 Alert Points)              |
...
|                                     │                                       |
|                                     ▼                                       |
|  [ ALERT 2: HUNTED & BARRICADED / HUNTED ] (8–11 Alert Points)               |
...
|                                     │                                       |
|                                     ▼                                       |
|  [ ALERT 3: DRUMS IN THE DEEP / OVERRUN ] (12+ Alert Points)                |
...
| **Sounding the Battle-horn of the Realm** | +3 AP | +1 | Grants immediate +1d to all companions in combat. |
...
* **False Trail / Decoy Trap**: Laying false bootprints or triggering a harmless remote rockfall to misdirect patrols: **-2 AP**.
...
```
  ...
  [1. POINT RECON]        [2. SHIELD-WALL PHALANX]    [3. REARGUARD EXTRACTION]
     ...
     (Point Scout)               (Frontline Breacher)         (Dúrmer & Dolg)
        │                                 │                          │
  (Bláin Scout)                    (Dúrmer & Dolg)             (Salvage Squad)
        │                                 │                          │
  [Main Company]                   (Support Heroes)           (Frontline Heroes)
        │                                 │                          │
  (Company Guide)                  (Bláin & Bróga)              (Rear Scout)
```
...
* **March Test**: The company's Guide or commander makes a **TRAVEL roll** or **ENHEARTEN roll** (invoking a Trait such as *Leadership* for **+1d**), OR the GM rolls Band **MANOEUVRE** (2d) against **Band TN 15** ($20 - \text{Readiness } 5$).
  * **Success ($\ge 1$ Icon)**: The squad moves silently, stepping in the shadows of the pillars (**+0 Alert / Noise Points** added).
  * **Success Icons ($\mathbf{6}$)**: Each $\mathbf{6}$ rolled reduces ambient suspicion by **-1 Noise Point** (stepping in rhythm to muffle gear), or grants **+1d** to the Point Scout's next roll.
...
4. **Shield-Wall Vanguard (Attached to Frontline)**:
   * Companions lock shields alongside frontline heroes in Forward or Defensive Stance.
...
* **Band Hope (Starts at 12)**: If Band Hope reaches 0, companion Dwarves become **Miserable** (-1d to all Band actions; cannot perform sacrifice saves).
* **Band Injury**: If a companion is reduced to 0 Endurance or takes a severe blow, mark a **Moderate Injury** (can be stabilized post-battle with a **HEALING roll** or Band **EXPERTISE** [2d vs Band TN 15]).
...
===============================================================================
  ...
  • Each hero must make a PROTECTION test (Armour dice + Feat Die) EVERY 1 MINUTE 
    (Ill-favoured).
  • Failure: Suffer 4 Endurance loss & gain 1 Shadow Point (Dread). On Eye of 
    Sauron (S): Suffer Severe Poison (8 Endurance loss and Weary condition).
  ...
  • Make a PROTECTION test only once per 1 HOUR (Standard roll).
  • Failure: Suffer 2 Endurance loss (Weary). On Eye of Sauron (S): Suffer Severe Poison.

  [MASTERWORK RESPIRATOR IMMUNITY]
  • Crafted dwarf-mask filter (Skill Endeavour: Resistance 3 using CRAFT rolls 
  ...
  • A hero can brew a neutralizing salve using Balin's medical supplies (Skill 
    Endeavour: Resistance 4, using CRAFT rolls or HEALING rolls). 
...
* **Trigger**: Any explosive noise (sounding the battle-horn, troll impacts) or vigorous hammering causes a slag-worm to fracture.
* **Mechanics**: Everyone in the zone must make an **ATHLETICS roll** or **PROTECTION test**:
...
* **Mechanics**: A hero carrying *The Broken Key* or acting as Scout can detect unstable spans on a **SCAN roll (Favoured)**. If triggered, a stonemason companion or hero can brace the ceiling with iron wedges in 1 round using a **CRAFT roll** or Band **WAR** (3d vs Band TN 15).
