# THE ARMOURIES OF THE THIRD DEEP
## A Masterclass Tabletop Delve for *The One Ring 2e* (*Moria: Through the Doors of Durin*)
### Complete Master Campaign Volume & Reference Atlas

---

> *"The world was young, the mountains green,*  
> *No stain yet on the Moon was seen,*  
> *No words were laid on stream or stone*  
> *When Durin woke and walked alone.*  
> *A king he was on carven throne*  
> *In many-pillared halls of stone,*  
> *With golden roof and silver floor,*  
> *And runes of power upon the door."*  
> — **The Song of Durin**

---

## Table of Contents & Master Volume Contents

* **Chapter 1: Campaign Context & Staging**
  * Historical Setting (2989 TA), Geopolitical Stakes, Lord Balin's Directives & Veteran Player-Heroes
* **Chapter 2: Squad Management, Marching & Band Operations**
  * The 8 Companion Specialists, Marching Formations, Band Dispositions & The 5-Tier Injury System
* **Chapter 3: Operational Mechanics, Alert Tracker & Environmental Hazards**
  * The 4-Stage Alert Ladder, Noise Points, Strategic Eye Awareness & Balrog Toxic Gas (*Breath of the Pit*)
* **Chapter 4: Keyed Locations & Spatial Atlas (Locations 1–10)**
  * Complete 10-Room Delve Atlas, Sensory Profiles, Swedish Read-Aloud Texts (*Högläsningstexter*), and TOR 2e Skill Tests
* **Chapter 5: Adversaries, Foes & Hazards**
  * Complete Mathematical Stat Blocks for The Mauler (AL 10 Troll), Grimnar the Disgraced (AL 6 Great Orc), and Garrison Ranks
* **Chapter 6: Relics, Rewards, Scavenge Tables & Royal Vault Loot**
  * Durin's Axe (Royal Artifact), Masterwork Tunnel-Guard Relics, Greater Hoard of Khazad-dûm & D66 Scavenge Table
* **Chapter 7: GM Playbook, Scene Pacing & Session Preparation**
  * Three-Act Session Playbook, Revelation Crisis Triggers, Table Cheat Sheets & Dynamic Encounter Flowcharts
* **Appendices: Tabletop Play Aids & Handouts**
  * **Appendix A**: Operational Node Map & Tactical Schematic
  * **Appendix B**: 1-Page Rapid GM Cheat Sheet & Operational Matrix
  * **Appendix C**: Dwarf Vanguard Band Worksheet & Tracking Log
  * **Appendix D**: In-World Player Handout — The Dying Scribe's Slate

---



<!-- PAGE BREAK: Chapter 1 -->

---


# Chapter 1: Campaign Context & Staging

> *"The world was young, the mountains green,*  
> *No stain yet on the Moon was seen,*  
> *No words were laid on stream or stone*  
> *When Durin woke and walked alone."*  
> — *The Song of Durin*

---

## 1. Historical Setting: Balin's Expedition (2989 TA)

### 1.1 The Strategic Situation in Khazad-dûm
In the year **2989 of the Third Age**, thirty years after the Battle of Five Armies and the slaying of Smaug the Golden, **Balin son of Fundin** led a brave host of Longbeards from the Lonely Mountain to reclaim the ancestral halls of Khazad-dûm. Entering through the East-Gate (the Great Gates of the Dimrill Dale), Balin’s vanguard drove the scattered Orcs from the First Hall and the Second Hall, securing a tenuous foothold in the upper eastern mansions.

Yet the triumph was brief and fragile. Balin’s colony numbers fewer than one hundred fighting Dwarves—a mere candle in the suffocating dark of the subterranean metropolis. Beneath them plunge the Deeps, miles of abandoned galleries, choked mine shafts, and shadowed halls where the Orcs of the Misty Mountains and the fire-touched zealots of the Deeps gather under the brutal banner of **Great Orc Lord Malech**.

```
========================================================================================
                         CAMPAIGN TIMELINE & GEOPOLITICAL STATE
========================================================================================
 Year 2799 TA: Battle of Azanulbizar. King Dáin Ironfoot slays Azog at the Great Gates 
               but peers into the shadow of the threshold and turns back, sensing the 
               slumbering horror of Durin's Bane.
 Year 2941 TA: The Quest of Erebor. Smaug slain; the Kingdom under the Mountain restored.
 Year 2989 TA: Balin's Expedition departs Erebor. First Hall and Dimrill Dale secured.
               Reclamation of the Caves of Thrym Thistlebeard (Second Deep).
 PRESENT DAY:  The expedition to the Armouries of the Third Deep begins.
========================================================================================
```

### 1.2 The High Council & Strategic Imperative
At the East-Gate Encampment, within the sound of the Mirrormere’s waters, the leadership of the expedition has reached a critical crossroad:

* **Lord Balin son of Fundin**: Lord of Moria, noble and resolute, but bearing the immense weight of his people’s hopes. Balin recognizes that the colony cannot endure on pickaxes and hunting bows alone. His small vanguard needs high-grade Dwarven armour, heavy shields, and siege craft to fortify the Deeps against the inevitable counter-offensive from the Lower Mansions.
* **Commander Fróra (daughter of Dwalin)**: Balin’s battle-hardened niece and chief of military operations. Stern, pragmatic, and unyielding, Fróra has identified the ancient **Armouries of the Third Deep** as the singular objective that can turn the tide. If the pristine gromril-mail and tunnel-guard wargear stored in the Third Deep can be salvaged, every dwarf in Balin's force can be clad in impenetrable iron and steel.
* **King Dáin Ironfoot (in Erebor)**: From his throne beneath the Mountain, King Dáin looks upon Balin’s venture with grim foreboding. Having felt the icy dread that lingers beyond the East-Gate, Dáin has forbidden royal military mobilization until Balin can present **undeniable proof** of Moria’s reclaimable splendour—specifically ancient royal heirlooms, such as the fabled **Durin's Axe**, and intact arsenals that prove Khazad-dûm is not merely a tomb, but a viable realm.

The strategic mandate given to the Player-Heroes is twofold:
1. **Salvage Garrison Wargear**: Infiltrate the sealed Upper and Lower Armouries of the Third Deep to secure 40–50 suits of gromril-mail, shields, and masterwork dwarf-weapons to equip Balin's frontline vanguard, eliminating the colony's defensive vulnerability and securing the Caves of Thrym Thistlebeard.
2. **Recover Royal Proof**: Unlock the ancient Vault of Kings to retrieve royal heirlooms—chief among them **Durin's Axe**—to dispatch with an emissary to King Dáin in Erebor to secure five hundred heavily armed Dwarf veterans.

---

## 2. The Expeditionary Force (Player-Heroes)

The delve into the Third Deep is entrusted to three seasoned Player-Heroes who have spearheaded Balin's advance through the upper levels.

```
========================================================================================
                             PLAYER-HERO QUICK MATRIX
========================================================================================
 HERO                 CULTURE / CALLING       STRENGTH   HEART   WITS   ROLE / KEY FOCUS
----------------------------------------------------------------------------------------
 Torvir Hammerstone   Durin's Folk / Champion  7 (TN 13) 2 (TN 18) 5 (TN 15) Sentry / Breacher
 Einar son of Anar    Iron Hills / Hunter      6 (TN 14) 3 (TN 17) 5 (TN 15) Scout / Infiltrator
 Khoril Hornblower    Durin's Folk / Captain   7 (TN 13) 3 (TN 16)*4 (TN 16) Guide / Commander
                                              *Heart TN reduced via Prowess Virtue
========================================================================================
```

---

### 2.1 Torvir Hammerstone
* **Culture**: Dwarf of Durin's Folk (Age: 157)  
* **Standard of Living**: Prosperous | **Cultural Blessing**: *Redoubtable*  
* **Calling**: Champion | **Shadow Path**: Curse of Vengeance  
* **Journey Role**: Sentry / Heavy Vanguard  

#### Attributes & Target Numbers
* **STRENGTH 7** $\rightarrow$ **Target Number (TN) 13**
* **HEART 2** $\rightarrow$ **Target Number (TN) 18**
* **WITS 5** $\rightarrow$ **Target Number (TN) 15**

#### Combat Ratings & Gear
* **Endurance**: Maximum 29 | **Current Fatigue**: 7 | **Weary Threshold**: 24
* **Hope**: Current 10 / Maximum 10 | **Shadow**: 0 (Cleansed)
* **Combat Proficiencies**: Axes (Mastery, 3d), Spears (2d)
* **Weapons**:
  * *Great Axe*: Damage 7 (+1 Grievous = **8**), Injury 20, Load 4, Notes: *Pierce on 9–10 via Axe Mastery, Two-handed*.
  * *Spear*: Damage 4, Injury 14, Load 3, Notes: *One or two-handed*.
* **Armour & Protection**:
  * *Coat of Mail*: Protection 5d, Load 16.
  * *Helm*: +1d Protection, Load 2.
  * *Parry*: **15** (Wits 5 + Base 10).
* **Virtues & Rewards**:
  * *Fierce-Minded (Cultural Virtue)*: When fighting against overwhelming odds or Orc-kind, Torvir may spend 1 Hope to make an attack roll or Valour test **Favoured**.
  * *Mastery (Axes)*: Scores a Piercing Blow on a Feat die result of 9, 10, or Gandalf rune.
  * *Grievous (Great Axe)*: Increases the weapon's base Damage rating by +1.
* **Distinctive Features**: Fierce, Willful, *Enemy-lore (Orcs)*.
* **Backstory & Motivation**: *"Orcs burnt down my smithy in the Grey Mountains, and my anvil has been cold ever since."* Torvir is a sullen, scarred veteran of countless tunnel skirmishes. His beard is braided with blackened iron clasps, and he speaks in low, rumbling monosyllables. His hatred of Orcs borders on madness; when the blood of goblins is spilled, his hammer-arm knows no restraint.
* **Carried Relic**:
  * **The Eye of Thrym**: A carved stone-and-sapphire talisman recovered from King Thrym’s tomb in the Second Deep. It allows the bearer to project vision and hearing through the carved statues of Thrym's Caves. *Attunement Note: Strictly inactive in the Third Deep.*

---

### 2.2 Einar son of Anar
* **Culture**: Dwarf of the Iron Hills (Age: 42)  
* **Standard of Living**: Martial | **Cultural Blessing**: *Redoubtable*  
* **Calling**: Treasure Hunter | **Shadow Path**: Dragon-sickness  
* **Journey Role**: Scout / Trap-Finder / Burglar  

#### Attributes & Target Numbers
* **STRENGTH 6** $\rightarrow$ **Target Number (TN) 14**
* **HEART 3** $\rightarrow$ **Target Number (TN) 17**
* **WITS 5** $\rightarrow$ **Target Number (TN) 15**

#### Combat Ratings & Gear
* **Endurance**: Maximum 28 | **Current Fatigue**: 5 | **Weary Threshold**: 15
* **Hope**: Current 11 / Maximum 11 | **Shadow**: 2 (Cleansed from 4)
* **Combat Proficiencies**: Swords (Mastery, 3d), Spears (2d)
* **Weapons**:
  * *Broadsword / Short Sword*: Damage 4, Injury 16 (*Keen*: Pierce on 9–10), Load 2.
  * *Spear*: Damage 4, Injury 14, Load 3, Notes: *Throwable, Pierce 9–10*.
* **Armour & Protection**:
  * *Mail-shirt*: Protection 3d, Load 9.
  * *Helm*: +1d Protection, Load 4.
  * *Reinforced Shield*: +1 Parry (+3 total shield bonus, Load 2). *Unbreakable*.
  * *Total Parry*: **20** (Wits 5 + Base 10 + Shield 3 + Shield Reward 1 + *Durin's Way* underground bonus 2 = **21 / 20**).
* **Virtues & Rewards**:
  * *Durin's Way (Cultural Virtue)*: Gains **+2 Parry** when fighting underground within stone halls or mines.
  * *Keen (Sword)*: Pierce trigger modified to 9–10 on the Feat die.
  * *Reinforced Shield*: Cannot be destroyed by an adversary's *Break Shield* special damage; provides +1 additional Parry.
  * *Mastery (Awareness, Healing)*: Rolls for Awareness and Healing are **Favoured**.
* **Distinctive Features**: Cunning, Wary, *Burglary* (Trait: invoked for +1d on **STEALTH**, **SCAN**, or **CRAFT** tests regarding locks, traps, and vaults).
* **Backstory & Motivation**: The youngest of nine brothers from the red mines of the Iron Hills. Agile, sharp-eyed, and quiet on his feet, Einar volunteered for Balin's venture dreaming of ancient gold and mithril veins. Yet the glittering dust of the Deeps whispers to him in the dark; his fascination with forgotten treasures threatens to kindle the fires of Dragon-sickness.
* **Carried Gear & Relics**:
  * **The Broken Key**: An ancient, fractured key of masterwork dwarf-craft. Makes all **SCAN** rolls **Favoured** (roll 2 Feat dice, keep the higher result) to detect concealed doors, hidden compartments, mechanical traps, and structural weaknesses.
  * **Wind-proof Lantern**: Dwarf-forged shuttered lantern (+1d on **SCAN** rolls in dark subterranean chambers).
  * **Soothing Balm**: Herbal salve (+1d on **HEALING** tests).
  * *Pack Pony (Frodo)*: Stationed safely at the East-Gate Camp.

---

### 2.3 Khoril Hornblower
* **Culture**: Dwarf of Durin's Folk (Age: 85)  
* **Standard of Living**: Prosperous | **Cultural Blessing**: *Redoubtable*  
* **Calling**: Captain | **Shadow Path**: Lure of Power  
* **Journey Role**: Guide / Squad Commander  

#### Attributes & Target Numbers
* **STRENGTH 7** $\rightarrow$ **Target Number (TN) 13**
* **HEART 3** $\rightarrow$ **Target Number (TN) 16** (Reduced from 17 via *Prowess*)
* **WITS 4** $\rightarrow$ **Target Number (TN) 16**

#### Combat Ratings & Gear
* **Endurance**: Maximum 29 | **Current Fatigue**: 7 | **Weary Threshold**: 16
* **Hope**: Current 11 / Maximum 11 | **Shadow**: 1 (Cleansed from 3)
* **Combat Proficiencies**: Axes (Mastery, 3d), Bows (2d)
* **Weapons**:
  * *Long-hafted Axe*: Damage 6, Injury 18 (1-handed) / 20 (2-handed), Load 3, Notes: *Break Shield option*.
  * *Dwarven Bow*: Damage 3, Injury 14, Load 2.
* **Armour & Protection**:
  * *Mail-shirt*: Protection 3d, Load 9.
  * *Helm*: +1d Protection, Load 2.
  * *Reinforced Shield*: +2 Parry (+1 Reward = +3 total), Load 2. *Unbreakable*.
  * *Total Parry*: **17** (Wits 4 + Base 10 + Shield 3 + *Durin's Way* underground bonus 2 = **19 / 17**).
* **Virtues & Rewards**:
  * *Prowess (Heart)*: Permanently reduces Heart TN by 1 (TN 16).
  * *Durin's Way*: +2 Parry when fighting underground.
  * *Reinforced Shield*: Unbreakable shield with +1 Parry bonus.
* **Distinctive Features**: Wary, Cunning, *Leadership* (Trait: invoked for +1d on **ENHEARTEN**, **TRAVEL**, or **BATTLE** tests).
* **Backstory & Motivation**: A veteran officer of the Erebor garrison who blew the muster horns during the clearing of the East-Gate. Khoril views the expedition through the strict lens of military discipline and order. His mind is consumed by ancient architectural treatises; he is obsessed with discovering the **Mithril key for the "Pillar Door on the Stairs"** along the Dimrill Stair, believing it holds the key to sealing Khazad-dûm from eastern invasion.
* **Carried Gear & Relics**:
  * **Battle-horn of the Realm**: A massive, silver-banded war-horn of Durin's Folk. Grants **+1d on all BATTLE rolls** made by Khoril or Band **WAR** / **RALLY** rolls, and allows the *Rally Companions* combat maneuver. *Acoustic Danger: Blown in enclosed halls, its thunderous echo immediately raises the Alert Tracker by +1 and adds +2 to Eye Awareness.*
  * **Dwarven Strong Spirits**: Cured spirit flask (+1d on **ENHEARTEN** rolls).
  * **Climbing Rig**: Silk-braided rope and iron pitons (+1d on **ATHLETICS** climbing tests).

---

## 3. The Dwarf Companion Band (The Seven Veterans)

Accompanying the three heroes is a seasoned squad of **seven veteran Dwarf Companions**. These warriors operate under *The One Ring 2e* Moria Band rules with **Readiness Rating: 5** and **Band TN: 15** ($20 - 5 = 15$) (see Chapter 2).

```
========================================================================================
                              COMPANION SQUAD ROSTER
========================================================================================
 NAME       ENDURANCE   INJURY / STATUS      GIFT & SPECIALTY          QUIRK / TRAIT
----------------------------------------------------------------------------------------
 Bláin      10 / 18     Moderate (Treated)   Goblin-Slayer (+1d vs Orcs) Scarred jaw
 Fáin       15 / 18     Uninjured            Dead-Eye (+1d Bow rolls)    Never unhelms
 Dúrmer     22 / 22     Hardened (Promoted)  Mighty (+1d Heavy labor)    Flask sipper
 Hjoldring  18 / 18     Uninjured            Smith (+1d Forge/Repair)    Soot-stained
 Bróga      12 / 12     Restored             Vaultbreaker (+1d Locks)    Copper rings
 Austri     10 / 18     Restored             Scout (+1d Bypass tunnels)  Mutters runes
 Dolg       18 / 18     Restored             Shield-Bearer (Intercepts)  Notched shield
========================================================================================
```

```
               [KHORIL HORNBLOWER] (Squad Guide & Commander)
               [TORVIR] (Heavy Vanguard) ── [EINAR] (Scout & Locks)
                                      │
         ┌────────────────────────────┼────────────────────────────┐
         ▼                            ▼                            ▼
 [FORWARD SCREEN]             [SHIELD-WALL]                [REARGUARD & SALVAGE]
  • Austri (Runner)            • Dúrmer (Hardened Anchor)   • Bláin (Cleaver)
  • Bróga (Vaultbreaker)       • Dolg (Bulwark)             • Fáin (Marksman)
                                                            • Hjoldring (Smith/Porter)
```

### 3.1 Detailed Companion Profiles

#### 1. Bláin the Cleaver
* **Health**: 10 / 18 Endurance | **Status**: Moderate Injury (bandaged and treated).
* **Gift — Goblin-Slayer**: Gains **+1d** on all attack and damage rolls when facing Orcs, Goblins, or Uruks.
* **Quirk**: Bears a jagged, ghastly scar from his jaw to his collarbone, inflicted by an Orc scimitar at the Bridge of Khazad-dûm. He speaks in a rasping hiss and never turns his back on an unsearched corridor.
* **Tactical Role & Bond**: Torvir's sworn shield-brother. He wields a heavy broad-bladed axe and is ideally assigned to **Rearguard Choke Point Defense** or the front rank of the **Shield-Wall Phalanx**.

#### 2. Fáin Iron-Visor
* **Health**: 15 / 18 Endurance | **Status**: Uninjured.
* **Gift — Dead-Eye**: Gains **+1d** on all ranged attacks using Dwarven bows or crossbows, and can fire from the Rearward stance without minimum distance penalties.
* **Quirk**: Wears an archaic, fully enclosed iron visor-helm forged in the Iron Hills. He has not removed it in the presence of others since entering Moria, claiming the dark air burns his eyes.
* **Tactical Role & Bond**: Quiet, stoic, and disciplined. Fáin coordinates with Khoril on interlocking fields of fire and provides deadly overwatch in vaulted halls.

#### 3. Dúrmer the Iron-Shouldered (Hardened Veteran)
* **Health**: 22 / 22 Endurance | **Status**: Hardened (Promoted Ally; adds +1 to Band Readiness).
* **Gift — Mighty**: Immense physical power. Gains **+1d** on all rolls involving feats of strength, forcing stone doors, hauling siege machinery, or breaking Orc barricades.
* **Quirk**: Keeps a flat iron flask of fiery dwarven spirit strapped to his breastplate, taking measured sips between combat clashes.
* **Tactical Role & Bond**: The physical anchor of the Band. Dúrmer held the collapsed entrance at the Slave-Mines against a score of goblins. He serves as the **Phalanx Anchor** and lead breacher for the **Heavy Salvage Squad**.

#### 4. Hjoldring the Armourer
* **Health**: 18 / 18 Endurance | **Status**: Uninjured.
* **Gift — Smith**: Gains **+1d** on all Craft and Lore rolls regarding metalwork, armour repair, salvage assessment, and detecting structural metal fatigue.
* **Quirk**: His thick red beard and leathern apron are permanently stained with forge soot, charcoal dust, and linseed oil.
* **Tactical Role & Bond**: Dispatched by Commander Fróra specifically to inspect the quality of ancient third-deep gromril-mail and crate salvageable wargear without damaging delicate runic seals.

#### 5. Bróga Vaultbreaker
* **Health**: 12 / 12 Endurance | **Status**: Fleeting Injury (Restored).
* **Gift — Vaultbreaker**: Gains **+1d** on **CRAFT** and **STEALTH** rolls to pick masterwork Dwarven puzzle locks, disarm mechanical pressure plates, and bypass counterweight traps (invoking the *Burglary* Trait).
* **Quirk**: Every finger on both hands is weighed down with thick copper, brass, and electrum rings, each notched with secret measuring marks and pick-angles.
* **Tactical Role & Bond**: Einar's trusted infiltration counterpart. Bróga moves ahead with picks, wedges, and listening horns to assess locked portals and trapped flagstones.

#### 6. Austri the Wayfinder
* **Health**: 10 / 18 Endurance | **Status**: Fleeting Injury (Restored).
* **Gift — Scout**: Gains **+1d** on all Explore and Stealth rolls to locate secret ventilation bypasses, flue crawls, and acoustic blind spots in ancient masonry.
* **Quirk**: Habitually mutters ancient architectural runes under his breath, tracing stone seams with his calloused fingertips as if reading a book.
* **Tactical Role & Bond**: Extremely nimble for a Dwarf. Austri serves as the point scout on the **Forward Scout Screen**, slipping through narrow chutes to reconnoitre enemy outposts.

#### 7. Dolg the Bulwark
* **Health**: 18 / 18 Endurance | **Status**: Fleeting Injury (Restored).
* **Gift — Shield-Bearer**: Can spend his combat reaction to interpose his massive tower-shield in front of an adjacent Player-Hero or wounded companion, intercepting one physical attack per round.
* **Quirk**: His heavy oak-and-iron shield is adorned with twenty-seven notched iron tips broken off from Orc spears that struck his guard.
* **Tactical Role & Bond**: Dedicated frontline protector. Dolg forms the unbreakable center of the **Shield-Wall Phalanx**, taking the brunt of enemy charges so Torvir and Khoril can deliver crushing blows.

---

## 4. Staging Ground: Safe Haven & Non-Combat NPCs

### 4.1 The Safe Haven: The Caves of Thrym Thistlebeard
The staging base for this expedition is not the distant East-Gate, but the newly reclaimed **Caves of Thrym Thistlebeard**, located along the eastern flank of the **Third Deep stratum** (Location 18 on the master elevation map) beneath the great cleft of Mount Silvertine.

```
========================================================================================
                          THE CAVES OF THRYM THISTLEBEARD
========================================================================================
 • LOCATION: Eastern Third Deep stratum (Location 18), beneath Thrym's Cleft.
 • STATUS: Cleansed Safe Haven (Moria Supplement pp. 112–114).
 • STRATEGIC VALUE: Shifts all travel routes between the East-Gate and the Deeps 
   from Dark Land (Hunt 14) to Wild Land (Hunt 16).
 • REST & RECOVERY: Allows prolonged rests, First Aid healing of Severe Injuries, 
   and Fellowship Phase Undertakings underground.
 • CONNECTION TO THE ARMOURIES: An ancient paved military gallery—the Westward Drift 
   (Great Transverse Gallery)—runs horizontally westward from the haven gates directly 
   into Location 1 (The Mustering-Yard).
========================================================================================
```

* **Environment**: A subterranean wonder of the Elder Days—petrified flowering stone, phosphorescent moss gardens, weeping mineral springs, and stone terraces carved by the ancient King Thrym.
* **Cleansed Sanctum**: The party cleared the foul blood-vines and brood-spiders in their previous delve, consecrating the hall with songs of Durin. The air is sweet and fresh, insulated from the sulfurous stench of the Deeps.
* **The Lateral Advance (The Westward Drift)**: From the western gate-arch of Thistlebeard's garden, the expedition advances horizontally along the *Great Transverse Gallery* (The Westward Drift). The rough karst walls and weeping mineral flowstones of the natural caverns gradually give way to smooth, dressed basalt, ancient Dwarven defensive breastworks, and massive iron-reinforced archways opening directly onto the high overlook of the Third Deep's Mustering-Yard.

### 4.2 Safe Placement of Non-Combat Rescued NPCs
During prior operations in the Slave-Mines and upper halls, the Fellowship liberated several captives. To preserve operational security, these non-combat NPCs have been escorted back to the **East-Gate Camp** under the direct protection of Lord Balin and Commander Fróra:

1. **Frór the Miner**: Sullen, soot-stained brother to the traitorous *Bildor*. Frór remains at the East-Gate armory working the bellows. Haunted by his brother’s descent into madness and treachery, Frór has warned the heroes: *"Bildor knows the vents of the Third Deep better than any living dwarf. If he lives, he walks the crawlspaces in the service of the Golden Eye."*
2. **The Traumatized Dwarf**: A rescued miner whose mind fractured under Orcish torture in the deep pits. He wanders the First Hall in a waking dream, convinced that Khazad-dûm is still in its golden age. In his lucid moments, he scratches remarkably precise architectural schematics of the Third Deep armouries on slate tablets, providing the party with vital maps of Locations 2, 5, and 9.
3. **Haldor of the Anduin Vale**: A rugged Northman woodman rescued from the Udûn slave pens. Deeply grateful to the Dwarves, Haldor manages the pack ponies and defensive stockade at the threshold of Dimrill Dale.

---

## 5. Relic Attunement & Operational Constraints

```
========================================================================================
                              RELIC PROFILE & CONSTRAINTS
========================================================================================
 RELIC                 HOLDER    MECHANICAL BENEFIT          THIRD DEEP CONSTRAINT
----------------------------------------------------------------------------------------
 The Eye of Thrym      Torvir    Scries via Haven statues    STRICTLY INERT / INACTIVE
 The Broken Key        Einar     Favoured on SCAN rolls      Active (traps/locks/caches)
 Battle-horn of Realm  Khoril    +1d Battle, Band Rally      Raises Alert (+1) & Eye (+2)
========================================================================================
```

### 5.1 The Eye of Thrym (Strictly Inactive in Third Deep)
* **Description**: A carved oval talisman of midnight-blue sapphire held in a filigree setting of star-iron, recovered from King Thrym’s sarcophagus.
* **Attunement Mechanics**: The relic is bound by ancient geomantic craft exclusively to the hundred stone sentinel statues carved into the rock walls of *The Caves of Thrym Thistlebeard*. When the bearer rests within the Caves, they may look through the eyes of any statue in the haven.
* **Operational Constraint**: The moment the bearer crosses the threshold archway out of the natural caverns and advances along the Westward Drift into the Third Deep, the sapphire dims to a dull, cold pebble. **The Eye of Thrym provides zero magical, scrying, or sensory benefits within the Armouries of the Third Deep.**

### 5.2 The Broken Key (Ancient Masterwork Tool)
* **Description**: A heavy, fractured key of blackened gromril-steel, etched with the runic cipher of the Royal Locksmiths of Durin III.
* **Operational Mechanics**: Makes the bearer's **SCAN** rolls **Favoured** (roll 2 Feat dice, keep the higher result) to locate concealed doors, detect mechanical tripwires, bypass pressure plates, and identify weakened structural lintels. Also allows invoking the *Burglary* Trait for **+1d** on lockpicking and trap-disarming **CRAFT** tests.
* **Limitation**: While it reveals the locking mechanisms of *The King's Door* (Location 9), the physical fragment is incomplete and cannot open the door alone without both the *Mithril Key* and the *King's Key* (or a masterwork Craft Endeavour).

### 5.3 Battle-horn of the Realm (Double-Edged War Horn)
* **Description**: A colossal war-horn carved from the horn of a Great Kine of Araw, banded in engraved silver and inscribed with the Battle-Runes of the Lonely Mountain.
* **Operational Mechanics**:
  * Grants **+1d on all BATTLE rolls** made by the bearer or Band **WAR** / **RALLY** rolls.
  * Allows a companion to sound the horn to automatically **Rally** all companions, removing the *Weary* condition for the next encounter.
  * **The Sound of Doom**: Sounding the horn is an incredibly loud act. It immediately generates **+5 Noise Points** on the tracker, advancing the Alert Tracker and the Eye of Sauron's awareness.

---

## 6. The Factions & Adversaries of the Third Deep

```
                  [GREAT ORC LORD MALECH] (Looming Threat at Palace of Stairs)
                                     │
        ┌───────────────────────────┴───────────────────────────┐
        ▼                                                       ▼
[GRIMNAR THE DISGRACED]                                 [THE MAULER]
 • Great Orc Stalker (AL 6)                              • Armoured Great Cave-Troll (AL 10)
 • Seeking revenge for Durin's Bridge                    • Terrified of pain; iron-plated
 • Stalks expedition at Alert Tier 2                     • Lairs in Location 6
        │                                                       │
        └───────────────────────────┬───────────────────────────┘
                                    ▼
                       [MORIA PATROLS & SNIFFERS]
                        • Sentry sniffers & drummers
                        • Mobilize at Alert Tiers 1-3
```

1. **Grimnar the Disgraced (Great Orc Bodyguard / Stalker)**: Stripped of his command after being humiliated by the heroes at Durin's Bridge, Grimnar stalks the Third Deep with a hand-picked pack of Mordor cutthroats. He fights with a stolen Dwarven dagger and his heavy scimitar, carrying the *Mithril Key* that allows secret passage through the pillar door, and biding his time to ambush the party at Location 9 (The King's Door).
2. **The Mauler (Armoured Great Cave-Troll)**: A gargantuan troll traumatized by injury during the Battle of Azanulbizar. Driven mad by fear of pain, it has wired tons of scrap breastplates, iron boiler plates, and anvil fragments to its hide, turning itself into an unyielding siege juggernaut in Location 6.
3. **Grik the Skulker (Goblin Informant)**: An opportunistic goblin spy lurking in the shadows of Location 1. Grik hates the brutal Mordor and Moria Orcs and is willing to sell secrets regarding patrol routes and the *Mithril Key* (which he knows the Mordor Orcs use to enter secretly via the pillar at the Dimrill Stairs, though he has no idea it leads to the treasure chamber) in exchange for bright silver, tobacco, or safety.
4. **Great Orc Lord Malech (The Looming Shadow)**: Ruling from the distant *Palace of Stairs*, Malech does not appear in person during this delve. However, if the party allows the Alert Tracker to reach Tier 3 (*Drums in the Deep*), Malech’s war-drums sound throughout the mountain, dispatching elite Black Uruk warbands to seal the shafts and cut off the heroes' retreat. Udûn Orcs under Malech's banner do not guard these armouries; they only come to trade at most.



<!-- PAGE BREAK: Chapter 2 -->

---


# Chapter 2: Moria Band Mechanics & Squad Operations

> *"Axes of the Mountain, shields of the Deep! Stand fast and let no shadow break the line!"*  
> — *Khoril Hornblower, Captain of Erebor*

---

## 1. The Moria Band System: Core Rules & Foundations

In *The One Ring 2e* supplement *Moria: Through the Doors of Durin*, expeditions operating in deep subterranean territory utilize the **Band Mechanics** to manage supporting companions, tactical formations, mass skirmishes, and collective endurance without bogging down individual initiative.The expeditionary force commands a seasoned vanguard of **seven veteran Dwarf Companions** (*Bláin, Fáin, Dúrmer, Hjoldring, Bróga, Austri, Dolg*).

```
========================================================================================
                                BAND STATISTICAL PROFILE
========================================================================================
 BAND READINESS RATING:       5 (Base 4 + 1 for Hardened Veteran Dúrmer)
 BAND READINESS TN:          15 (Calculated as 20 - Band Readiness)
 ACTIVE BAND SIZE:            Medium Band (7 Active Veteran Companions)
 STARTING BAND BURDEN:        Medium Burden (Prepared Ingress) -> Heavy (When Salvaging)
 STARTING BAND HOPE:         12 Points (Restored from Fellowship Pool)
 STARTING BAND SHADOW:        1 Point (Lingering trauma from prior delves)
 FELLOWSHIP POOL:             4 Points
========================================================================================
```

### 1.1 Band Readiness & Target Number (TN)
A Band’s efficiency, discipline, and collective grit are represented by its **Band Readiness Rating**.

$$\mathbf{\text{Band Readiness TN}} = 20 - \text{Band Readiness} = 20 - 5 = \mathbf{15}$$

* **Making Band Tests**: When the Band executes a collective tactical action, the GM calls for a test using one of the five **Band Dispositions**. The roll consists of **1 Feat Die + Success Dice equal to the Disposition Rating**. If the total equals or exceeds the **Readiness TN (15)** (modified by enemy Might or environmental threats), the action succeeds.
* **Success Icons ($\mathbf{6s}$)**: Each Success icon rolled on a Band test allows the Band to trigger specialized squad maneuvers (e.g. flanking, disarming traps silently, or shielding an ally).
* **Readiness Degradation**: If the Band suffers catastrophic losses (e.g. 3 or more companions slain or lost), the Readiness Rating drops from 5 to 4 ($\text{TN}$ rises to 16).

---

### 1.2 The Five Band Dispositions

The Band’s tactical capabilities are divided into five specialized ratings:

```
                          ┌───────────────────────────┐
                          │   BAND READINESS: 5       │
                          │   (READINESS TN: 15)      │
                          └─────────────┬─────────────┘
                                        │
     ┌──────────────┬──────────────┬────┴─────────┬──────────────┬──────────────┐
     ▼              ▼              ▼              ▼              ▼              ▼
  [WAR: 3]     [VIGILANCE: 2] [MANOEUVRE: 2] [EXPERTISE: 2]  [RALLY: 1]   [HOPE: 12]
 (Combat,       (Scouting,     (Silent March, (Engineering,  (Morale,     (Shared Pool)
  Phalanx,       Perimeter,     Withdrawal,    Locks,         Fatigue,
  Breaching)     Ambushes)      Evasion)       First Aid)     Courage)
```

1. **WAR (Rating 3 — 3d6)**:
   * *Scope*: Resolving combat clashes, executing shield-wall charges, breaching reinforced barricades, and holding choke points.
   * *Specialist Champions*: Dúrmer (*Mighty*), Bláin (*Goblin-Slayer*), Dolg (*Shield-Bearer*).
2. **VIGILANCE (Rating 2 — 2d6)**:
   * *Scope*: Scouting vaulted chambers, setting perimeter sentry watches, spotting Orc ambushes, and detecting acoustic echoes.
   * *Specialist Champions*: Austri (*Scout*), Fáin (*Dead-Eye*).
3. **MANOEUVRE (Rating 2 — 2d6)**:
   * *Scope*: Silent marching through echo chambers, navigating collapsed stairwells, rapid tactical redeployment, and disciplined fighting withdrawals.
   * *Specialist Champions*: Austri (*Scout*), Bróga (*Vaultbreaker*).
4. **EXPERTISE (Rating 2 — 2d6)**:
   * *Scope*: Crafting alchemical respirators, disarming complex mechanical traps, forcing ancient locks, repairing damaged wargear, and assessing stone masonry.
   * *Specialist Champions*: Hjoldring (*Smith*), Bróga (*Vaultbreaker*).
5. **RALLY (Rating 1 — 1d6)**:
   * *Scope*: Resisting the supernatural Dread of Morgoth/Balrog idols, making Band Fatigue tests, recovering from shock, and performing emergency triage under fire.
   * *Specialist Champions*: Supported directly by a hero's *Leadership* Trait or the *Battle-horn of the Realm*.

---

## 2. The Four Tactical Squad Deployments

The Player-Heroes can assign their seven companions to four distinct tactical roles depending on the operational needs of the delve. Assigning companions to these roles modifies group tests and provides specific tactical perks.

```
========================================================================================
                               TACTICAL SQUAD DEPLOYMENTS
========================================================================================
 ROLE                      ASSIGNED DWARVES    PRIMARY BENEFIT & MECHANIC
----------------------------------------------------------------------------------------
 1. Forward Scout Screen   Austri & Bróga      Rolls Vigilance (2d) vs Band TN 15. Prevents 
                                               surprise; enables Favoured SCAN rolls for 
                                               heroes inspecting the route.
 2. Shield-Wall Phalanx    Dúrmer, Dolg,       Rolls War (3d) vs Band TN 15. Intercepts 1 
                           Bláin               melee attack/round; prevents swarming.
 3. Rearguard Choke Point  Bláin & Fáin        Held at Location 2 Gatehouse. Guarantees 
                                               escape line; grants +1d on withdrawal tests.
 4. Heavy Salvage Porters  Hjoldring & Dúrmer  Hauls 40+ suits of mail from Location 8.
                                               Shifts Burden to Heavy; equips vanguard.
========================================================================================
```

---

### 2.1 Role 1: Forward Scout Screen
* **Assigned Roster**: **Austri the Wayfinder** & **Bróga Vaultbreaker** (2 Dwarves).
* **Positioning**: Advances 40–50 paces ahead of the main body, hugging shadow lines, archway cornices, and drainage troughs.
* **Operational Rules**:
  * Whenever the expedition enters a new keyed location or intersection, the GM rolls **Band Vigilance (2d6)** or **Band Manoeuvre (2d6)** against **Band TN 15** (modified by ambient Alert Level).
  * **On a Success**: The scouts identify enemy sentry posts (such as the Moria sniffers in Location 1), tripwires, or acoustic traps before the main expedition steps into danger. The Company cannot be surprised, and Player-Heroes gain **+1d** on their opening actions.
  * **Scouting Synergy**: A hero moving alongside the screen can make **SCAN rolls (Favoured)** to locate hidden compartments or mechanisms safely without breaking group stealth.
  * **On a Failure**: The scouts blunder into an acoustic tripwire or loose shale, generating **+1 Noise Point** on the Alert Tracker.

---

### 2.2 Role 2: Shield-Wall Phalanx
* **Assigned Roster**: **Dúrmer the Iron-Shouldered**, **Dolg the Bulwark**, and **Bláin the Cleaver** (3 Dwarves).
* **Positioning**: Formed in the central or front ranks, locking broad rim-reinforced shields across narrow corridors (10–15 feet wide) or doorframes.
* **Operational Rules**:
  * **Interception Reaction**: Once per combat round, Dolg or Dúrmer can interpose their tower-shields to absorb one incoming physical melee attack targeted at a Player-Hero or wounded companion. The damage is resolved against the companion’s Endurance and Armour dice (Armour 3d/4d).
  * **Anti-Swarm Bastion**: While the Shield-Wall remains unbroken, enemy adversaries cannot utilize flanking bonuses, and Orc swarm attacks (such as Moria Sniffers trying to overwhelm through numbers) cannot target rear-rank characters (such as Fáin or archers in Rearward stance).
  * **Shield-Bash Manoeuvre**: When the Band tests **War (3d6)** in combat, spending a Success icon ($\mathbf{6}$) allows the Phalanx to push the enemy frontline back 10 paces, creating room for devastating close combat strikes.

---

### 2.3 Role 3: Rearguard Choke Point Defense
* **Assigned Roster**: **Bláin the Cleaver** and **Fáin Iron-Visor** (2 Dwarves).
* **Positioning**: Stationed at a fortified fallback position, specifically **Keyed Location 2: The Upper Gatehouse**.
* **Operational Rules**:
  * **Securing the Line of Retreat**: Leaving 2 companions to fortify and hold Location 2 creates an active forward redoubt. The rearguard watches the murder-holes, keeps the barricades secure, and prevents enemy patrols from cutting off the transverse retreat gallery (Westward Drift) back to Thrym's Caves.
  * **Extraction Bonus**: During Act III's *Fighting Withdrawal*, having an established Rearguard provides **+1d** on Band extraction tests and grants the Company one automatic free round to cross the Mustering-Yard without taking missile fire.
  * **Keystone Trap Trigger**: Fáin and Bláin can be instructed to pull the ceiling winch keystone, triggering a controlled cave-in on pursuing Orc warbands (dealing 30 damage / crushing the pursuing vanguard).

---

### 2.4 Role 4: Heavy Salvage Porter Squad
* **Assigned Roster**: **Hjoldring the Armourer** and **Dúrmer the Iron-Shouldered** (assisted by 2 additional companions if available).
* **Positioning**: Formed in the center of the column with heavy pack litters, hemp ropes, and padded leather panniers.
* **Operational Rules**:
  * **Salvage Operation**: In **Location 8 (The Upper Armoury)**, the squad can pack up to 40 suits of ancient Dwarf mail-shirts, 30 shields, and crates of masterwork weapons.
  * **Burden Shift**: Packing this immense weight shifts the Band’s Burden rating from **Medium Burden** to **Heavy / Overburdened**:
    * Band Manoeuvre rolls suffer **-1d**.
    * Marching Fatigue tests suffer a **-1d penalty** (or **-2d** if fully Overburdened).
    * Stealth checks to move silently generate **+1 Noise Point** per hall traversed unless padded with cloth (**CRAFT roll**).
  * **Strategic Payoff**: Safely extracting this wargear back to the East-Gate Camp equips 40–50 frontline Dwarves in Balin’s vanguard with gromril-mail and masterwork weapons, eliminating the colony's defensive vulnerability and securing royal recognition from King Dáin.

---

## 3. Band Condition, Morale & Injury Systems

Subterranean warfare in Khazad-dûm exacts a brutal toll on living flesh and dwarven resolve. The Band tracks four interlocking operational metrics: **Band Hope**, **Band Shadow**, **Band Injury**, and **Band Fatigue**.

```
========================================================================================
                               BAND HEALTH & MORALE DASHBOARD
========================================================================================
 BAND HOPE POOL:      [ ■ ■ ■ ■ ■ ■ ■ ■ ■ ■ ■ ■ ] (12 / 12 Max Points)
 BAND SHADOW:         [ ■ □ □ □ □ □ □ □ □ □ □ □ ] (1 Point - Cleansed baseline)
 WEARY CONDITION:     TRIGGERED WHEN >= 4 COMPANIONS INJURED (SEVERE+) OR SPENT
 DESPERATE STAND:     HEROIC SACRIFICE ON FAILED CRITICAL TEST (G RUNE TO SURVIVE)
========================================================================================
```

---

### 3.1 Band Hope & Shadow Dynamics

* **Band Hope (Starting Pool: 12 Points)**:
  * Band Hope represents the collective courage, camaraderie, and divine resilience of Durin's Folk.
  * *Expending Hope*: A Player-Hero can spend 1 point of Band Hope to grant **+1d** on any Band Disposition roll (War, Vigilance, Manoeuvre, Expertise, Rally) or to activate a companion’s Gift in a critical moment.
  * *Restoring Hope*: Band Hope is restored by resting in a Safe Haven (Thrym's Caves), by Khoril playing his *Battle-horn of the Realm*, or by achieving major milestones (such as toppling the Balrog Idol in Location 4 or claiming Durin's Axe).
* **Band Shadow (Starting Rating: 1 Point)**:
  * Moria is an ancient wound in the earth, heavy with the spiritual malice of the Shadow.
  * *Accumulating Shadow*:
    * An ally suffers a **Severe or Grievous Injury** $\rightarrow \mathbf{+1\text{ Shadow Point (Dread)}}$.
    * An ally is **slain, lost, or sacrificed** $\rightarrow \mathbf{+2\text{ Shadow Points (Dread)}}$.
    * The Company witnesses an ancient desecration (e.g. the idol in Location 4) $\rightarrow$ Party rolls Dread tests; failure adds Shadow.
  * *Shadow Breakdown*: If Band Shadow ever exceeds Band Hope, the Band becomes **Miserable**. In this state, any roll showing the Eye of Sauron ($\mathbf{S}$) on the Feat die results in an automatic catastrophic failure and panic.

---

### 3.2 Band Injury System (The 5 Tiers)

When the Band is targeted by area attacks, traps, or melee clashes, injuries are resolved against individual companion profiles according to five formal severity tiers:

```
[FLEETING] ──(Prolonged Rest)──> [MODERATE] ──(First Aid)──> [SEVERE] ──(Emergency 1-Hr)──> [GRIEVOUS]
(Brief Rest)                     (Prolonged)                  (+1 Shadow)                    (Dying!)
```

```
========================================================================================
                               THE FIVE INJURY TIERS
========================================================================================
 TIER         SEVERITY & EFFECTS                       RECOVERY & TREATMENT
----------------------------------------------------------------------------------------
 1. Fleeting   Bruises, shallow cuts, winded (-2 End)   Cleared completely after a Brief 
                                                       Rest (30 minutes).
 2. Moderate   Deep gashes, cracked ribs (-5 End)       Cleared after a Prolonged Rest in 
                                                       a Safe Haven (Thrym's Caves).
 3. Severe     Broken bones, deep thrusts (-10 End).    Requires First Aid (**HEALING roll** 
               Adds +1 Band Shadow (Dread).             or Band EXPERTISE 2d vs Band TN 15) 
                                                       during Prolonged Rest to reduce to 
                                                       Moderate.
 4. Grievous   DYING! Mortal wound, artery severed.     MUST receive First Aid (**HEALING roll** 
               Adds +1 Band Shadow (Dread).             or Band EXPERTISE vs Band TN 15) within 
                                                       1 hour or companion dies. Prolonged 
                                                       rest reduces to Lingering.
 5. Lingering  Permanent crippling trauma. Second       Permanent until next Fellowship Phase. 
               Grievous wound causes INSTANT DEATH.     -1d on all individual actions.
========================================================================================
```

* **Endurance Test vs Damage Threat**: When a companion is struck by a trap or weapon blow, the GM rolls Band **RALLY** (1d6) against **Band TN 15 + Threat Modifier**:
  * *Bothersome Threat (+0)*: Arrow grazing, stone chip $\rightarrow$ Fleeting Injury.
  * *Painful Threat (+1)*: Orc scimitar strike, spear thrust $\rightarrow$ Moderate Injury.
  * *Vicious Threat (+2)*: Troll club, falling stalactite $\rightarrow$ Severe Injury.
  * *Dreadful Threat (+3)*: Cave-in crushing blow, Balrog fire $\rightarrow$ Grievous Injury.

---

### 3.3 Band Fatigue System (The 4 Tiers)

Marching through suffocating, airless shafts while carrying tons of wargear drains physical stamina. Fatigue progresses across four tiers:

$$\mathbf{\text{Fatigued}} \longrightarrow \mathbf{\text{Faltering}} \longrightarrow \mathbf{\text{Spent}} \longrightarrow \mathbf{\text{Collapsed}}$$

* **Fatigue Testing**: After every 4 hours of heavy subterranean travel, or after navigating toxic zones (Location 7), the GM rolls Band **RALLY** (1d6) against **Band TN 15 + Fatigue Points**, modified by Band Burden:
  * *Light Burden*: **+1d** on the test.
  * *Medium Burden (Standard)*: Standard roll (1d6).
  * *Heavy Burden (Salvage Mode)*: **-1d** on the test.
  * *Overburdened*: **-2d** on the test.
* **Fatigue Effects**:
  * *Fatigued*: Companions complain of aching limbs; no mechanical penalty.
  * *Faltering*: Band Hope cannot be spent on Rally rolls.
  * *Spent*: Band Readiness drops by 1 ($\text{TN 16}$).
  * *Collapsed*: Companions must be carried on litters; travel speed halved; counts as incapacitated.

---

### 3.4 The Band Weary Condition

If **50% or more (4 or more)** of the seven companions are incapacitated—meaning they are suffering from **Severe Injury, Grievous Injury, Spent Fatigue, or Collapsed status**—the entire Band becomes **Band Weary**.

$$\mathbf{\text{BAND WEARY EFFECT:}}\quad \text{All Success dice rolled on Band Disposition tests showing } \mathbf{1, 2, \text{ or } 3} \mathbf{\text{ count as } 0.}$$

* *Restoring Freshness*: The Weary condition is removed only when enough companions receive First Aid, hot food, and rest to bring the number of incapacitated allies below 4.

---

### 3.5 The Desperate Stand Mechanic

When a Player-Hero or companion is about to suffer a mortal blow, or when a critical Band test fails during a high-stakes escape, an uninjured or lightly injured companion can choose to make a **Desperate Stand** (self-sacrifice).

```
========================================================================================
                              DESPERATE STAND RESOLUTION
========================================================================================
 1. TRIGGER: A Player-Hero suffers a lethal wound, or a critical Band Clash fails.
 2. DECLARATION: A companion (e.g. Dolg or Dúrmer) throws themselves into the breach.
 3. RE-ROLL: The failed test is immediately re-rolled FAVOURED (roll 2 Feat dice, take best) 
    and INSPIRED (Success dice are doubled).
 4. SURVIVAL CHECK (THE FEAT DIE):
    • GANDALF RUNE (G): The companion miraculously survives the impossible feat, 
      suffering only a Moderate Injury!
    • ANY OTHER RESULT (1-10 or Eye): The companion is SLAIN or LOST in the dark. 
      The Company immediately gains +2 BAND SHADOW (Dread).
========================================================================================
```

---

## 4. Stealth & Marching Discipline

Navigating the echoing stone vaults of Khazad-dûm requires rigid marching discipline. The acoustics of Moria amplify the slightest scraping of mail or clinking of belt buckles.

```
========================================================================================
                             TACTICAL MARCHING FORMATIONS
========================================================================================
 FORMATION             CONFIGURATION                  OPERATIONAL BENEFIT / DRAWBACK
----------------------------------------------------------------------------------------
 A. Ingress Column     Single file; scouts 40 paces   Standard speed; +1d to detect traps; 
                       ahead; vanguard leads.         Vulnerable to broadside ambushes.
 B. Shadow Crawl       Felt-padded boots; muffled     Movement speed halved; all Stealth 
                       mail; lantern shuttered.       rolls gain +1d; -1 Noise on all actions.
 C. Phalanx Advance    Locked shields in front/rear;  Immune to surprise charges; movement 
                       weapons drawn.                 speed slow; generates +1 Noise.
 D. Withdrawal Column  Rearguard holding choke;       Maximum movement speed; +2 to escape 
                       porters in center.             clashes; rear rank exposed to arrows.
========================================================================================
```

---

### 4.1 Group Stealth & Marching Tests
* **Resolving Group Movement**: When the expedition attempts to traverse hostile sectors or bypass enemy sentry posts (such as the Moria sniffers in Location 1 or wandering patrols in Location 5), marching discipline is tested:
  * **Leader Check**: The company's Guide or leader makes a **TRAVEL roll** or **ENHEARTEN roll** (invoking a Trait such as *Leadership* for **+1d**).
  * **Alternative Band Check**: The GM rolls Band **MANOEUVRE** (2d6) against **Band TN 15** ($20 - \text{Readiness } 5$).
* **Noise Escalation & Consequences of Failure**:
  * **Failure**: Clanking wargear or loose gravel echoes down the galleries: **+1 Alert / Noise Point**.
  * **Eye of Sauron ($\mathbf{S}$)**: A dropped pry-bar or slipped shield clatters on stone (**+2 Alert / Noise Points**), immediately triggering a wandering scout patrol check.
* **Degrees of Success & Noise Reduction on $\mathbf{6}$s**:
  * **Success**: The squad moves silently in the shadows of the pillars: **+0 Noise Points** generated.
  * **Success Icons ($\mathbf{6}$)**: Each $\mathbf{6}$ rolled reduces ambient suspicion by **-1 Noise Point** (stepping in rhythm to muffle gear), or grants **+1d** to the Point Scout's next check.
  * **Gandalf Rune ($\mathbf{G}$)**: Flawless stealth; the squad discovers a forgotten maintenance flue, completely bypassing the next encounter zone.

---

## 5. Mass Combat: Battles & Clashes

When the expedition is engaged by organized Orc warbands (such as Grimnar's stalkers or garrison reinforcements), combat can be resolved using the **Band Clash** subsystem.

```
========================================================================================
                             BAND CLASH RESOLUTION FLOW
========================================================================================
  STEP 1: CHOOSE BAND STANCE (Aggressive / Balanced / Guarded / Fleeing)
                              │
                              ▼
  STEP 2: HERO LEADER ACTION (Command / Inspire / Fight / Duel)
                              │
                              ▼
  STEP 3: RESOLVE CLASH ROLL (Band War or Manoeuvre vs Band TN 15 + Foe Might)
                              │
                              ▼
  STEP 4: DAMAGE & RESISTANCE (Reduce Enemy Resistance; resolve companion casualties)
========================================================================================
```

---

### 5.1 Step 1: Band Stances
At the start of each combat round, the Company chooses a collective Band Stance:

1. **Aggressive Stance**:
   * *Effect*: The Band launches a ferocious assault. The Clash roll is **Ill-favoured**, but on any success, it automatically reduces enemy Resistance by an additional **1 point**.
2. **Balanced Stance**:
   * *Effect*: Standard operational stance. Full tactical flexibility; roll standard dice pools.
3. **Guarded Stance**:
   * *Effect*: The Band locks shields and focuses on survival. The Clash roll is **Favoured**, and the Band ignores the first Special Damage trigger rolled by the enemy.
4. **Fleeing Stance**:
   * *Effect*: The Band attempts to disengage. Roll Band **MANOEUVRE** (2d6) vs **Band TN 15 + Enemy Might**. Success allows the squad to retreat to the previous keyed location.

---

### 5.2 Step 2: Hero Leader Actions
During a Clash round, each Player-Hero takes a specialized Leader Action before the Band rolls:

* **Command**: A hero makes a **BATTLE roll** (invoking a Trait such as *Leadership* for **+1d**). On a success, the hero grants **+1d** to the Band’s Clash roll and can change the Band’s Stance mid-round.
* **Inspire**: A hero makes an **ENHEARTEN roll**. Success restores **1 point of Band Hope** or clears the *Faltering* fatigue condition from all companions.
* **Fight**: A hero attacks the enemy frontline in a close combat stance using their Combat Proficiency against the adversary's Parry rating. A successful attack reduces enemy Resistance by the weapon’s full Damage.
* **Duel**: A hero engages the enemy Champion (e.g. Grimnar the Disgraced) in single combat, nullifying the Champion's Might penalty on the Band Clash roll while locked in duel.

---

### 5.3 Step 3: Clash Roll & Enemy Resistance
* **The Clash Roll**: The GM rolls Band **WAR** (3d6) against $\mathbf{\text{Band TN 15}} + \text{Enemy Might}$.
* **Enemy War Party Profiles**:
  * **Patrol (3–4 Orcs)**: Might 0 | Resistance 3 | Threat: Bothersome (+0)
  * **Pack (6–8 Orcs)**: Might 1 | Resistance 6 | Threat: Painful (+1)
  * **Warband (12–16 Orcs)**: Might 2 | Resistance 9 | Threat: Vicious (+2)
  * **Horde (20+ Orcs / Black Uruks)**: Might 3 | Resistance 12 | Threat: Dreadful (+3)
* **Resolving Casualties**:
  * Each point of damage dealt by the Heroes and the Band reduces the Enemy Resistance pool. When Resistance reaches 0, the enemy breaks and flees.
  * If the Band fails the Clash roll, the enemy inflicts an **Injury Test** on 1d3 companions.



<!-- PAGE BREAK: Chapter 3 -->

---


# Chapter 3: Operational Mechanics, Alert Systems & Environmental Hazards

> *"The dark was not empty. It was full of listening stone, and every breath of wind through the cracked flues carried the stink of brimstone and old ash."*

---

## 1. The 4-Stage Alert Tracker Subsystem

To simulate high-stakes subterranean infiltration without resorting to arbitrary monster spawns or automatic, overwhelming total party kills, operations in the Third Deep are regulated by the **4-Stage Alert Tracker**.

The Alert Tracker measures the local garrison’s awareness, suspicion, and defensive readiness. It advances when the expedition generates **Noise Points** or allows enemy messengers to escape, and directly dictates enemy behavior, patrol frequency, ambient tension, and tactical modifiers.

```
========================================================================================
                                 4-STAGE ALERT TRACKER
========================================================================================
 [ALERT 0: QUIET SHADOWS] ──► [ALERT 1: UNEASE & SCENT] ──► [ALERT 2: HUNTED & BARRICADED] ──► [ALERT 3: DRUMS IN THE DEEP]
   Noise: 0–3 Points            Noise: 4–7 Points              Noise: 8–11 Points               Noise: 12+ Points
   • Routine patrols            • Udûn sniffers active         • Choke points fortified        • Full garrison assault
   • Surprise rounds allowed    • Sentry watches doubled       • Grimnar stalks expedition     • Tunnel seal countdown
========================================================================================
```

---

### 1.1 Alert Stage Profiles & Mechanical Impact

```
========================================================================================
                              ALERT STAGE DETAILED MATRIX
========================================================================================
 STAGE            ATMOSPHERE & SENSORY CUES    ENEMY POSTURE & BEHAVIOR    MECHANICAL MODIFIERS
----------------------------------------------------------------------------------------
 Alert 0:         Tomb-like stillness; cold    Sentries loose and drowsy;  All Stealth & Explore 
 Quiet Shadows    drafts; slow water drops     Orcs gambling or squabbling tests gain +1d. 
 (0–3 Noise)      in distant shafts.           behind crude hides.         Surprise rounds allowed.
----------------------------------------------------------------------------------------
 Alert 1:         Faint rhythmic knocks in     Udûn sniffers and cave bats Standard rolls. 
 Unease & Scent   walls (tom-tap, tap-tom);    dispatched to check drafts; Wandering encounter 
 (4–7 Noise)      scent of burnt grease.       sentries stand to arms.     check on Feat Die (S).
----------------------------------------------------------------------------------------
 Alert 2:         Distant horn echoes; sulfur  Barricades manned; iron     Awareness to avoid 
 Hunted &         smoke in galleries; torches  portals bolted from inside; ambushes loses -1d. 
 Barricaded       lit along high balconies.    Grimnar actively stalks.    Hunt Threshold -2.
 (8–11 Noise)
----------------------------------------------------------------------------------------
 Alert 3:         Thunderous "Doom, doom"      Full garrison mobilization; Revelation Episode! 
 Drums in Deep    drums shake the foundations; Black Uruk shock-troops      Boss/Ambush triggered. 
 (12+ Noise)      toxic fumes vented in ducts. seal the exit shafts.       6-Round Seal Countdown!
========================================================================================
```

---

#### Alert 0: Quiet Shadows (0–3 Noise Points)
* **Atmosphere**: The vast halls are wrapped in suffocating, tomb-like stillness. Faint drafts whistle through ancient ventilation flues, and slow water droplets chime like glass against distant flags.
* **Enemy Posture**: Orc sentry posts are relaxed. Goblins are asleep in hide tents, squabbling over bone dice, or gnawing cave-bat wings. Sentries suffer **-1d** to Awareness tests.
* **Tactical Benefits**:
  * All **STEALTH** and **EXPLORE rolls** made by Player-Heroes or the Band gain **+1d**.
  * The Company automatically gains a **Surprise Round** on any combat engagement initiated from stealth.
  * Heroes can make **SCAN rolls** without risk of alerting guards.

#### Alert 1: Unease & Scent (4–7 Noise Points)
* **Atmosphere**: Rhythmic, unsettling tapping sounds echo through stone walls (*tom-tap, tap-tom*). The acrid smell of burnt animal grease and guttering tallow torches drifts down from upper balconies.
* **Enemy Posture**: Orc lookouts realize something is amiss. Sentry watches are doubled, and two-man patrols of **Udûn Sniffers** accompanied by screeching cave bats are dispatched to investigate unusual drafts.
* **Tactical Impact**:
  * Standard dice pools apply (no bonus dice for stealth).
  * Whenever the Company spends more than 10 minutes in an unfortified location, the GM rolls a Feat Die: on the **Eye of Sauron ($\mathbf{S}$)**, a roaming Udûn Sniffer patrol enters the chamber.
  * Attempting to rest outside of a fortified redoubt restores only half normal Endurance.

#### Alert 2: Hunted & Barricaded (8–11 Noise Points)
* **Atmosphere**: Bellowing iron horn blasts echo between pillars. Choking sulfur smoke billows from brazier vents, and shadows dance wildly across the high vaulted roofs.
* **Enemy Posture**: The Orcs recognize a disciplined Dwarven infiltration. Major corridors and archways are barred with heavy cedar balks and iron chains. Murder-holes are manned by archers with poisoned arrows. **Grimnar the Disgraced** actively stalks the party, preparing a coordinated ambush.
* **Tactical Penalties**:
  * Awareness tests made to detect concealed ambushes lose **-1d**.
  * Forcing closed stone doors requires an **ATHLETICS roll (Ill-favoured or at -1d)** or generating **+2 Noise Points** with crowbars.
  * Strategic Eye Awareness Hunt Threshold is temporarily reduced by **2 points** (e.g. from 14 to 12).

#### Alert 3: Drums in the Deep (12+ Noise Points)
* **Atmosphere**: A deep, rolling rumble—*"Doom, doom, roll-and-go"*—shakes the bedrock of the mountain as the massive war-drums of Malech's Fortress begin their terrible cadence.
* **Enemy Posture**: Total garrison mobilization. Heavy Black Uruk shock-troops, armored Orc guards, and cave-trolls pour into the thoroughfares. Orc engineers begin winching down iron portcullises and dumping loose slag down the ascent shafts.
* **The Revelation Episode**: Triggers an immediate **Moria Revelation Episode** (see Section 2.2).
* **The Escape Countdown**: The GM initiates a strict **6-Round / 6-Turn Evacuation Timer**. If the Company does not reach and secure Keyed Location 2 (The Upper Gatehouse) before the countdown expires, the main transverse avenue (Westward Drift) back to Thrym's Safe Haven is collapsed or sealed with iron portcullises, forcing a desperate detour through the lethal lower goblin pits!

---

## 2. Sound & Action Economy Table

In the acoustics of Khazad-dûm, stone acts as an amplifier. Noise Points accumulate across delve actions, directly advancing the Alert Tracker and adding to the Company's Strategic Eye Awareness.

```
========================================================================================
                             SOUND & ACTION ECONOMY TABLE
========================================================================================
 DELVE ACTION PERFORMED                            NOISE PTS   ALERT TRACKER IMPACT
----------------------------------------------------------------------------------------
 Silent stealth movement / shadow crawl            +0 Noise    None
 Whisper communication / muffled lockpicking       +0 Noise    None
 Spoken conversation / opening stone chests        +1 Noise    Advances Alert by 1/4 Step
 Melee combat round (muffled weapons, 1-rd kill)  +1 Noise    Advances Alert by 1/4 Step
 Loud combat round (shouting, clashing iron, bow)  +2 Noise    Advances Alert by 1/2 Tier
 Extended combat engagement (4+ combat rounds)     +3 Noise    +1 Full Tier, +1 Eye Awareness
 Sledgehammering doors / Toppling stone idol       +3 Noise    +1 Full Tier, +1 Eye Awareness
 Firing Siege Ballista / Triggering Grond-ram      +4 Noise    +1 Full Tier, +1 Eye Awareness
 Sounding the Battle-horn of the Realm             +5 Noise    INSTANT ALERT 3! (+2 Eye)
 Claiming Durin's Axe from Royal Dais              Special     +4 STRATEGIC EYE AWARENESS!
========================================================================================
```

---

### 2.1 Strategic Eye Awareness & The Hunt Threshold
In addition to the immediate local Alert Tracker, actions within the Third Deep interact with the overarching **Strategic Eye Awareness** rules from *The One Ring 2e* (*Moria Supplement pp. 38–43*).

```
========================================================================================
                            STRATEGIC EYE AWARENESS TRACK
========================================================================================
 REGION CLASSIFICATION:       Dark Land (Default Base Hunt Threshold: 14)
 STARTING EYE AWARENESS:      0 Points (Post-Fellowship Phase clean baseline)
 HUNT THRESHOLD:              14 Points (Reduced to 12 at Alert Tier 2)
 REVELATION TRIGGER:          Triggers immediately when Eye Awareness >= Hunt Threshold!
========================================================================================
```

* **Gaining Eye Awareness**:
  * Loud actions generating 3+ Noise Points: **+1 Eye Awareness**.
  * Sounding the *Battle-horn of the Realm*: **+2 Eye Awareness**.
  * Claiming the royal artifact *Durin's Axe*: **+4 Eye Awareness**.
  * Orc Drummers using *Drums in the Deep* Fell Ability: **+3 Eye Awareness**.
  * Allowing enemy messengers or lookouts to escape:

```
========================================================================================
                            ENEMY DISCOVERY & ESCAPE MATRIX
========================================================================================
 ESCAPE CIRCUMSTANCE                               EYE AWARENESS GAIN
----------------------------------------------------------------------------------------
 Lone surviving Orc / Goblin flees combat           +1 Eye Awareness
 Organized scout or messenger slips into vents      +2 Eye Awareness
 Messenger reaches an active warband / garrison     +3 Eye Awareness
========================================================================================
```

---

### 2.1 Operational Tests: Degrees of Success (6 icons)
When Player-Heroes or the Band make operational checks (Stealth, Scan, Explore, Craft):
* **6 (One 6 icon)**: Action completed silently without raising the Alert Tracker (+0 Noise Points) or grants a minor tactical advantage.
* **66 (Two 6 icons)**: Flawless execution; generates 0 Noise Points and grants +1d to the lead hero's next test in the room.
* **Gandalf (G)**: Critical ancestral breakthrough; uncovers a hidden dwarven stash or bypasses a hazard completely without rolling.

---

### 2.2 Revelation Episodes in the Third Deep
When Strategic Eye Awareness reaches or exceeds the **Hunt Threshold (14)**, a **Revelation Episode** occurs immediately. The GM rolls a Feat Die on the table below:

```
========================================================================================
                        THIRD DEEP REVELATION EPISODE TABLE
========================================================================================
 FEAT DIE ROLL   EPISODE TYPE          TACTICAL CRISIS & ENEMY DEPLOYMENT
----------------------------------------------------------------------------------------
 1–3             Orc Assault           A reinforced warband of 8 Moria Orc Guards and 
                                       4 Udûn Sniffers charges the party's position.
 4–6             Grimnar's Ambush      Grimnar the Disgraced and 4 elite stalkers strike 
                                       from overhead catwalks with poisoned javelins.
 7–9             Terrors of the Dark   A Nameless Thing (The Wailing Horror) or Ash-wraith 
                                       crawls from a deep fissure; all roll Dread tests.
 10              The Shadow Looms      The air turns scalding hot; Durin's Bane stirs in 
                                       the abyss; all heroes gain 3 Shadow (Sorcery).
 Eye (S)         Ghâsh!                Durin's Bane unleashes a burst of demonic fire; 
                                       chambers fill with smoke; immediate flee required!
 Gandalf (G)     Dwarven Echoes        A secret stone shutter opens, providing an ancient 
                                       escape flue; Eye Awareness resets with NO ambush!
========================================================================================
```

* **Resetting Eye Awareness**: After the Revelation Episode is resolved (or escaped), Strategic Eye Awareness **resets to 0**.

---

## 3. Environmental Hazards of the Third Deep

The physical environment of the Third Deep is as lethal as its Orcish inhabitants. Centuries of neglect, volcanic venting, and demonic corruption have left three major environmental perils.

```
                  ┌──────────────────────────────────────────────┐
                  │      ENVIRONMENTAL HAZARDS OF THIRD DEEP     │
                  └──────────────────────┬───────────────────────┘
                                         │
         ┌───────────────────────────────┼───────────────────────────────┐
         ▼                               ▼                               ▼
 [BALROG NEUROTOXIC MIASMA]     [STRUCTURAL COLLAPSE]         [TAINTED WATER PERILS]
  • Preserved toxic breath       • Cracked lintels & vaults    • Untested springs & pits
  • Poison/Dread effects         • 30 Dmg keystone traps       • 1d12 Water Peril roll
  • Masterwork respirators       • Clearing rubble via War     • Sorcery/Poison effects
```

---

### 3.1 The Balrog Neurotoxic Miasma (*Breath of the Pit* / Poison of the Armouries)
* **Origin & Lore**: Preserved within the depressed vaults of **Location 7 (The Poisoned Halls)** and **Location 8 (The Upper Armoury)** is the lingering volcanic breath unleashed by Durin's Bane during the slaughter of the ancient Dwarven captains. It hangs as an oily, emerald-yellow vapor rising to chest height.

```
========================================================================================
                               TOXIC MIASMA DEGRADATION MATRIX
========================================================================================
 PROTECTION STATUS             EXPOSURE TESTING RATE        CONSEQUENCE ON FAILURE
----------------------------------------------------------------------------------------
 Unprotected                   PROTECTION test              Lose 4 Endurance and gain 1 
 (Raw breathing, open skin)    (Ill-favoured)               Shadow Point (Dread). On Eye (S): 
                               EVERY MINUTE                 Severe Poison (lose 8 End, Weary).
----------------------------------------------------------------------------------------
 Protected                     PROTECTION test              Lose 2 Endurance (Weary). 
 (Vinegar cloth & herbs)       (Standard roll)              On Eye (S): Suffer Severe Poison 
                               EVERY HOUR                   (lose 6 Endurance).
----------------------------------------------------------------------------------------
 Masterwork Respirator         COMPLETE IMMUNITY            No tests required for 4 hours.
 (Crafted dwarf-mask filter)   FOR 4 HOURS
========================================================================================
```

* **Countermeasures & Engineering Remedies**:
  1. **Crafting Masterwork Respirators**: Skill Endeavour (**Resistance 3**). Heroes make **CRAFT rolls** (invoking Traits like *Smith* or *Burglary* for **+1d**), or the Band tests **EXPERTISE** (2d vs Band TN 15). Success crafts squad filter masks utilizing oiled leather, charcoal granules, crushed sponge, and dwarf-goggles, granting **4 hours of complete immunity** for up to 10 characters.
  2. **Field Herbal Treatments**: A hero makes a **HEALING roll** or **CRAFT roll** (+1d if using dried herbs or vinegar). Soaking cloth in strong vinegar, chewing dried *Athelas* or mountain herbs, and sealing collars with grease downgrades the hazard to **Protected status** (1 check per hour).
  3. **First Aid Triage**: Treating the poison on an afflicted character requires a **HEALING roll** (Ill-favoured if severe) or Band **EXPERTISE** (2d vs Band TN 15).
  4. **Unjamming the Flue**: In Location 7, unjamming the ancient overhead iron damper lever requires a **CRAFT roll** or **ATHLETICS roll (Ill-favoured)** due to corroded iron. Success vents the entire hall in 3 rounds, but the shrieking iron generates **+3 Noise Points**.

---

### 3.2 Structural Collapse & Falling Masonry
Thousands of years of subterranean tremors and the colossal weight of Mount Silvertine have weakened the granite ribs of the Third Deep.

```
========================================================================================
                                STRUCTURAL COLLAPSE RULES
========================================================================================
 • TRIGGER CONDITIONS: Firing siege ballistas, triggering the Grond-ram, toppling the 
   Morgoth idol, or pulling the Upper Gatehouse keystone winch.
 • IMPACT ZONE: 30-foot radius area of effect.
 • DAMAGE & PROTECTION: All targets in the zone must make a PROTECTION test (or ATHLETICS roll):
   - On a Failure: Target suffers 20 Damage and is PINNED under fallen stone (Weary until freed).
   - On a Success: Target suffers 10 Damage and dives clear to the perimeter.
 • CLEARING RUBBLE: Pinned characters require an ATHLETICS roll or a Band WAR (3d vs Band TN 15) 
   test to extract before suffocating.
========================================================================================
```

---

### 3.3 Subterranean Water Perils Table
Untested water sources in the deeps are frequently befouled by mine runoff, Orc filth, or lingering demonic sorcery. Whenever a character drinks from an untested pool or cistern in the Third Deep, roll the **Feat Die**:

```
========================================================================================
                            SUBTERRANEAN WATER PERIL TABLE
========================================================================================
 FEAT DIE ROLL   WATER CONDITION & EFFECTS
----------------------------------------------------------------------------------------
 Gandalf (G)     Pristine Ancient Dwarven Spring: Pure, chilled mineral water. Restores 
                 2 points of Hope or clears 1 tier of Fatigue immediately!
 9–10            Icy Snowmelt: Freezing cold runoff from Silvertine; refreshing and safe.
 7–8             Orc-Filth: Befouled with offal and rancid grease. Moderate Poison 
                 (Endurance loss 4, cured by Brief Rest).
 5–6             Bitter Mineral Water: Heavy with alkali. Must pass a PROTECTION test 
                 or ENHEARTEN roll to force swallow; failure causes violent nausea 
                 (-1d on all rolls for 1 hour).
 3–4             Acidic Mine Runoff: Tainted with nitric acid and heavy metals. Severe 
                 Poison (8 Endurance loss and Weary condition; requires First Aid to purge).
 1–2             Malice of Durin's Bane: Tainted by demonic residue. The imbiber suffers 
                 2 Shadow Points (Sorcery) and experiences terrifying hallucinations.
 Eye (S)         The Lurker's Pool: The water is clean, but the pool is inhabited by a 
                 stone-crawler or guarded by a concealed Udûn sentry ambush!
========================================================================================
```



<!-- PAGE BREAK: Chapter 4 -->

---


# Chapter 4: Keyed Locations of the Armouries of the Third Deep

> *"The mansions of Durin were not built in a day, nor were they hollowed out by fear. Every arch was measured by the stars, every pillar squared with the song of iron; and though shadow now crawls where kings once walked, the stone remembers its masters."*  
> — *Ancient Longbeard Inscription, Third Deep Gallery*

---

## 1. Architectural Overview & Spatial Navigation

The **Armouries of the Third Deep** comprise an ancient subterranean military complex constructed during the First and Second Ages of Khazad-dûm. Situated along the same horizontal stratum just west of *The Caves of Thrym Thistlebeard* (Location 18 on the master elevation map) and directly above the magma-warmed lower foundries, the complex is reached via the paved **Westward Drift (Great Transverse Gallery)**. It was engineered to equip, train, and marshal the heavy vanguard of Durin's Folk during the wars against Morgoth and Sauron.

```
========================================================================================================================
                                      THREE-TIER ELEVATION & CONNECTION SCHEMATIC
========================================================================================================================

 [Level 3A: Upper Staging Tier]
  • The Caves of Thrym Thistlebeard (Safe Haven)
     │
     └──► [The Westward Drift / Great Transverse Gallery: Lateral Paved Ingress]
           │
           ▼
  ┌────────────────────────────────────────────────────────┐
  │ 1. THE MUSTERING-YARD                                  │ (Landing, Balrog Scorches, Grik Contact, Sentry Watch)
  │    • East: The Westward Drift to Safe Haven (Haven Gate)│
  │    • South: Fortified Archway to Location 2            │
  │    • Northeast: Choked Debris Gallery (Crawlway bypass)│
  └───────────────────────────┬────────────────────────────┘
                              │
                              ▼
  ┌────────────────────────────────────────────────────────┐
  │ 2. THE UPPER GATEHOUSE                                 │ (Adamant Blast-Doors, Fallback Redoubt, Keystone Trap)
  │    • North: Grand Archway to Location 1                │
  │    • South: Ramp Descent to Level 3B (Location 3)      │
  │    • Murder-Hole Parapets & Winch Room                 │
  └───────────────────────────┬────────────────────────────┘
                              │
                              ▼
 [Level 3B: Middle Arsenal Tier]
  ┌────────────────────────────────────────────────────────┐
  │ 3. THE FIRST ARMOURY                                   │ (Despoiled Weapon Racks, Orc Scrap-Traps, Black Venom)
  │    • North: Ascending Ramp to Location 2               │
  │    • South: Pillared Archway to Location 4             │
  │    • West: Smuggler's Flue bypass to Location 5        │
  └───────────────────────────┬────────────────────────────┘
                              │
                              ▼
  ┌────────────────────────────────────────────────────────┐
  │ 4. THE BROKEN HALL                                     │ (Desecrated Museum, Morgoth/Balrog Idol, Royal Cartouche)
  │    • North: Archway to Location 3                      │
  │    • South: Great Cedar Portals to Location 5          │
  │    • East: Sealed Arch of Durin III (Ancient Secret)   │
  └───────────────────────────┬────────────────────────────┘
                              │
                              ▼
  ┌────────────────────────────────────────────────────────┐
  │ 5. THE SECOND ARMOURY (Siege Workshop)                 │ (Grond-Ram, Torsion Ballistas, Counterweight Cranes)
  │    • North: Great Cedar Portals to Location 4          │
  │    • South: Heavy Bronze Double Doors to Location 6    │
  │    • West: Sealed Iron Pressure Lock to Location 7     │
  └─────────────┬────────────────────────────┬─────────────┘
                │                            │
  (West Ramp)   ▼              (South Ramp)  ▼
  ┌──────────────────────────┐ ┌────────────────────────────────────────────────────────┐
  │ 7. THE POISONED HALLS    │ │ 6. THE HALL OF THE MAULER (Level 3C Drill Arena)         │
  │    • Toxic Balrog Miasma │ │    • Lair of the Armoured Cave-Troll (AL 10)             │
  │    • Preserved Knights   │ │    • High Catwalks, Weapon Piles, Stalactite Hazards    │
  │    • Scribe's Desk &     │ │    • North: Bronze Doors to Location 5                   │
  │      Lead Tube Letter    │ │    • South: Grand Colonnade to Location 9                │
  └─────────────┬────────────┘ └───────────────────────────┬────────────────────────────┘
                │                                          │
                ▼                                          │
  ┌──────────────────────────┐                             │
  │ 8. THE UPPER ARMOURY     │                             │
  │    • Garrison Salvage    │                             │
  │      (50 Suits of Mail)  │                             │
  │    • Dead Goblin Looters │                             │
  │    • East: Pressure Flue ├─────────────────────────────┘
  └──────────────────────────┘                             │
                                                           ▼
 [Level 3C: Deep Sanctuary Tier]
  ┌────────────────────────────────────────────────────────┐
  │ 9. THE KING'S DOOR                                     │ (Star-Iron Portal, Ithildin Runes, Dual Master Locks)
  │    • North: Grand Colonnade to Location 6              │
  │    • West: Reinforced Flue from Location 8             │
  │    • Overhead: Ambush Parapet (Grimnar's Stalkers)     │
  │    • South: Threshold to Location 10                   │
  └───────────────────────────┬────────────────────────────┘
                              │
                              ▼
  ┌────────────────────────────────────────────────────────┐
  │ 10. THE LOWER ARMOURY (The Royal Vault)                │ (Black Marble Dais, DURIN'S AXE, Greater Hoard)
  │    • North: Inner King's Door Threshold                │
  │    • The Vault Blast Barricade (Brief Rest Redoubt)    │
  │    • Sacred Stasis Field & Relic Coffers               │
  └────────────────────────────────────────────────────────┘
========================================================================================================================
```

---

## 2. Global Tactical Rules Across All Keyed Locations

Every keyed location in this chapter interacts directly with the core systems established in Chapters 1, 2, and 3:

1. **The 4-Stage Alert Tracker**:
   - **Alert 0 (*Quiet Shadows*, 0–3 Noise)**: Stealth and Explore rolls gain **+1d**. The Company automatically gains a **Surprise Round** against unaware sentries.
   - **Alert 1 (*Unease & Scent*, 4–7 Noise)**: Standard rolls. Roaming Moria Sniffer patrols check the room on a Feat die result of the **Eye of Sauron ($\mathbf{S}$)**.
   - **Alert 2 (*Hunted & Barricaded*, 8–11 Noise)**: Awareness rolls to avoid ambushes lose **-1d**. Reinforced doors are barred. **Grimnar the Disgraced** actively stalks the party.
   - **Alert 3 (*Drums in the Deep*, 12+ Noise)**: Triggers an immediate **Revelation Episode**. A 6-round countdown begins before the Upper Gatehouse escape shafts are sealed.
2. **Sound Economy & Noise Generation**:
   - Routine whispering and shadow movement: **+0 Noise**.
   - Standard speech / opening stone chests: **+1 Noise**.
   - Melee combat round: **+1 Noise** (1-round quiet kill) to **+2 Noise** (clashing blades and shouted orders).
   - Extended combat (4+ rounds) or toppling stone idols: **+3 Noise** (+1 Alert Tier, +1 Eye Awareness).
   - Siege weapon firing / controlled cave-ins: **+4 Noise** (+1 Alert Tier, +1 Eye Awareness).
   - Sounding the *Battle-horn of the Realm*: **+5 Noise** (**Instant Alert Tier 3!** +2 Eye Awareness).
3. **Band Deployment Roles**:
   - **Forward Scout Screen**: Advance 50 paces ahead; roll Band **VIGILANCE** (2d) or Band **MANOEUVRE** (2d) against **Band TN 15** to spot traps and sentries.
   - **Shield-Wall Phalanx**: Form front rank; roll Band **WAR** (3d) against **Band TN 15** to bottle doorways and intercept melee attacks.
   - **Rearguard Choke Defense**: Held at Location 2 to guarantee the extraction corridor (+2 Band Readiness on escape).
   - **Heavy Salvage Porters**: Haul wargear from Location 8 (40 Dwarf Mail-shirts, 30 Shields, 50 Mattocks/Axes; shifts Band to Heavy Burden).
4. **Hero Signature Items & Flaws**:
   - ***The Broken Key***: Grants **Favoured** rolls (roll 2 Feat dice, take the better result) on all **SCAN rolls** for traps, concealed doors, and mechanisms.
   - ***Battle-horn of the Realm***: Grants **+1d to all BATTLE and RALLY rolls** and enables the *Rally Companions* action, but its acoustic roar adds **+1 Alert Tier** and **+2 Eye Awareness**.
   - ***Curse of Vengeance* Flaw**: Triggered by Orc desecrations and blasphemous idols; failing Dread tests sparks reckless rage and bonus Noise.
   - ***Dragon-sickness* Flaw**: Triggered by glittering gold-leaf, gems, and ancient hoards; failing tests compels the hero to covet and linger.

---

## 3. The Ten Keyed Locations

---

### Location 1: The Mustering-Yard

* **Role**: Infiltration Landing & Crossroads  
* **Elevation**: Upper Tier (Level 3A)  
* **Connections**:
  * *East*: The Westward Drift (Great Transverse Gallery) connecting horizontally eastward to *The Caves of Thrym Thistlebeard* (Safe Haven).
  * *South*: Broad 30-foot granite archway descending gently into **Location 2 (The Upper Gatehouse)**.
  * *Northeast*: Choked masonry gallery leading to a narrow ventilation crawlway that bypasses directly into Location 3 (**EXPLORE roll**).
* **Baseline Threat**: Alert Tier 0 (Quiet Shadows). Two Moria Sniffers stationed behind Pillar #4.

```
========================================================================================================================
                                        LOCATION 1: THE MUSTERING-YARD
========================================================================================================================
      [The Westward Drift from Safe Haven]
                   │
                   ▼ (High Overlook Portal)
    ┌────────────────────────────────────────────────────────┐
    │  [Pillar 1]     [Pillar 2]     [Pillar 3]    [Pillar 4]│ <── Moria Sentry Tent &
    │                                              (Sniffers)│     Smoldering Brazier
    │             ┌────────────────────────┐                 │
    │             │   SCORCHED PAVILION    │                 │
    │             │ (Balrog Scorch Marks & │                 │
    │             │  Cracked War-Plinth)   │                 │
    │             └────────────────────────┘                 │
    │  [Pillar 5]     [Pillar 6]     [Pillar 7]    [Pillar 8]│
    └───────────────────────────┬────────────────────────────┘
                                │
                                ▼
                   [To Location 2: Upper Gatehouse]
========================================================================================================================
```

#### Boxed Read-Aloud Text
> *You emerge from the shadows of the Westward Drift onto a high stone overlook. Before you opens an immense subterranean muster-plaza hewn from the mountain's black basalt. Colossal octagonal pillars, thick as ancient oaks, rise into the oppressive gloom forty feet above. Below lies a vast paved floor where Khazad-dûm's warlords once marshalled ten thousand axemen. Now, splintered spears and rusted iron shards lie scattered across cracked flagstones like dead leaves, and a cold, sulfur-tainted draft whispers through the dark.*

#### GM Sensory Reference Bullets
* **Lighting**: Total subterranean darkness. Faint, ghostly green bio-luminescent lichen clings in patches to the highest ceiling vaults forty feet overhead. Lantern beams cast long, dancing shadows behind the colossal octagonal pillars.
* **Drafts**: A steady, cold updraft pulls from the lower vents and shafts, carrying the bitter, acrid tang of brimstone, cold slag, and ancient soot.
* **Echoes**: Extreme acoustic resonance. The high basalt vault amplifies metal-on-stone contact; an unpadded mail-shirt or dropped coin rings out like a bell for a hundred paces.
* **Smells**: Old coal soot, damp stone, stale tallow grease, and the faint, biting ozone tang of rock vitrified by demonic fire.

#### Interactive Environmental Features
1. **The Scorched Pavilion**: A raised platform of dark granite (4 feet high) with shallow steps on three sides. The center bears the glassy, vitrified footprint of Durin's Bane—a four-foot circle of fused black glass where the demon stood during the Fall. The stone parapet grants **Half Cover (+1 Parry)** during ranged exchanges.
2. **Elevated Sentry Galleries**: Narrow stone ledges running along the east and west walls ten feet above the yard floor, accessed by shallow recessed wall-rungs. Provides an ideal vantage point for archers (Rearward stance without distance penalty).
3. **The Moria Sentry Post**: Tucked behind Pillar #4 sits a crude shelter of stitched cave-bear hides and blackened cedar poles. Inside, two **Moria Sniffers** crouch over a smoldering brazier of dried bat dung, squabbling over knucklebone dice.
4. **Discarded Charcoal Dispatch Slate**: Half-buried in cold ash near the sentry brazier lies a flat basalt fragment scrawled with crude orkish runes detailing watch rotations and guard warnings.

#### TOR 2e Skill Tests & Mechanics
* **Perimeter Infiltration — STEALTH roll**: Slip around the perimeter pillars to the First Armoury (Location 3) or Broken Hall (Location 4) without alerting sentries.
  * *Modifiers*: Alert Tier 0 grants **+1d**; massive basalt pillars provide total cover.
  * *Consequence of Failure*: A companion's boot or shield clips an iron brazier stanchion, generating **+1 Noise Point** (advancing the Alert Tracker) and alerting the sniffing hounds.
  * *Degrees of Success (6 icons)*:
    * **6**: The Company moves like smoke across the basalt floor; generates **+0 Noise** and grants **+1d** to the lead hero's next test in Location 2 or 3.
    * **66**: Flawless silent infiltration; bypasses sentries completely and identifies the optimal rear ambush angle.
* **Overlook Sentry Scouting — SCAN roll**: Spot the Moria sentry post behind Pillar #4 and identify patrol timing before stepping onto the open floor.
  * *Modifiers*: A hero utilizing *The Broken Key* rolls **Favoured**. Having a scout on point grants **+1d**.
  * *Consequence of Failure*: Sentry post is not spotted before entering open ground; Company loses the surprise round if combat starts.
  * *Degrees of Success (6 icons)*:
    * **6**: Discovers the 2 Moria Sniffers dozing behind Pillar #4, notes their bone-dice distraction, and spots the crude charcoal slate near their brazier.
    * **66**: Uncovers a discarded Orc message scroll regarding friction between Malech and Thu the Firespeaker (*"Malech demands more iron from the deep foundries. Let the Fire-priests choke on their smoke"*).
* **Ambush Assault — BATTLE roll**: Attack sentries from surprise from the high balconies or unseen pillars.
  * *Modifiers*: Attacking from surprise grants **+1d**.
  * *Consequence of Failure*: Sentries raise an alarm shout before being engaged (**+2 Noise Points**).
  * *Degrees of Success (6 icons)*:
    * **6**: Eliminates 2 sentries immediately before they sound a horn (**+1 Noise Point** total for a 1-round quiet kill).
    * **66**: The entire sentry squad is eliminated in absolute silence (**+0 Noise**).
* **Ancient Scorch Analysis — LORE roll or BATTLE roll**: Inspect the vitrified scorch marks on the pavilion.
  * *Modifiers*: Invoking *Enemy-lore (Orcs)* or *Dwarven Lore* grants **+1d**.
  * *Consequence of Failure*: Inconclusive inspection; wastes 10 minutes.
  * *Degrees of Success (6 icons)*:
    * **6**: Confirms scorch marks are vitrified Balrog footprints from 1980 T.A. and identifies ancient engineered Dwarven defensive kill-zones across the plaza.
* **Searching the Sentry Camp / Interrogating a Moria Sentry — SCAN roll, RIDDLE roll, or AWE roll**: Inspect the charcoal dispatch slate or interrogate a captured sniffer.
  * *Modifiers*: Invoking a relevant Trait such as *Enemy-lore (Orcs)*, *Fierce*, or *Cunning* grants **+1d**.
  * *Consequence of Failure*: The charcoal runes crumble or the sniffer spits black bile before dying; wastes 5 minutes.
  * *Degrees of Success (6 icons)*:
    * **6**: Recovers critical intelligence: confirms that **Grimnar the Disgraced** (a Mordor Orc) is stationed on the Lower Level defending the royal approach, reveals that his patrol carries the **Mithril Key** (which unlocks the secret pillar door by the Dimrill Stairs), and notes a crude sketch warning of the scythe tripwires in Location 3.

#### Tactical Band Deployment & Companion Operations
* **Forward Scout Screen**: Advance along the high eastern gallery. Roll Band **VIGILANCE** (2d) against **Band TN 15**. Success spots the Moria sentries instantly and allows the Company to bypass or neutralize them in a silent surprise round.
* **Marching Discipline**: A Player-Hero leading the Company rolls **TRAVEL** or **ENHEARTEN** (invoking *Leadership* Trait grants **+1d**), or Band **MANOEUVRE** (2d) against **Band TN 15**. Success allows the entire squad to traverse the 100-pace yard in complete silence (**+0 Noise generated**).

#### Sound & Alert Tracker Impact
* Crossing the yard in silent shadow-crawl: **+0 Noise**.
* Engaging the 2 Moria Sniffers and slaying them within 1 round: **+1 Noise**.
* Letting a Moria Sniffer blow a bone whistle or escape south: **+3 Noise (+1 Alert Tier, +2 Eye Awareness)**.
* Blowing the *Battle-horn of the Realm*: **+5 Noise (Instant Alert 3 / Drums in the Deep!)**.

#### Loot & Discoveries
* **Moria Sentry Pouch**: 8 crude iron pence, a bag of greasy dried mushrooms, and a jagged bone-handled skewer.
* **Scorched Rubble Cache** (**SCAN roll**): A rusted iron coffer containing 3 heavy Dwarven throwing mattocks (Damage 5, Injury 16) and an intact flask of *Dwarven Lantern Oil* (burns for 6 hours).

---

### Location 2: The Upper Gatehouse

* **Role**: Fortified Forward Rally Point & Retreat Choke Point  
* **Elevation**: Upper Tier (Level 3A)  
* **Connections**:
  * *North*: 30-foot granite archway ascending into **Location 1 (The Mustering-Yard)**.
  * *South*: Broad sloping rampway descending 20 feet into **Location 3 (The First Armoury)**.
  * *East*: Murder-hole defense gallery overlooking the southern rampway.
* **Baseline Threat**: Alert Tier 0. Unoccupied structural fortress; potential fallback bastion.

```
========================================================================================================================
                                       LOCATION 2: THE UPPER GATEHOUSE
========================================================================================================================
                   [From Location 1: Mustering-Yard]
                                  │
                                  ▼
    ┌────────────────────────────────────────────────────────┐
    │  [LEFT TURRET]          INNER GATEWAY        [RIGHT]   │
    │  • Arrow Slits       (Buckled Adamant       • Winch    │
    │  • Murder-Holes        Blast-Doors)           Room     │
    │                                                        │
    │               ┌────────────────────────┐               │
    │               │  FORTIFIED REDOUBT     │ <── Fallback  │
    │               │  (Timber & Iron Balks) │     Position  │
    │               └────────────────────────┘               │
    │                                                        │
    │  [KEYSTONE WINCH TRAP] ── Overhead Arch (30 Dmg Cave-In)│
    └─────────────────────────────┬──────────────────────────┘
                                  │ (Sloping Rampway)
                                  ▼
                   [To Location 3: The First Armoury]
========================================================================================================================
```

#### Boxed Read-Aloud Text
> *Before you looms the inner gatehouse of the Upper Armouries, a monumental bulwark of dressed granite hewn directly into the living mountain. Its massive blast-doors—forged of four-inch-thick adamant star-iron—hang buckled outward on massive pivots, warped by ancient explosives and rams. Heavy iron chains and counterweight blocks hang motionless from ceiling winches. Narrow murder-holes and arrow slits pierce the high flanking parapets, commanding the sloping ramp below. Though scarred by centuries of siege, the stonework stands unbroken—a natural fortress in the throat of the deep.*

#### GM Sensory Reference Bullets
* **Lighting**: Deep gloom. The narrow ceiling embrasures admit no light from above, creating deep shadow banks within the gatehouse archway.
* **Drafts**: A violent, whistling draft funnels through the buckled adamant doors, moaning with a low, rhythmic hum like an ancient horn.
* **Echoes**: The thick granite walls heavily muffle sound from within, deadening spoken conversation to a flat, dry whisper that does not carry into adjacent halls.
* **Smells**: Pulverized stone dust, dry iron rust, dried bat guano, and the crisp, clean draft of deep mountain rock.

#### Interactive Environmental Features
1. **Buckled Adamant Blast-Doors**: The two massive door leaves are warped outward at a 45-degree angle. By spending 10 minutes and using iron wedges and timber beams, the Company can wedge them into a rigid barricade, creating an impassable barrier against frontal charges.
2. **The Keystone Winch Trap**: Directly above the southern archway hangs a cracked granite keystone (weighing three tons) held in place by an ancient iron winch cable. The cable runs into the right-hand guard turret. A trip-rope or lever pull releases the winch, dropping the keystone and causing a **Controlled Cave-In** across the passage.
   * *Cave-In Damage*: Inflicts **30 Damage** (Protection roll 4d vs Injury TN 18) to all targets beneath it and permanently seals the southern rampway with ten tons of rubble (**+4 Noise Points**).
3. **Murder-Hole Defense Turrets**: Two elevated flanking guard chambers accessed by narrow stone spiral stairs. Companions stationed here have **Total Cover (+3 Parry, immune to ranged attacks from below)** and can fire down through murder-holes into the southern approach.

#### TOR 2e Skill Tests & Mechanics
* **Skill Endeavour: Fortifying the Forward Redoubt (Resistance 3)**:
  * *Context*: Bracing the buckled adamant blast-doors with cedar balks, iron pitons, and granite blocks.
  * *Resistance*: **3**. Each test represents 10 minutes of disciplined labor.
  * *Allowed Skills*: **CRAFT roll**, **ATHLETICS roll**, **BATTLE roll**.
  * *Modifiers*: Assisting companions or heroes with *Smith* or *Mighty* traits grant **+1d**.
  * *Consequence of Failure*: Sledgehammer slips or timber creaks loudly (**+1 Noise Point** per failed attempt); if unfinished after 3 attempts, the barricade is partial (+1 Parry instead of Total Cover).
  * *Degrees of Success (6 icons)*:
    * **6**: Contributes 2 points of Resistance.
    * **66**: Contributes 3 points of Resistance (completes the endeavour in a single 10-minute turn!).
  * *Outcome on Success*: Establishes the Gatehouse as a **Fortified Forward Redoubt**. A rearguard garrison stationed here receives **Total Cover (+3 Parry, immune to lower missile fire)** and adds **+2 Band Readiness** to the final Fighting Withdrawal.
* **Rigging the Keystone Winch Trap — CRAFT roll or BATTLE roll**:
  * *Modifiers*: **Ill-favoured** if attempted in pitch darkness without lantern light.
  * *Consequence of Failure*: The rusted winch jams or slips, dropping iron flakes (**+1 Noise Point**) and requiring a reset.
  * *Degrees of Success (6 icons)*:
    * **6**: Rigs a hair-trigger trip-rope. During withdrawal, triggering the trap is a free combat reaction, dropping 3 tons of granite (**30 Damage, Protection 4d vs Injury TN 18**) and permanently sealing the southern rampway with 10 tons of rubble (**+4 Noise Points**).
* **Discovering the Maintenance Flue — EXPLORE roll**:
  * *Modifiers*: A hero investigating with *The Broken Key* rolls **Favoured**.
  * *Consequence of Failure*: Flue entrance behind left turret remains hidden; Company must proceed down the main ramp into Location 3.
  * *Degrees of Success (6 icons)*:
    * **6**: Locates the narrow crawlway bypassing Location 3 directly into Location 5.
* **Opening the Armoire Wall-Locker — CRAFT roll or STEALTH roll**:
  * *Modifiers*: Invoking the *Burglary* Trait (or utilizing *Vaultbreaker* gifts) grants **+1d**.
  * *Consequence of Failure*: Heavy bronze lock mechanism jams solid; forcing it requires iron crowbars (**+1 Noise Point**).
  * *Degrees of Success (6 icons)*:
    * **6**: Silent opening; recovers 24 masterwork arrows (+1 Ranged Damage), 4 iron grappling hooks with 50ft rope, and a flask of *Dwarven Blasting Pitch*.

#### Tactical Band Deployment & Companion Operations
* **Rearguard Choke Point Defense (Critical Decision)**:
  * The Company may elect to leave **2 to 3 veteran companions** to garrison the Upper Gatehouse.
  * *Mechanical Benefits*:
    1. Guarantees that the transverse retreat avenue (Westward Drift) back to Thrym's Haven cannot be captured or cut off by wandering Udûn patrols.
    2. Grants an automatic **+2 bonus to Band Readiness** during the final extraction.
    3. Allows the rearguard to man the murder-holes and keystone trap to cover the heroes' retreat.
* **Phalanx Fortification**: Companions with *Mighty* and *Smith* traits haul massive iron plates from the broken doors to build an arrow-proof firing slit across the main threshold.

#### Sound & Alert Tracker Impact
* Shoring timber and hammering iron wedges quietly: **+1 Noise**.
* Sledgehammering stone or forcing rusty winches without grease: **+3 Noise (+1 Alert Tier)**.
* Triggering the Keystone Cave-In: **+4 Noise (+1 Alert Tier, +1 Eye Awareness)**.

#### Loot & Discoveries
* **Gatehouse Armoire**: A sealed bronze wall-locker containing 2 bundles of masterwork Dwarven arrows (24 arrows, +1 to ranged Damage), 4 iron grappling hooks with 50 feet of braided hemp rope, and a stone flask of *Dwarven Blasting Pitch* (alchemical accelerant).

---

### Location 3: The First Armoury

* **Role**: Despoiled Outpost & Trap Hazard  
* **Elevation**: Middle Tier (Level 3B)  
* **Connections**:
  * *North*: Ascending rampway to **Location 2 (The Upper Gatehouse)**.
  * *South*: Broad pillared archway opening into **Location 4 (The Broken Hall)**.
  * *West*: Narrow maintenance duct connecting to the siege workshop in Location 5 (**EXPLORE roll**).
* **Baseline Threat**: Alert Tier 0–1. Orcish tripwire network; discarded black-venom blades.

```
========================================================================================================================
                                        LOCATION 3: THE FIRST ARMOURY
========================================================================================================================
                   [From Location 2: Upper Gatehouse]
                                  │
                                  ▼
    ┌────────────────────────────────────────────────────────┐
    │  [Weapon Rack A]     [Weapon Rack B]    [Weapon Rack C]│
    │  (Stripped Bare)     (Stripped Bare)    (Stripped Bare)│
    │                                                        │
    │   ═ ═ ═ ═ ═ [TRIPWIRE 1: Counterweight Scythe] ═ ═ ═ ═ │ <── Black Venom
    │                                                        │      Tripwire
    │  [Weapon Rack D]     [Weapon Rack E]    [Weapon Rack F]│      Hazard Zone
    │  (Orc Bone Debris)   (Loose Flagstone)  (Poison Vats)  │
    │                                                        │
    │   ═ ═ ═ ═ ═ [TRIPWIRE 2: Falling Caltrop Rack] ═ ═ ═ ═ │
    └─────────────────────────────┬──────────────────────────┘
                                  │
                                  ▼
                   [To Location 4: The Broken Hall]
========================================================================================================================
```

#### Boxed Read-Aloud Text
> *You descend into a long, low-vaulted hall where twin rows of stone weapon racks stand plundered and desolate, their bronze fittings and velvet linings stripped away long ago. The floor is strewn with discarded refuse: shattered wooden bucklers, gnawed bones, and rusted iron debris scattered across the flagstones. Cold drafts stir the stale air, and the low ceiling casts heavy shadows across the despoiled aisle ahead.*

#### GM Sensory Reference Bullets
* **Lighting**: Complete subterranean blackness. The iridescent, oily sheen of black Orc-venom on the tripwire blades catches lantern rays with a greasy purple glare.
* **Drafts**: Stagnant, dead air smelling of damp rot and stagnant grease; no perceptible breeze stirs the dust.
* **Echoes**: The low stone ceiling (12 feet high) creates a sharp, slapping echo. Every kicked pebble or scuffed iron boot rattles loudly across the floor.
* **Smells**: The sickening, sweet stench of rotting marrow, stale Orc dung, and the pungent, bitter almond tang of Orcish black venom.

#### Interactive Environmental Features
1. **The Scythe-Blade Scrap-Traps**: Two interlocking tripwire lines stretched 4 inches above the floor across the main aisle. Stepping on a wire releases heavy overhead scythe-arms made of scavenged Dwarven broad-axes lashed to iron bars.
   * *Trap Stats*: Attack roll 4d vs **Target Strength TN + Shield**. On a hit, inflicts **14 Damage** and a **Piercing Blow (Injury TN 16)**. The blade is coated in **Black Orc-Venom** (*see below*).
2. **Black Orc-Venom Vats**: Two rusted iron cauldrons in the corner filled with coagulated viper-bile, nightshade resin, and coal tar.
   * *Poison Effect*: A character wounded by a poisoned blade must make a **PROTECTION test** or pass a **HEALING roll**. Failure inflicts the **Weary condition** and the loss of 2 Endurance per hour until treated with First Aid.
3. **Concealed Stone Arming Cache**: Beneath stripped weapon rack #12, a loose floor-flag bears a faint hammer-rune at its corner.
   * *Contents*: Conceals a sealed cedar chest containing 4 pristine Dwarven war-mattocks and 6 steel throwing knives untouched by the Orcs.

#### TOR 2e Skill Tests & Mechanics
* **Detecting Sinew Tripwires — SCAN roll**: Detect the taut sinew tripwires and caltrop fields before stepping into the crossfire.
  * *Modifiers*: A hero investigating with *The Broken Key* rolls **Favoured**. Having a scout on point grants **+1d**.
  * *Consequence of Failure*: The tripwire is missed; lead character steps on the cord, triggering the trap!
  * *Degrees of Success (6 icons)*:
    * **6**: Spots both interlocking tripwire lines, the caltrop field, and the loose floor-flag concealing the arming cache beneath rack #12.
* **Skill Endeavour: Disarming the Scythe Scrap-Trap Network (Resistance 3)**:
  * *Context*: Wedging counterweighted scythe pivots and snipping taut sinew cords silently.
  * *Resistance*: **3**.
  * *Allowed Skills*: **CRAFT roll**, **STEALTH roll**, **SCAN roll**.
  * *Modifiers*: Invoking the *Burglary* Trait (or *Vaultbreaker*) grants **+1d**.
  * *Consequence of Failure*: On a failed roll, the mechanism slips (**+1 Noise Point**). If a Feat die shows an Eye of Sauron ($\mathbf{S}$), the scythe swings down! Lead character faces an attack (4d vs Strength TN + Shield; on hit: 14 Damage, Piercing Blow Injury TN 16 coated in Black Venom, **+2 Noise Points**).
  * *Degrees of Success (6 icons)*:
    * **6**: Contributes 2 points of Resistance.
    * **66**: Entire trap network disarmed silently in a single action (**+0 Noise**).
* **Harvesting Black Orc-Venom — HEALING roll or CRAFT roll**: Safely scrape and harvest the black venom from the vats into glass vials.
  * *Consequence of Failure*: Hands shake; vial shatters or blade pricks skin (hero must make Protection test vs Injury TN 14 or suffer *Weary* + lose 2 Endurance per hour).
  * *Degrees of Success (6 icons)*:
    * **6**: Safely harvests 3 pristine vials of *Black Orc-Venom* (applying to a blade grants **+2 Injury Rating** for 1 combat encounter).
* **Re-Arming Traps for Rearguard — CRAFT roll**: Re-arm the scythe traps to face backwards toward Location 2.
  * *Consequence of Failure*: Traps disabled permanently; cannot be reused.
  * *Degrees of Success (6 icons)*:
    * **6**: Re-arms scythe blades facing north; during Act III withdrawal, automatically ambushes the Orc vanguard (delays pursuit by 2 rounds).
* **Scavenging Concealed Cache — HUNTING roll or CRAFT roll**:
  * *Consequence of Failure*: Nothing usable recovered from the scrap.
  * *Degrees of Success (6 icons)*:
    * **6**: Sifting through the debris recovers 4 pristine Dwarven War-Mattocks (Damage 6, Injury 18, Break Shield), 6 Throwing Knives (Damage 3, Injury 14), and 2 intact iron crowbars.

#### Tactical Band Deployment & Companion Operations
* **Forward Scout Screen**: Companions with *Vaultbreaker* gifts use copper-ringed picks to wedge trap pivots while scouts map the safe path around the caltrops.
* **Salvage Assessment**: Companions with the *Smith* trait examine the stripped racks, confirming that the Orcs only carried away crude iron weapons, leaving the heavy gromril storehouses further down undisturbed.

#### Sound & Alert Tracker Impact
* Silently creeping through and disarming traps: **+0 Noise**.
* Accidental trap trigger (scythe blade smashing stone): **+2 Noise (+1/2 Alert Tier)**.
* Searching through scrap heaps loudly: **+1 Noise**.

#### Loot & Discoveries
* **Concealed Arming Cache**: 4 Dwarven War-Mattocks (Damage 6, Injury 18, Break Shield) and 6 Throwing Knives (Damage 3, Injury 14).
* **Scavenged Orc Pouch**: 14 brass coins minted in Gundabad, a piece of dried troll-flesh, and an iron key that fits the lockbox in Location 8.

---

### Location 4: The Broken Hall

* **Role**: Dread Encounter, Lore Clue & Psychological Crucible  
* **Elevation**: Middle Tier (Level 3B)  
* **Connections**:
  * *North*: Pillared archway leading back to **Location 3 (The First Armoury)**.
  * *South*: Massive double cedar doors bound in bronze opening into **Location 5 (The Second Armoury)**.
  * *East*: Sealed decorative wall-arch displaying the defaced *Mural of the First Kings* (conceals secret Old Moria architectural glyphs).
* **Baseline Threat**: Alert Tier 0–1. Supernatural Dread aura radiating from the central dark idol.

```
========================================================================================================================
                                        LOCATION 4: THE BROKEN HALL
========================================================================================================================
                   [From Location 3: First Armoury]
                                  │
                                  ▼
    ┌────────────────────────────────────────────────────────┐
    │  DEFACED FRIESES OF DURIN'S KINGS (Blood & Obsidian)   │
    │                                                        │
    │              ┌──────────────────────────┐              │
    │              │  THE JAGGED BALROG IDOL  │ <── Radiates │
    │              │  (Charred Bones, Iron,   │     Dread    │
    │              │   Molten Gold Veins)     │     Aura     │
    │              └──────────────────────────┘              │
    │                                                        │
    │  [SECRET CARTOUCHE] ── Low Border of East Wall Frieze   │
    │  (Reveals Dual King's Key & Mithril Key Cipher)        │
    └─────────────────────────────┬──────────────────────────┘
                                  │ (Bronze-Bound Doors)
                                  ▼
                   [To Location 5: The Second Armoury]
========================================================================================================================
```

#### Boxed Read-Aloud Text
> *You enter a grand memorial hall that once served as Khazad-dûm's martial museum. Soaring vaults hang twelve meters overhead, supported by fluted pillars of gray marble. But the glory of elder days has been ruthlessly desecrated: magnificent wall friezes have been shattered with hammers, smeared with dried blood, and gouged with crude runes in the Black Speech. In the center of the hall looms a monstrous silhouette: a twelve-foot effigy of jagged black iron, charred bones, and slag, shaped into the winged terror of the Shadow. An oppressive, biting cold radiates from the dark metal, whispering despair into the gloom.*

#### GM Sensory Reference Bullets
* **Lighting**: Pitch black. The jagged iron idol seems to actively drink lantern light, casting distorted, wing-like shadows across the defaced walls.
* **Drafts**: A sudden, sickeningly warm downdraft emanates from the dark idol, carrying the faint, sickening scent of smoldering blood and hot grease.
* **Echoes**: Unnaturally deadened acoustics. Spoken words sound flat, hollow, and choked, as if the room itself is suffocating under an ancient spiritual weight.
* **Smells**: Coagulated blood, charred bone, burnt hair, sulfur, and the cold, oppressive stench of ancient malice.

#### Interactive Environmental Features
1. **The Balrog/Morgoth Dark Idol**: A 12-foot totem constructed from jagged iron bars, slagged armour plates, and the charred bones of slaughtered Dwarves. The idol acts as a focal point for the lingering Shadow of Durin's Bane.
2. **Defaced Wall of Kings & The Secret Cartouche**: A 30-foot frieze on the east wall depicting the coronation of Durin III. While the king's face has been smashed away, the decorative runic border along the base remains intact beneath a layer of soot.
   * *The Architectural Clue*: Deciphering the runes (**LORE roll or RIDDLE roll**) reveals the master cipher of the Armouries: *"The Vault of the Crown shall yield neither to pick nor hammer, save when the Mithril Ring turns the left ward and the King’s Signet turns the right."* This proves that opening **Location 9 (The King's Door)** requires both the *Mithril Key* and *King's Key* (or an extraordinary lockpicking bypass).
3. **Toppling the Effigy**: The iron idol is anchored to a cracked stone plinth with rusted iron pins. Can be toppled via the Skill Endeavour below.

#### TOR 2e Skill Tests & Mechanics
* **Resisting Spiritual Dread — AWE roll or ENHEARTEN roll**: Resist the spiritual corruption of the Balrog Idol.
  * *Modifiers*: Dwarves testing vs Orc/Balrog desecration test **Ill-favoured** if they have an applicable active Flaw.
  * *Consequence of Failure*: The character gains **2 Shadow Points (Dread)** and becomes **Miserable** until taking a Rest.
    * *Curse of Vengeance Flaw*: On a failure, a hero possessing the *Curse of Vengeance* Flaw gains **1 additional Shadow Point (Dread)** and must spend their next action striking the blasphemous effigy (generating **+2 Noise Points** and 2 Fatigue).
    * *Dragon-sickness Flaw*: On a failure, a hero possessing the *Dragon-sickness* Flaw is transfixed by the gold-leaf veins, wasting 10 minutes attempting to pry them free before moving on.
  * *Degrees of Success (6 icons)*:
    * **6**: Unshakable resolve; completely immune to the room's Dread aura and grants **+1d** to an adjacent ally's test.
    * **66**: Resolute defiance; inspires the Company, clearing the **Miserable** condition from all companions and restoring **+1 Hope**.
* **Deciphering the Royal Cartouche — LORE roll or RIDDLE roll**: Decipher the hidden cartouche on the east wall frieze.
  * *Modifiers*: A Dwarf of Durin lineage gains **+1d**.
  * *Consequence of Failure*: Runic cipher remains garbled; key mechanics of the King's Door remain unknown until Location 9.
  * *Degrees of Success (6 icons)*:
    * **6**: Translates the master cipher (*"The Vault of the Crown shall yield neither to pick nor hammer, save when the Mithril Ring turns the left ward and the King's Signet turns the right"*), confirming that the King's Door requires both keys or an extraordinary bypass endeavour.
* **Banish the Gloom — SONG roll or ENHEARTEN roll**: A Player-Hero sings a stanza of *The Song of Durin* or raises a solemn chant to banish the oppressive gloom.
  * *Consequence of Failure*: Voice falters in the oppressive gloom; no effect.
  * *Degrees of Success (6 icons)*:
    * **6**: Singing *The Song of Durin* cleanses the oppressive aura, removes **Miserable** from all heroes, and restores **+1 Band Hope**.
* **Skill Endeavour: Controlled Toppling of the Balrog Idol (Resistance 3)**:
  * *Context*: Rigging crowbars and blanketing the stone base with canvas to topple the 12-foot iron effigy without alarming the deeps.
  * *Resistance*: **3**.
  * *Allowed Skills*: **ATHLETICS roll**, **CRAFT roll**.
  * *Modifiers*: Invoking *Mighty* grants **+1d**; wrapping in blankets/felt (*Craft*) reduces acoustic impact.
  * *Consequence of Failure*: Idol crashes unpadded onto granite flagstones, generating **+3 Noise Points (+1 Alert Tier, +1 Eye Awareness)**.
  * *Degrees of Success (6 icons)*:
    * **6**: Contributes 2 points of Resistance.
    * **66**: Idol is lowered gently in absolute silence (**+0 Noise**), restores **+1 Band Hope**, and allows 30 silver pennies (3 Treasure) of gold scrap to be safely recovered.
* **Prying the Idol's Jewels — STEALTH roll or CRAFT roll**:
  * *Modifiers*: Invoking the *Burglary* Trait grants **+1d**.
  * *Consequence of Failure*: The precarious skull tilts, dislodging iron scrap (**+1 Noise Point**).
  * *Degrees of Success (6 icons)*:
    * **6**: Pries the embedded jewel without destabilizing the bone frame, recovering an *Elven Tourmaline* (worth 15 Treasure Points).

#### Tactical Band Deployment & Companion Operations
* **Spiritual Bastion**: A companion or hero raises their voice in command to brace the squad's resolve against the shadow.
* **Scouting the Secret Flue**: Scouts examine stone seams to locate an ancient ventilation shaft in the ceiling leading toward the siege workshops in Location 5.

#### Sound & Alert Tracker Impact
* Whispering and inspecting the friezes quietly: **+0 Noise**.
* Toppling the Balrog Idol onto stone flags: **+3 Noise (+1 Alert Tier, +1 Eye Awareness)**.
* Muffling the idol with blankets before toppling: **+1 Noise**.

#### Loot & Discoveries
* **Pried Gold-Leaf Veins**: If safely pried from the idol, yields 30 silver pennies' worth of refined gold scrap.
* **Smashed Relic Pedestal**: Beneath the rubble of the king's frieze lies a shattered display case containing an ancient *Dwarven Bronze Astrolabe* (+1d to Explore tests in mountain halls) and a scroll case of fire-hardened parchment detailing the defensive layout of Level 3B.

---

### Location 5: The Second Armoury (The Siege Workshop)

* **Role**: Tactical Sandbox & Heavy Artillery  
* **Elevation**: Middle Tier (Level 3B)  
* **Connections**:
  * *North*: Massive bronze-bound double doors to **Location 4 (The Broken Hall)**.
  * *South*: Colossal iron-reinforced blast doors leading into **Location 6 (The Hall of the Mauler)**.
  * *West*: Sealed iron pressure-door with wheel-latch leading into **Location 7 (The Poisoned Halls)**.
* **Baseline Threat**: Alert Tier 0–1. Massive mechanical war engines; potential area-of-effect tactical weapons.

```
========================================================================================================================
                                     LOCATION 5: THE SECOND ARMOURY
========================================================================================================================
                   [From Location 4: The Broken Hall]
                                  │
                                  ▼
    ┌────────────────────────────────────────────────────────┐
    │  [WORKBENCHES & SPARE TORSION SPRINGS]                 │
    │                                                        │
    │  ┌──────────────────────┐    ┌──────────────────────┐  │
    │  │ THE GROND-RAM        │    │ TWIN-LIMB TORSION    │  │ <── Functional
    │  │ (Iron-Headed Ram on  │    │ BALLISTA (6-ft Bolt) │  │     Siege Engines
    │  │  Bronze Trackways)   │    └──────────────────────┘  │
    │  └──────────┬───────────┘                              │
    │             │ (Aimed South)                            │
    │             ▼                                          │
    │  [COUNTERWEIGHT HOIST CRANE] ── (2-Ton Granite Block)  │
    │                                                        │
    │  [WEST PRESSURE-DOOR] ── Sealed Entry to Location 7   │
    └─────────────────────────────┬──────────────────────────┘
                                  │ (Iron Blast Doors)
                                  ▼
                   [To Location 6: The Hall of the Mauler]
========================================================================================================================
```

#### Boxed Read-Aloud Text
> *You step into a vast, echoing workshop hall filled with the scent of dry cedar, refined machine oil, and aged pine resin. Massive timber scaffoldings rise thirty feet into the darkness above. Arrayed across the floor stand the great siege engines of the Third Deep—monuments to dwarven mechanical mastery. Covered war machines rest upon bronze guide-tracks facing the southern gates, while heavy torsion ballistas and counterweight cranes hang from overhead beams, their skeins preserved in oiled leather casings.*

#### GM Sensory Reference Bullets
* **Lighting**: Deep gloom. Lantern light reflects off polished brass fittings, greased iron guide rails, and oiled leather casings.
* **Drafts**: A gentle, dry draft filters through high ceiling ventilation flues, rustling dried canvas dust-covers.
* **Echoes**: The immense stacks of aged cedar timbers and canvas sheets absorb high frequencies, deadening footfalls to a soft wooden thud.
* **Smells**: Rich cedarwood, linseed oil, pine resin, cured animal sinew, and axle grease.

#### Interactive Environmental Features
1. **The Grond-Ram of the Third Deep**: A massive, 20-foot siege ram tipped with a solid cast-iron ram-head shaped like a snarling dwarf-dragon. It rests on greased bronze guide tracks aligned with the southern double doors to Location 6.
   * *Releasing the Ram*: Pulling the master release lever sends the ram hurtling down the inclined track. It smashes through the southern doors with thunderous force, crashing into Location 6!
   * *Tactical Effect*: Smashing into Location 6 automatically deals **25 Damage** (ignores armour) to **The Mauler** if the troll is standing near the doors, or knocks it Prone and strips 2d of its scrap armour!
   * *Acoustic Penalty*: Firing the ram generates **+4 Noise Points (+1 Alert Tier, +1 Eye Awareness)**.
2. **Heavy Twin-Limb Torsion Ballista**: Mounted on a rotating bronze turntable. It is loaded with a six-foot steel-tipped harpoon bolt connected to an iron chain.
   * *Operating the Ballista*: Requires 2 crew members and an **ATHLETICS roll** or **CRAFT roll** to aim and fire.
   * *Ballista Stats*: Ranged attack roll 4d vs **Target TN**. On a hit, inflicts **30 Damage** and a **Piercing Blow (Injury TN 22)**. If fired at The Mauler, the harpoon anchors into its flesh/armour, pinning the brute to the doorway.
3. **The Counterweight Hoist Crane**: A heavy ceiling crane rigged with a 2-ton block of dressed granite. Dropping the counterweight creates a devastating crushing trap (inflicts 25 damage in a 10-foot radius) or barricades an archway permanently.
4. **Stacked Cedar Balks**: Piles of interlocking 10-foot cedar beams that can be assembled in 10 minutes (**CRAFT roll**) to create a fortified chokepoint (+2 Parry, Half Cover) across the northern entrance.

#### TOR 2e Skill Tests & Mechanics
* **Skill Endeavour: Calibrating & Arming the Siege Engines (Resistance 3)**:
  * *Context*: Oiling bronze guide-tracks, stringing preserved sinew skeins, and loading the 6-foot star-iron harpoon.
  * *Resistance*: **3**.
  * *Allowed Skills*: **CRAFT roll**, **ATHLETICS roll**.
  * *Modifiers*: Assisting companions with *Smith* grant **+1d**; Dwarven engineering tools grant **+1d**.
  * *Consequence of Failure*: Rusted gears grind loudly (**+1 Noise Point** per failed attempt); takes 20 minutes instead of 10.
  * *Degrees of Success (6 icons)*:
    * **6**: Contributes 2 points of Resistance.
    * **66**: Both the Grond-ram and Torsion Ballista are calibrated and cocked with masterwork precision.
* **Manhandling & Re-aiming Artillery — ATHLETICS roll**:
  * *Alternative*: Band **WAR** (3d) against **Band TN 15**.
  * *Consequence of Failure*: Heavy winch slips; takes 4 rounds instead of 2.
  * *Degrees of Success (6 icons)*:
    * **6**: Pivots the heavy turntable and aligns sights in a single combat round.
* **Designing the Chokepoint Kill-Zone — BATTLE roll**:
  * *Consequence of Failure*: Poor tactical angles; siege engines cannot cover flanking archways.
  * *Degrees of Success (6 icons)*:
    * **6**: Flawless tactical placement; the companion band gains **+1d** to all Band Clash and ranged rolls while fighting in Location 5.
* **Unlocking the Engineer's Strongbox — CRAFT roll or STEALTH roll**:
  * *Modifiers*: Invoking the *Burglary* Trait (or *Vaultbreaker*) grants **+1d**.
  * *Consequence of Failure*: Complex pin-tumbler jams; must be forced open with crowbars (**+1 Noise Point**).
  * *Degrees of Success (6 icons)*:
    * **6**: Silent opening; recovers 3 masterwork steel calipers, 2 flasks of *Dwarven Lubricant Oil*, a heavy *Iron Spanner* (acts as a club: Damage 4, Injury 14, Break Shield), and 4 star-iron harpoon heads.

#### Tactical Band Deployment & Companion Operations
* **Artillery Crew**: Stationing companions with archery or mechanics gifts at the ballista sights manages tension winches and sighting.
* **Barricade Builders**: Companions construct a heavy wooden redoubt at the western pressure door leading to Location 7.

#### Sound & Alert Tracker Impact
* Inspecting gears and loading bolts silently: **+0 Noise**.
* Firing the Heavy Torsion Ballista: **+3 Noise (+1 Alert Tier)**.
* Triggering the Grond-Ram into Location 6: **+4 Noise (+1 Alert Tier, +1 Eye Awareness)**.

#### Loot & Discoveries
* **Master Engineer's Chest**: 3 masterwork calipers, 2 flasks of *Dwarven Lubricant Oil*, *Iron Spanner* (Club: Damage 4, Injury 14, Break Shield), and 4 heavy ballista harpoon heads of star-iron.

---

### Location 6: The Hall of the Mauler

* **Role**: Apex Boss Arena & Dynamic Multi-Level Tactical Environment  
* **Elevation**: Lower Tier (Level 3C)  
* **Connections**:
  * *North*: Heavy iron-reinforced blast doors ascending to **Location 5 (The Second Armoury)**.
  * *South*: Grand pillared avenue descending into **Location 9 (The King's Door)**.
  * *East*: Collapsed slag chute leading down to unmapped furnace pits (hazardous terrain).
* **Baseline Threat**: Alert Tier 1–2. Home of **The Mauler** (Armoured Great Cave-troll, AL 10, Might 2, Hate 10, Armour 5d, Parry —).

```
========================================================================================================================
                                     LOCATION 6: THE HALL OF THE MAULER
========================================================================================================================
                   [From Location 5: The Second Armoury]
                                  │
                                  ▼
    ┌────────────────────────────────────────────────────────┐
    │  ═════════ [SUSPENDED IRON CATWALKS] ═════════ (20 ft) │
    │  (Narrow Walkways, Chain Ladders, Hanging Stalactites) │
    │                                                        │
    │  ┌──────────────────────────────────────────────────┐  │
    │  │             THE MAULER'S SCRAP NEST              │  │ <── The Mauler (AL 10)
    │  │  (Mountain of Scavenged Shields, Anvils, Bones) │  │     Sleeping / Alert
    │  └──────────────────────────────────────────────────┘  │
    │                                                        │
    │  [WEAPON HEAPS] ── Improvised Heavy Throwing Projectiles│
    │  [EAST SLAG CHUTE] ── Dangerous Chasm Edge             │
    └─────────────────────────────┬──────────────────────────┘
                                  │ (Grand Colonnade)
                                  ▼
                   [To Location 9: The King's Door]
========================================================================================================================
```

#### Boxed Read-Aloud Text
> *A colossal amphitheater for drill and weapon training opens before you, its vaulted ceiling lost in darkness forty feet above. Suspended by heavy iron chains, narrow catwalks cross the upper reaches like an iron web. Below, the floor is an apocalyptic graveyard of crushed wargear, shattered stone statues, and gnawed bone heaps. An overpowering musk of foul breath and stale lard hangs thick in the air, accompanied by a deep, rhythmic vibration that shakes the floorstones from the shadows ahead.*

#### GM Sensory Reference Bullets
* **Lighting**: Dim gloom. Patches of sickly pale-green phosphorescent fungus cling to high stalactites, casting long, eerie shadows across the scrap heaps below.
* **Drafts**: Heavy, rhythmic gusts of foul, scalding breath pulse from the center of the hall, like the exhaust draft of a blast furnace.
* **Echoes**: The troll’s guttural snores reverberate through the floor flagstones with a deep, bone-rattling vibration (*hrr-ungh, hrr-ungh*). Loose gravel quivers in rhythm.
* **Smells**: The suffocating musk of cave-troll, sour rotting marrow, scorched iron, dry rust, and rancid animal lard.

#### Interactive Environmental Features
1. **Suspended Iron Catwalks (20 Feet Elevation)**: Narrow grid-iron walkways suspended from ceiling chains. Accessible via three iron chain ladders.
   * *Tactical Benefit*: Characters on catwalks gain **High Ground (+1d to ranged attacks and combat tasks)** and are completely out of reach of the troll’s normal melee attacks (unless it throws scrap or climbs).
   * *Catwalk Hazards*: The catwalks are narrow; making an attack roll showing an Eye of Sauron ($\mathbf{S}$) requires an **ATHLETICS roll** to avoid losing footing.
2. **Hanging Limestone Stalactites**: Colossal, cracked stone spikes hanging directly above the troll’s nest.
   * *Triggering a Drop*: A character on a catwalk can use a crowbar or fire a heavy missile at a cracked ceiling anchor (**ATHLETICS roll or CRAFT roll**).
   * *Damage*: Drops a 2-ton stalactite onto the troll, dealing **20 Damage** (bypasses armour), knocking The Mauler **Prone**, and stripping 1d from its Armour rating!
3. **The Scavenged Iron Carapace**: The Mauler’s hide is wrapped in scrap iron (Armour 5d).
   * *Severing the Master Wires*: A character in Forward stance or on a catwalk can make a **Called Shot — HUNTING roll or CRAFT roll** to target the master copper wire bundle across its chest. Success cuts the wires, dropping its Armour rating permanently from **5d to 3d**!
4. **Scattered Weapon Heaps**: Piles of heavy dwarf-mattocks, 50-pound anvil blocks, and iron wagon axles. Can be hurled down from catwalks as improvised heavy missiles (Damage 8, Injury 18, Break Shield).

#### TOR 2e Skill Tests & Mechanics
* **The Catwalk Traverse — STEALTH roll**:
  * *Alternative*: Band **MANOEUVRE** (2d) against **Band TN 15**.
  * *Modifiers*: Moving across scrap floor is **Ill-favoured** (-1d); moving along high catwalks is standard. Having a scout lead the point grants **+1d**.
  * *Consequence of Failure*: A loose chain or iron ring clatters down into the nest (**+2 Noise Points**), waking The Mauler and initiating combat at Alert Tier 2!
  * *Degrees of Success (6 icons)*:
    * **6**: Company traverses the entire 40-pace hall in absolute silence (**+0 Noise**), reaching Location 9 undetected.
* **Combat Task: The Riddle Duel (Forward Stance) — RIDDLE roll**:
  * *Modifiers*: The Mauler’s *Dull-Witted* Fell Ability makes this test **Favoured**!
  * *Consequence of Failure*: The troll ignores the mockery and swings its massive maul directly at the taunting hero (**Heavy Blow**, +10 Damage).
  * *Degrees of Success (6 icons)*:
    * **6**: The Mauler bellows in confusion, swinging at shadows; it loses **1 point of Hate** plus **1 additional Hate per 6 icon rolled**!
    * **Gandalf Rune (G)**: The brute clutches its skull in terror, losing its entire turn; all Company attacks against it this round are Favoured.
* **Severing the Carapace Wires — HUNTING roll or CRAFT roll**:
  * *Position*: Forward Stance or from elevated Catwalk.
  * *Modifiers*: Called Shot rules apply (must roll at least one 6 icon).
  * *Consequence of Failure*: Blade bounces off iron plates; hero is exposed to counter-attack.
  * *Degrees of Success (6 icons)*:
    * **6**: Snips the master copper wire bundle, dropping The Mauler's scrap Armour rating permanently from **5d to 3d**!
* **Dropping the Limestone Stalactite — ATHLETICS roll or CRAFT roll**:
  * *Consequence of Failure*: Stalactite misses, smashing flagstones (**+3 Noise Points, +1 Alert Tier**).
  * *Degrees of Success (6 icons)*:
    * **6**: Direct hit: inflicts **20 Direct Damage** (bypassing Armour), knocks The Mauler **Prone**, and strips 1d from its Armour.
* **Plunging Strike — ATHLETICS roll**:
  * *Consequence of Failure*: Hero falls prone on the scrap floor, suffering 4 damage from the impact.
  * *Degrees of Success (6 icons)*:
    * **6**: Delivers a devastating plunging weapon strike (**Favoured attack roll; inflicts +4 Damage**).

#### Tactical Band Deployment & Companion Operations
* **Shield-Wall Phalanx**: Form a locked line at the northern doorway to bottle the troll if it charges, preventing it from sweeping past to crush archers.
* **Overwatch Sharpshooters**: Stationed on the high catwalks with bows to target the troll's unarmoured eyes and joints.

#### Sound & Alert Tracker Impact
* Sneaking across catwalks silently: **+0 Noise**.
* Engaging in melee combat with the troll (per round): **+2 Noise**.
* Dropping a 2-ton stalactite / Troll howling in rage: **+3 Noise (+1 Alert Tier, +1 Eye Awareness)**.
* Blowing the *Battle-horn of the Realm*: **+5 Noise (Instant Alert 3 / Drums in the Deep!)**.

#### Loot & Discoveries
* **The Troll's Scrap Nest**: Buried beneath the bone piles lies a crushed Dwarven chest containing 80 silver pennies, a golden torque bearing the crest of the House of Durin (worth 1 Treasure), and a pristine *Mithril-Steel War Dagger* (Damage 4, Injury 14, Keen, Load 1).

---

### Location 7: The Poisoned Halls (Twelfth & Fourteenth Halls)

* **Role**: Environmental Hazard & Clue Investigation  
* **Elevation**: Middle Tier / Depressed Basin (Level 3B-minus)  
* **Connections**:
  * *East*: Heavy sealed iron pressure-door leading to **Location 5 (The Second Armoury)**.
  * *South*: Bronze portal opening into **Location 8 (The Upper Armoury)**.
  * *Overhead*: Jammed iron ceiling ventilation flues leading to upper mine ducts.
* **Baseline Threat**: Alert Tier 0–1. Lethal Balrog Neurotoxic Miasma; calcified Dwarven knights.

```
========================================================================================================================
                                     LOCATION 7: THE POISONED HALLS
========================================================================================================================
                   [From Location 5: The Second Armoury]
                                  │
                                  ▼ (Sealed Pressure Door)
    ┌────────────────────────────────────────────────────────┐
    │  ≈ ≈ ≈ ≈ ≈ ≈ ≈ BALROG TOXIC MIASMA POOL ≈ ≈ ≈ ≈ ≈ ≈ ≈  │ (Emerald-Yellow
    │  (Rising 5 ft from floor; Grievous/Severe Exposure)    │  Neurotoxin)
    │                                                        │
    │  [PETRIFIED KNIGHTS IN STASIS]   [STONE COUNCIL TABLE] │
    │  (Dozens of Preserved Dwarves)   (Slumped Scribe Form) │
    │                                           │            │
    │  [JAMMED CEILING VENTILATION FLUE]        ▼            │
    │  (Overhead Iron Damper Lever)     [LEAD SCROLL TUBE:   │
    │                                    Scribe's Slate]     │
    └─────────────────────────────┬──────────────────────────┘
                                  │ (Bronze Pressure Door)
                                  ▼
                   [To Location 8: The Upper Armoury]
========================================================================================================================
```

#### Boxed Read-Aloud Text
> *A heavy, emerald-yellow vapor clings to the flagstones of this sunken council chamber, gathered chest-deep like a stagnant swamp of venom. Emerging from the mist near the entrance stand dozens of ancient dwarven warriors in full mail, frozen mid-stride like petrified statues where the volcanic breath of the Shadow overtook them. The air is oily, suffocatingly still, and pungent with the stinging stench of sulfur and burning copper.*

#### GM Sensory Reference Bullets
* **Lighting**: Deep gloom illuminated only by the sickly, iridescent emerald-yellow phosphorescence of the toxic vapor. Visibility through the mist is reduced to less than ten paces.
* **Drafts**: Oppressive, dead stillness. The air is oily, humid, and feverishly warm, hanging like a wet woolen blanket over the face.
* **Echoes**: The dense chemical fog heavily dampens all acoustics. Spoken words become muffled, wheezing gasps that die within five paces.
* **Smells**: Overpowering, stinging stench of bitter almonds, burning sulfur, metallic copper, and biting ozone that sears the nostrils.

#### Interactive Environmental Features
1. **The Balrog Neurotoxic Miasma**:
   * *Hazard Mechanics*:
     * **Unprotected Exposure** (raw breathing): Counts as **Grievous Poison**. Exposed characters must make a **PROTECTION test**, **Ill-favoured**, *every minute*. Failure inflicts 1d6 Endurance loss; rolling an Eye of Sauron ($\mathbf{S}$) reduces the character to 0 Endurance (inflicting a Wound; if already Wounded, the hero becomes **Dying**).
     * **Protected Exposure** (vinegar-soaked cloth, chewed mountain herbs via Healing check): Downgraded to **Severe Poison**. Roll **PROTECTION test** *every hour* (lose 1d3 Endurance on failure).
     * **Masterwork Squad Respirators**: Provides **4 hours of complete gas immunity** for up to 10 characters (crafted via Skill Endeavour below).
2. **The Preserved Scribe & Lead Scroll Tube**:
   * Slumped over the council table sits Scribe **Frár son of Frerin, Scribe of the Third Deep**. Prying the lead tube from his calcified grip requires a gentle hand (**ATHLETICS roll or CRAFT roll**).
   * *The Prop Clue (Handout #1 — The Dying Scribe's Slate)*: Inside lies a preserved slate written in Angerthas Moria runes detailing how the *Marshal of the Armies* was slain in the deeps, and how the **Marshal's Key** was carried away by goblin looters toward Goblin Village.
3. **The Jammed Overhead Ventilation Flue**:
   * A heavy iron damper lever is mounted on the wall ten feet up, connected to an overhead exhaust flue.
   * *Clearing the Flue*: Pulling the rusted lever requires an **ATHLETICS roll or CRAFT roll**.
   * *Effect*: Vents the entire hall clear of toxic gas in 2 combat rounds, but the shrieking, rusted iron gears generate **+3 Noise Points (+1 Alert Tier)**.

#### TOR 2e Skill Tests & Mechanics
* **Skill Endeavour: Assembling Squad Respirator Masks (Resistance 3)**:
  * *Context*: Constructing airtight masks using oiled leather scraps, charcoal granules, crushed sponge, and crystal goggles.
  * *Resistance*: **3**. Each test represents 10 minutes of disciplined labor.
  * *Allowed Skills*: **CRAFT roll**, **HEALING roll**.
  * *Modifiers*: Assisting companions with *Smith* grant **+1d**; utilizing workshop supplies from Location 5 grants **+1d**.
  * *Consequence of Failure*: Flawed filter seals (provides only *Protected* status rather than full immunity).
  * *Degrees of Success (6 icons)*:
    * **6**: Contributes 2 points of Resistance.
    * **66**: Completes masterwork respirators providing **4 hours of complete gas immunity** for up to 10 companions!
* **Unjamming the Overhead Ventilation Flue — ATHLETICS roll or CRAFT roll**:
  * *Modifiers*: **Ill-favoured** due to rusted iron corrosion. Invoking *Mighty* grants **+1d**.
  * *Consequence of Failure*: The rusted iron lever binds solid; hero suffers 2 Fatigue from physical exertion.
  * *Degrees of Success (6 icons)*:
    * **6**: Lever frees with a screech (**+3 Noise Points**); exhaust flues open, venting the entire hall clear of toxic gas in 2 combat rounds!
* **Prying the Lead Scroll Tube — ATHLETICS roll or CRAFT roll**:
  * *Consequence of Failure*: Calcified fingers snap; brittle lead tube is crushed, making runes partially illegible.
  * *Degrees of Success (6 icons)*:
    * **6**: Flawless extraction of Handout #1 (*The Dying Scribe's Slate*), revealing the full fate of the Marshal's Key.
* **Preparing Herbal Poultices & Vinegar Cloths — HEALING roll**:
  * *Consequence of Failure*: Spoiled mixture; provides no protection.
  * *Degrees of Success (6 icons)*:
    * **6**: Prepares field breathing poultices granting *Protected Exposure* status to the entire squad for 4 hours.
* **Deciphering the Dying Scribe's Slate — LORE roll or RIDDLE roll**:
  * *Consequence of Failure*: Inscription remains garbled.
  * *Degrees of Success (6 icons)*:
    * **6**: Translates the archaic Khuzdul runes, revealing that the *King's Door* locks can be bypassed if one understands the internal counterweight ciphers.
* **Scouting the Gas-Chamber Floor — SCAN roll**:
  * *Modifiers*: Investigating with *The Broken Key* rolls **Favoured**.
  * *Consequence of Failure*: Fails to spot boiling sulfur fissures beneath the mist.
  * *Degrees of Success (6 icons)*:
    * **6**: Identifies safe stepping-stones and spots the gold-and-garnet *Ring of Durin's Guard* (+1 to Valour tests, worth 2 Treasure) on the Captain's hand.

#### Tactical Band Deployment & Companion Operations
* **Rapid Hazardous Transit**: The guide leads the Band in tight breath-holding formation along the high stone bench ledges, keeping heads above the 5-foot vapor pool.
* **First Aid Overwatch**: A companion or hero keeping healing supplies ready administers emergency triage if a companion inhales a lungful of toxic vapor.

#### Sound & Alert Tracker Impact
* Moving silently through the heavy gas: **+0 Noise** (gas naturally dampens sound).
* Unjamming the rusted ceiling flue lever: **+3 Noise (+1 Alert Tier)**.
* Dropping an iron tool into a floor fissure: **+1 Noise**.

#### Loot & Discoveries
* **The Dying Scribe's Slate**: In-world physical prop (Handout #1) detailing the fate of the Marshal's Key.
* **Preserved Knightly Signet**: On the hand of the petrified Captain at the table rests a gold-and-garnet *Ring of Durin's Guard* (+1 to Valour tests, worth 2 Treasure).

---

### Location 8: The Upper Armoury (The Garrison Salvage Cache)

* **Role**: Garrison Salvage Cache & Goblin Grave  
* **Elevation**: Middle Tier (Level 3B)  
* **Connections**:
  * *North*: Heavy bronze door to **Location 7 (The Poisoned Halls)**.
  * *East*: Reinforced stone flue leading to **Location 9 (The King's Door)**.
  * *South*: Sealed emergency escape chimney ascending toward upper ventilation shafts.
* **Baseline Threat**: Alert Tier 0. Unlooted Dwarven armoury; desiccated goblin corpses.

```
========================================================================================================================
                                      LOCATION 8: THE UPPER ARMOURY
========================================================================================================================
                   [From Location 7: The Poisoned Halls]
                                  │
                                  ▼ (Heavy Bronze Door)
    ┌────────────────────────────────────────────────────────┐
    │  [DESICCATED GOBLIN LOOTERS] ── Dead at threshold      │
    │                                                        │
    │  ┌──────────────────────────────────────────────────┐  │
    │  │ GARRISON WARGEAR CACHES (40+ Suits of Mail,      │  │ <── +2 Band Readiness,
    │  │ 30 Tower-Shields, 50 War-Mattocks/Axes)          │  │     50 Treasure Tribute
    │  └──────────────────────────────────────────────────┘  │     for Balin's Colony!
    │                                                        │
    │  [LEAD-SEALED MUNITIONS CHEST] (6 Flasks Liquid Fire)  │
    │  [OFFICER'S LOCKER] (Masterwork Dwarven Wargear)       │
    └─────────────────────────────┬──────────────────────────┘
                                  │ (Reinforced Flue)
                                  ▼
                   [To Location 9: The King's Door]
========================================================================================================================
```

#### Boxed Read-Aloud Text
> *Sealed behind airtight bronze double doors, this massive vaulted storehouse has stood untouched by shadow for over a thousand years. The air is dry, crisp, and sterile cold. Beyond the threshold, row upon row of tall cedar lockers and stone storage chests stand in pristine order. Lantern light catches the mirror-gleam of dwarf-forged mail, polished tower-shields, and bins of razor-sharp war-mattocks—enough unmarred wargear to equip a vanguard.*

#### GM Sensory Reference Bullets
* **Lighting**: Pitch darkness until unsealed. Torchlight explodes into dazzling silver and gold reflections across mirror-polished dwarf-steel and bronze rims.
* **Drafts**: Absolutely zero draft. The air is crisp, completely dry, sterile, and cold.
* **Echoes**: Clean, sharp acoustics. The dry stone and oiled timbers cause metal clasps and footsteps to ring with high, musical clarity.
* **Smells**: Refined sweet machine oil, dry cedar shavings, polished iron, beeswax, and the papery, desiccated husk of dead goblins.

#### Interactive Environmental Features
1. **The Garrison Salvage Cache**:
   * *Contents*: 40 Suits of Dwarf Mail-shirts (Protection 3d, Load 9), 30 Heavy Tunnel-Shields (+2 Parry, Load 2), and 50 Masterwork War-Mattocks and Axes (Damage 6, Injury 18).
   * *Campaign Value*: Safely extracting these crates back to the East-Gate Camp equips Balin's frontline vanguard, permanently awards **+2 Band Readiness**, provides **50 Treasure Points** in colony tribute, and earns the royal favor of King Dáin Ironfoot!
2. **Lead-Sealed Munitions Chest**: A heavy iron coffer sealed with poured lead stamped with the Seal of the Royal Armouries.
   * *Cracking the Seal*: Requires a **CRAFT roll or STEALTH roll**, invoking *Burglary* Trait (+1d).
   * *Contents*: Contains **6 flasks of Dwarven Liquid Fire** (alchemical incendiary weapons). Rules: Thrown as a ranged attack using **ATHLETICS** (Short range 10y, Medium 20y [-1d], Long 30y [-2d]). On a hit, deals 8 Endurance damage. If a Piercing Blow is scored, or if the target fails a **PROTECTION test** (Injury TN 18), they suffer a Wound and are set ablaze, taking 3 Endurance damage per round at the start of their turn until they spend an action and succeed on an Athletics or Craft roll (TN 12) to extinguish the flames. It ignites flammable materials and illuminates a 30-foot radius for 1 minute (suppressing the *Denizen of the Dark* fell ability for Orcs).
3. **Dead Goblin Looter Corpses**: Searching the desiccated bodies (**SCAN roll**) reveals stolen loot pried from the outer halls.
   * *Found*: 18 tarnished silver coins, a brass whistle, and a stolen iron key that unlocks the Officer's Locker in this room.
4. **The Officer's Locker**: A reinforced iron cabinet containing officer-grade equipment.
   * *Contents*: 1 suit of *Reinforced Dwarf-mail* (Protection 4d, Load 12), 1 *Gleaming Broadsword* (Damage 5, Injury 16, Keen), and an engraved silver horn-mouthpiece.

#### TOR 2e Skill Tests & Mechanics
* **Skill Endeavour: Securing & Padding Heavy Salvage (Resistance 3)**:
  * *Context*: Organizing 50 suits of heavy gromril-mail and weapons into transport litters and wrapping iron plates in felt.
  * *Resistance*: **3**.
  * *Allowed Skills*: **EXPLORE roll**, **CRAFT roll**, **ATHLETICS roll**.
  * *Modifiers*: Assisting companions with *Smith* and *Mighty* traits each grant **+1d**.
  * *Consequence of Failure*: Crates clatter loudly (**+1 Noise Point** per failed roll); packing takes 45 minutes instead of 20.
  * *Degrees of Success (6 icons)*:
    * **6**: Contributes 2 points of Resistance.
    * **66**: Flawlessly packed and muffled (**+0 Noise**); prevents Band Manoeuvre penalties during withdrawal.
* **Cracking the Munitions Seal — CRAFT roll or STEALTH roll**:
  * *Modifiers*: Invoking the *Burglary* Trait grants **+1d**.
  * *Consequence of Failure*: Lead seal shears violently; 1 glass ampoule cracks, destroying 1 flask of Liquid Fire (**+1 Noise Point**).
  * *Degrees of Success (6 icons)*:
    * **6**: All 6 flasks of *Dwarven Liquid Fire* extracted intact with zero hazard.
* **Unlocking the Officer's Locker — CRAFT roll or STEALTH roll**:
  * *Modifiers*: Invoking the *Burglary* Trait grants **+1d**; using the recovered goblin iron key unlocks it automatically.
  * *Consequence of Failure*: Lock pin shears; cabinet must be forced with iron crowbars (**+1 Noise Point**).
  * *Degrees of Success (6 icons)*:
    * **6**: Silent opening; recovers Reinforced Dwarf-mail (Protection 4d) and Gleaming Broadsword (Damage 5, Keen).

#### Tactical Band Deployment & Companion Operations
* **Heavy Salvage Porter Squad**:
  * Deploying 4 companions as porters shifts the Band's status from **Medium Burden** to **Heavy Burden / Overburdened**.
  * *Operational Penalties*: Band Manoeuvre rolls suffer **-1d**; Marching Fatigue tests suffer **-1d**; moving through stone halls generates **+1 Noise Point per hall** unless muffled with felt and cloth.
* **Rearguard Sentry Watch**: Keep watch at the northern bronze door to ensure roaming patrols do not corner the squad while packing.

#### Sound & Alert Tracker Impact
* Carefully packing mail and padding crates with cloth: **+1 Noise**.
* Rushing the salvage and clattering iron shields across the floor: **+3 Noise (+1 Alert Tier)**.
* Opening the bronze pressure doors: **+0 Noise** (greased pivots).

#### Loot & Discoveries
* **The Garrison Arsenal**: 40 Suits of Mail, 30 Shields, 50 Mattocks/Axes (+2 Band Readiness, 50 Treasure Points tribute).
* **6 Flasks of Dwarven Liquid Fire**.
* **Officer's Gear**: Reinforced Dwarf-mail (Protection 4d), Gleaming Broadsword (Damage 5, Keen).

---

### Location 9: The King's Door

* **Role**: Runic Gate Puzzle & Ambush Choke Point  
* **Elevation**: Deepest Tier (Level 3C)  
* **Connections**:
  * *North*: Grand colonnade ascending to **Location 6 (The Hall of the Mauler)**.
  * *West*: Reinforced maintenance flue from **Location 8 (The Upper Armoury)**.
  * *South*: The King's Door threshold opening directly into **Location 10 (The Lower Armoury / Royal Vault)**.
  * *Overhead*: High defensive parapet and murder-hole balcony where Grimnar and his stalkers lurk.
* **Baseline Threat**: Alert Tier 2. Grimnar the Disgraced and 4 Stalkers positioned for ambush.

```
========================================================================================================================
                                        LOCATION 9: THE KING'S DOOR
========================================================================================================================
                   [From Location 6: Colonnade] / [Location 8: Flue]
                                  │
                                  ▼
    ┌────────────────────────────────────────────────────────┐
    │  ═════════ [OVERHEAD DEFENSIVE PARAPET] ═════════ (15ft)│ <── Grimnar & 4 Elite
    │  (Murder-Holes, Crossbow Slits, Poison Javelins)       │     Moria Orc Stalkers!
    │                                                        │
    │               ┌────────────────────────┐               │
    │               │  THE KING'S DOOR       │               │
    │               │  • Star-Iron & Granite │               │
    │               │  • Ithildin Runes      │               │
    │               │  • Dual Master Locks   │               │
    │               └───────────┬────────────┘               │
    └───────────────────────────┼────────────────────────────┘
                                │ (Unlocking the Vault)
                                ▼
                   [To Location 10: The Lower Armoury]
========================================================================================================================
```

#### Boxed Read-Aloud Text
> *At the terminus of the grand colonnade rises the legendary King's Door—the monumental threshold to Khazad-dûm's royal treasury. Hewn from massive blocks of star-iron and polished black granite, the great portal is inlaid with shimmering Ithildin runes depicting Durin's crown and anvil flanked by seven stars. A profound, sacred hush envelops the stone, and as you approach the threshold, the silver runes awaken with soft, watchful starlight that illuminates the quiet avenue.*

#### GM Sensory Reference Bullets
* **Lighting**: Soft silver-blue starlight radiating gently from the Ithildin runes. Deep shadows veil the high defensive parapets fifteen feet above.
* **Drafts**: Absolute stillness. A sacred, cool hush envelops the threshold, smelling of clean mountain air and ancient stone.
* **Echoes**: Deep harmonic resonance. Even a spoken whisper rings with the solemn clarity of a cathedral bell.
* **Smells**: Pure ozone, star-iron, ancient cedar resin, and the faint, bitter scent of Orc grease drifting down from the overhead parapet.

#### Interactive Environmental Features
1. **The King's Door & The Dual Runic Locks**:
   * *Ithildin Inscription*: Reads in ancient Sindarin/Khuzdul: *"None shall pass into the Vault of Kings save by the Mithril Ring and Durin's Will."*
   * *Unlocking Options*:
     1. **Dual Keys**: Inserting both the *Mithril Key* (recovered from Grimnar's Mordor patrol) and the *King's Key* (recovered from the Last Redoubt of the Dwarves in the Fifth Hall) and speaking the King's Invocation smoothly opens the door in **1 combat round** with zero noise!
     2. **The Masterwork Bypass**: If the keys are missing, the Company can attempt the **Skill Endeavour: Bypassing the Adamant Runic Lock (Resistance 6)** (*see below*).
2. **Grimnar's Overhead Ambush Parapet**:
   * A raised gallery fifteen feet above the door with narrow arrow embrasures.
   * *The Ambush Trigger*: **Grimnar the Disgraced** and four **Moria Orc Stalkers** wait until the Company gathers at the door to begin unlocking it, then unleash a coordinated volley of poisoned broad-headed javelins!

#### TOR 2e Skill Tests & Mechanics
* **Overhead Ambush Detection — AWARENESS roll**: Spot the glint of Grimnar’s stolen Dwarven dagger and poisoned javelins before stepping into the kill-zone.
  * *Alternative*: Forward Scout Screen rolls Band **VIGILANCE** (2d) against **Band TN 15**.
  * *Modifiers*: Invoking the *Wary* Trait grants **+1d**.
  * *Consequence of Failure*: Grimnar and his stalkers spring their ambush, gaining a **Surprise Round** (+1d to attacks; heroes cannot use Shield Parry).
  * *Degrees of Success (6 icons)*:
    * **6**: Spots the glint of poisoned javelins in the murder-holes before stepping into the kill-zone; Company avoids surprise and gains a free ranged volley.
* **Skill Endeavour: Bypassing the Adamant Runic Lock (Resistance 6)**:
  * *Context*: Picking the dual meteoric-iron and adamant tumblers if the *Mithril Key* and *King's Key*, or royal blood, are unavailable.
  * *Resistance*: **6**. Each attempt represents 1 combat round (or 5 minutes outside combat).
  * *Allowed Skills*: **CRAFT roll**, **STEALTH roll**, **RIDDLE roll**.
  * *Modifiers*: Invoking the *Burglary* Trait grants **+1d**; using *The Broken Key* rolls **Favoured**; assistance from a companion with *Vaultbreaker* grants **+1d**.
  * *Consequence of Failure*: Fine pick binds (**+1 Noise Point** per failed round); after 3 consecutive failures, a pick snaps, imposing **-1d** on subsequent attempts.
  * *Degrees of Success (6 icons)*:
    * **6**: Contributes 2 points of Resistance.
    * **66**: Contributes 3 points of Resistance!
  * *Outcome on Success*: The twin adamant wards click back in harmonic sequence; the King's Door swings open in total silence (**+0 Noise**).
* **The Blood of Durin Inscription Ritual — AWE roll or ENHEARTEN roll**:
  * *Context*: A Dwarf hero of Durin's royal lineage slicing their palm and pressing royal blood against the Crown rune.
  * *Modifiers*: Heir of Durin lineage grants **+1d** and **Favoured**.
  * *Consequence of Failure*: Hero loses 2 Endurance from the cut; the Crown rune fails to awaken; 1 Shadow Point (Dread) gained.
  * *Degrees of Success (6 icons)*:
    * **6**: The Ithildin Crown blazes with white starlight; the royal ward disengages instantly with a deep resonant tone.
* **Deciphering the Ithildin Invocation — LORE roll or RIDDLE roll**:
  * *Consequence of Failure*: Inscription remains dormant.
  * *Degrees of Success (6 icons)*:
    * **6**: Translates the activation glyphs, aligning the tumblers for the keys or bypass endeavour.
* **Duel Combat Task**: A hero in Forward stance challenges Grimnar in single combat (**AWE roll** vs Grimnar's Attribute Level 6), avenging the desecration of the Bridge and negating Grimnar’s Might bonus on the Band Clash roll!

#### Tactical Band Deployment & Companion Operations
* **Shield-Wall Bastion**: Form a protective testudo over the companion working the locks, granting them **Total Cover** from overhead missile fire.
* **Counter-Sniper Overwatch**: Companions with bows fire upward into the parapet murder-holes to suppress Grimnar's javelin-throwers.

#### Sound & Alert Tracker Impact
* Opening the door smoothly with both keys: **+0 Noise**.
* Picking the adamant lock with picks and wedges: **+1 Noise per round**.
* Resolving the skirmish with Grimnar's stalkers: **+2 Noise per round (+1 Alert Tier)**.

#### Loot & Discoveries
* **Grimnar's Gear (Upon Defeat)**: Grimnar's heavy scimitar (Damage 5, Injury 18, Break Shield) and the **Stolen Dwarven Dagger** (Damage 3, Injury 14, Keen, inlaid with gold filigree), and the **Mithril Key** (which unlocks the secret pillar door by the Dimrill Stairs).
* **Mordor Stalker Pouches**: 4 doses of Black Venom, 24 silver pennies, and an iron climbing claw.

---

### Location 10: The Lower Armoury (The Royal Vault)

* **Role**: Legendary Treasure Vault, Artifact Chamber & Ultimate Campaign Objective  
* **Elevation**: Deepest Sanctuary (Level 3C)  
* **Connections**:
  * *North*: The King's Door opening back onto **Location 9 (The King's Door)**.
* **Baseline Threat**: Sacred Sanctum. The spiritual weight of the Shadow; **Durin's Axe** claiming trigger (+4 Eye Awareness).

```
========================================================================================================================
                                     LOCATION 10: THE LOWER ARMOURY
========================================================================================================================
                   [From Location 9: The King's Door]
                                  │
                                  ▼
    ┌────────────────────────────────────────────────────────┐
    │  ══════════════ [VAULT BLAST BARRICADE] ══════════════ │ (Can be bolted
    │  (Inner Adamant Portcullis — Brief Rest Redoubt)       │  from inside)
    │                                                        │
    │               ┌────────────────────────┐               │
    │               │  THE DAIS OF DURIN     │               │
    │               │  • Black Marble Plinth │               │
    │               │  • DURIN'S AXE         │ <── Artifact!
    │               │  • Radiant Stasis Halo │     (+4 Eye)
    │               └────────────────────────┘               │
    │                                                        │
    │  [COFFER 1: Shield]   [COFFER 2: Mattock]   [COFFER 3: │
    │  (Deep Gate Shield)   (Moria-Silver)        Stone Mail)│
    │                                                        │
    │  [CHESTS OF MITHRIL INGOTS & FIRST AGE GROMRIL-MAIL]   │
    └────────────────────────────────────────────────────────┘
========================================================================================================================
```

#### Boxed Read-Aloud Text
> *The King's Door glides inward on silent, counterweighted pivots, revealing the innermost sanctum of Khazad-dûm's royal vault. The air is crisp, cool, and mountain-pure, untouched by shadow or rot. Slender ribs of white granite rise into the vault above, adorned with intricate filigree of mithril and electrum. In the center of the hall, resting upon an elevated dais of mirror-polished black marble, a radiant glow of golden and silver light pierces the gloom, illuminating ancient relic coffers arrayed in silent splendor.*

#### GM Sensory Reference Bullets
* **Lighting**: Radiant gold and silver luminescence radiating from *Durin’s Axe* and the polished mithril filigree. The chamber seems to glow with an inner, sacred light.
* **Drafts**: Completely still, crisp, and sweet. The air smells like fresh mountain snow, cedar incense, and cold spring water.
* **Echoes**: Deep, solemn acoustics. Footsteps land with a quiet, ringing reverence like footsteps in a sacred cathedral.
* **Smells**: Pure cedar oil, gold dust, ancient resin, and clean mountain wind.

#### Interactive Environmental Features
1. **The Dais of Durin & Durin's Axe**:
   * A hexagonal plinth of black marble inscribed with the names of the First Six Kings of Durin's Line. Resting upon it is **Durin’s Axe** (see full profile in Chapter 6).
   * *Claiming the Relic*: Lifting the axe from its plinth releases an immense harmonic chime of stone that echoes throughout the mountain.
   * *Strategic Eye Awareness Trigger*: Claiming Durin's Axe instantly adds **+4 to Strategic Eye Awareness** and alerts every Orc chieftain in the Third Deep!
2. **The Three Royal Relic Coffers**:
   * *Coffer 1 — Shield of the Deep Gate*: A massive Great Shield of star-iron and mithril (+3 Parry, Load 3, *Unyielding* trait: wearer cannot be knocked down or Seized).
   * *Coffer 2 — Mattock of Moria-Silver*: A two-handed heavy war-mattock forged of mithril-steel (Damage 8, Injury 18, *Gleaming Edge*: attacks in darkness are Favoured).
   * *Coffer 3 — Mail of Unyielding Stone*: A suit of interlocking gromril-mail (Protection 5d, Load 12, *Impenetrable*: spend 1 Hope to reduce Injury severity by one step).
3. **The Vault Blast Barricade**: The inner adamant portcullis can be locked from within using a heavy star-iron wheel-bolt.
   * *Safe Redoubt*: Allows the Company to take a secure **30-minute Brief Rest**, expend Fellowship Points, restore Hope, and organize their marching order before commencing the *Fighting Withdrawal*.

#### TOR 2e Skill Tests & Mechanics
* **Contemplating the Royal Relic — AWE roll or SONG roll**: Gaze upon the sacred relic without being overwhelmed by weeping reverence.
  * *Consequence of Failure*: Overwhelmed by sorrow and reverent weeping; the hero gains **1 Shadow Point (Dread)** and becomes **Miserable** until taking a Rest.
  * *Degrees of Success (6 icons)*:
    * **6**: Heart filled with ancestral majesty; clears all *Weary* conditions from all Player-Heroes and restores **+2 Hope Points**.
* **Resisting Greed & Ancient Splendour — SHADOW test**: Resist the overwhelming allure of the Greater Hoard.
  * *Modifiers*: A hero possessing the *Dragon-sickness* Flaw tests **Ill-favoured** (-1d).
  * *Consequence of Failure*: The hero gains **2 Shadow Points (Greed)** and is compelled to seize extraneous gold and goblets, shifting the Company to Heavy Burden.
  * *Degrees of Success (6 icons)*:
    * **6**: Wisdom prevails over avarice; the hero focuses solely on securing the royal relics and vital mithril ingots.
* **Disengaging the Runic Stasis Field — CRAFT roll or LORE roll**: Safely disengage the runic stasis field surrounding Durin's Axe before lifting it.
  * *Consequence of Failure*: The stasis field discharges with a thunderous electrical snap, inflicting 5 Damage to the lifter and generating **+2 Noise Points**.
  * *Degrees of Success (6 icons)*:
    * **6**: The stasis field dissipates in complete silence; Durin's Axe is lifted cleanly from the black marble anvil.
* **Vault Blast Barricade — CRAFT roll**:
  * *Outcome*: Bolting the inner adamant portcullis establishes a secure 30-minute Brief Rest redoubt before commencing the Fighting Withdrawal.

#### Tactical Band Deployment & Companion Operations
* **Ceremonial Claim**: A Dwarf hero lifts Durin's Axe while the Company marks the sacred moment, dedicating the reclamation to Lord Balin and King Dáin.
* **Packing the Royal Treasures**: The Band carefully packs the three Relic Coffers and bags of refined Mithril ingots into armored panniers.

#### Sound & Alert Tracker Impact
* Lifting Durin's Axe from the dais: **Special (+4 Strategic Eye Awareness!)**.
* Locking the inner vault blast door: **+0 Noise**.
* Commencing the Fighting Withdrawal: **Advances Alert Tracker to Alert Tier 3!**

#### Loot & Discoveries
* **Durin's Axe (Legendary Royal Artifact)**.
* **The Three Royal Relics**: *Shield of the Deep Gate*, *Mattock of Moria-Silver*, *Mail of Unyielding Stone*.
* **Chests of Refined Mithril Ingots**: 5 bars of pure Moria-silver (worth 50 Treasure Points in royal tribute for King Dáin Ironfoot).

---

## 4. Location Connection & Elevation Summary Matrix

```
========================================================================================================================
                                     CERTIFIED TOR 2E LOCATION SUMMARY MATRIX
========================================================================================================================
 LOC  NAME                TIER     LIGHTING   PRIMARY HAZARD / OBSTACLE        KEY SKILL CHECKS (ATTRIBUTE) NOISE / ALERT IMPACT
------------------------------------------------------------------------------------------------------------------------
  1   Mustering-Yard      Level 3A Pitch/Moss Udûn Sentries / Open Ground      Stealth (WIT), Scan (WIT)    +0 (Sneak) / +1 (1-rd kill)
  2   Upper Gatehouse     Level 3A Shadow     Buckled Doors / Fallback Point   Craft (STR), Battle (HRT)    +1 (Fortify) / +4 (Cave-In)
  3   First Armoury       Level 3B Pitch      Scythe Scrap-Traps / Venom       Scan (WIT), Craft (STR)      +0 (Disarm) / +2 (Spring)
  4   Broken Hall         Level 3B Darkness   Balrog Idol Dread / Desecration  Awe (STR), Lore (WIT)        +0 (Lurk) / +3 (Topple)
  5   Second Armoury      Level 3B Gloom      Siege Engines / Heavy Machinery  Craft (STR), Athletics (STR) +0 (Prep) / +4 (Ram Fire)
  6   Hall of the Mauler  Level 3C Dim Moss   The Mauler (AL 10 Troll)         Riddle (WIT), Stealth (WIT)  +0 (Sneak) / +3 (Stalactite)
  7   Poisoned Halls      Level 3B Emerald    Balrog Neurotoxic Miasma         Protection (STR), Craft (STR)+0 (Silent) / +3 (Vent Flue)
  8   Upper Armoury       Level 3B Dark/Steel Garrison Salvage / Burden Shift  Explore (WIT), Craft (STR)   +1 (Pack) / +3 (Clatter)
  9   The King's Door     Level 3C Ithildin   Dual Locks / Grimnar Ambush      Awareness (STR), Craft (STR) +0 (Key) / +2 (Skirmish)
 10   Lower Armoury       Level 3C Radiant    Durin's Axe / Greater Hoard      Awe (STR), Shadow (HRT)      Special (+4 Eye / Alert 3)
========================================================================================================================
```



<!-- PAGE BREAK: Chapter 5 -->

---


# Chapter 5: Adversaries, Foes & Subterranean Hazards

> *"There are older and fouler things than Orcs in the deep places of the world... but even the common vermin of the Deeps fight with the desperate fury of cornered beasts when the shadows lengthen."*  
> — *Ancient Lore of the Longbeards*

---

## 1. Subterranean Combat & Adversary System

In *The One Ring 2nd Edition* (*Moria: Through the Doors of Durin*), combat within the claustrophobic vaults of Khazad-dûm is rapid, brutal, and lethal. Adversaries do not track the three Player-Hero attributes (Strength, Heart, Wits); instead, each foe is defined by a single unified **Attribute Level**, fueling their combat target numbers, damage output, defensive thresholds, and fell powers.

```
========================================================================================
                          TOR 2e ADVERSARY STATISTICAL SCHEMA
========================================================================================
 ATTRIBUTE LEVEL (AL) : Core rating (1–12+). Sets default TNs and Heavy Blow bonuses.
 ENDURANCE (End)      : Physical stamina and structural hit points.
 MIGHT (M)            : Actions/attacks per round (1, 2, or 3) & Wounds required to kill.
 HATE / RESOLVE       : Fuel rating for bonus dice (+1d) and Fell Ability activation.
 PARRY                : Modifier added to the Player-Hero's Strength TN (+0 to +3, or —).
 ARMOUR               : Protection test dice rolled when struck by a Piercing Blow (1d–5d).
 PROFICIENCIES        : Attack Name, Rank (dice rolled), Damage, Injury, Special Damage.
 FELL ABILITIES       : Innate monstrous traits, tactical reactions, and aura effects.
========================================================================================
```

---

### 1.1 Core Adversary Combat Rules

* **Target Numbers for Attacks**:
  * **Player-Hero attacking an Adversary**: The hero rolls against $\mathbf{\text{Strength TN} + \text{Adversary Parry modifier}}$.
  * **Adversary attacking a Player-Hero**: The adversary rolls **1 Feat Die + Success Dice equal to Combat Proficiency** against $\mathbf{\text{Hero's Target TN} + \text{Hero Shield/Parry modifier}}$.
* **The Feat Die for Servants of Shadow**:
  * The **Eye of Sauron ($\mathbf{S}$)** represents the ultimate result: it counts as **10** (and triggers a Piercing Blow).
  * The **Gandalf Rune ($\mathbf{G}$)** counts as **0** (a total miss/fumble for dark creatures).
* **Piercing Blows**:
  * An adversary scores a Piercing Blow whenever the Feat Die shows a **10** or the **Eye of Sauron ($\mathbf{S}$)**.
  * The struck Player-Hero must immediately make a **Protection Test** (rolling their Armour dice + Helm) against the weapon's **Injury rating**. On a failure, the hero suffers a **Wound**.
* **Hate & Resolve Expenditure**:
  * Minions of the Shadow burn **Hate**; non-corrupted beings utilize **Resolve**.
  * An adversary may spend **1 point of Hate** to add **+1d** to an attack roll, **+1d** to a Protection test, or to trigger an active **Fell Ability**.
  * The maximum Hate spent in a single combat round cannot exceed the adversary's **Might rating**.
  * When an adversary is reduced to **0 Hate**, it becomes **Weary** and cannot spend Hate until rested.
* **Special Damage Triggers**: Each Success icon ($\mathbf{6}$) scored on an adversary's successful attack roll can trigger one Special Damage option:
  * **Heavy Blow**: The adversary expends 1 Success icon to inflict additional Endurance loss equal to its **Attribute Level ($+L$)**.
  * **Break Shield**: Smashes the target's non-magical shield, destroying it permanently (hero loses shield Parry bonus).
  * **Pierce**: Adds **+2** to the Feat Die result for the purpose of beating the hero's Injury TN.
  * **Seize**: The hero is pinned into **Forward Stance**, unable to change stances and restricted to Brawling attacks until spending a Success icon ($\mathbf{6}$) on an attack roll to break free.
  * **Fiery Blow**: Inflicts immediate **Severe or Grievous fire damage** (burning gear, igniting cloaks, and rupturing respirators).

---

### 1.2 Morale, Rout & Break Points

Orcs and goblins are cowardly by nature, driven only by fear of their chieftains and hatred of the Free Peoples. A skirmish rarely fights to the last minion.

```
========================================================================================
                                 MORALE BREAK POINT MATRIX
========================================================================================
 TRIGGER EVENT                    MORALE CHECK REQUIRED           ROUT CONSEQUENCE
----------------------------------------------------------------------------------------
 Chieftain / Champion Slain       Roll Valour vs Attribute Level  Warband breaks immediately; 
 (e.g. Grimnar or Black Uruk)     (Ill-favoured for Goblins)      minions flee into side vents.
----------------------------------------------------------------------------------------
 Casualties Exceed 50%            Roll Valour vs Attribute Level  Surviving Orcs enter Craven 
 (Half of warband defeated)       (Craven foes fail instantly)    state; withdraw to barricades.
----------------------------------------------------------------------------------------
 The Mauler Defeated / Pacified   No roll needed                  All accompanying goblins scatter 
 (Troll collapses or befriends)   Automatic Goblin Panic          screaming in blind terror.
----------------------------------------------------------------------------------------
 Powerful Light / Flame Unleashed Roll Valour vs Attribute Level  Orcs suffer -1d to all attacks 
 (Durin's Axe Flame of Hope)      (Hate Sunlight applies)         and attempt to flee darkward.
========================================================================================
```

---

## 2. Apex Adversary: The Mauler (Armoured Great Cave-Troll)

> *"A mountain of scarred grey flesh encased in twisted plates of scavenged iron, rusted boiler plates, and crushed Dwarven shields. When it swings its tree-trunk maul, the stone arches tremble."*

```
========================================================================================
                     THE MAULER (ARMOUR-CLAD GREAT CAVE-TROLL)
========================================================================================
 "Driven mad by the memory of steel biting flesh, the beast has forged itself into an iron tomb."
 Culture: Great Cave-troll of Moria | Distinctive Features: Brutish, Lumbering, Terrified of Pain
----------------------------------------------------------------------------------------
 ATTRIBUTE LEVEL: 10
 ENDURANCE:       80 (Weary at 0 Hate)
 MIGHT:           2 (Takes 2 Wounds to kill; 2 attacks per combat round)
 HATE:            10
 PARRY:           — (0 / Unarmoured baseline 0; scrap plating modeled by Armour 5d)
 ARMOUR:          5d (Twisted scrap-iron, boiler shields, anvil fragments)
----------------------------------------------------------------------------------------
 COMBAT PROFICIENCIES:
 • Heavy Club / Maul:  3d  (Damage 8, Injury 16, Break Shield, Heavy Blow)
 • Seize / Slam:       3d  (Damage 4 / 6, Injury 12, Seize)
 • Scrap Shrapnel:     2d  (Damage 6, Injury 12, Ranged missile, Area burst)
----------------------------------------------------------------------------------------
 FELL ABILITIES:
 • Dull-Witted: Player-heroes in Forward stance can use their main combat action to 
   attempt a RIDDLE roll (Favoured), benefiting from Dull-Witted. On a success, The Mauler 
   loses 1 point of Hate, plus 1 additional point of Hate per Success icon (6) rolled 
   (the troll bellows and attacks shadows/echoes in confusion). A Gandalf rune (G) causes 
   the troll to lose its full turn in confusion. 3 cumulative successes pacify or bypass 
   the creature.
 • Hideous Toughness: Unarmed blows cannot harm the troll. When an attack reduces the 
   Mauler to 0 Endurance, it causes a Piercing Blow instead. If it survives the Protection 
   roll, its Endurance immediately resets to 40 (half maximum).
 • Strike Fear: Spend 1 Hate at combat start. All Player-heroes within sight must make a 
   VALOUR test or gain 2 Shadow Points (Dread); heroes whose current Shadow equals or 
   exceeds their Hope become Miserable.
 • Thick Hide: Spend 1 Hate on a Protection roll to gain +2d Armour dice (rolling 7d total).
 • Scavenged Iron Carapace: If a Piercing Blow fails to inflict a Wound, the attacker's 
   weapon becomes lodged in the plating unless the hero succeeds on a CRAFT roll or 
   ATHLETICS roll. A direct hit from a heavy siege engine strips this plating, reducing 
   Armour from 5d to 3d.
========================================================================================
```

```
                        [THE MAULER'S LAIR: COMBAT TOPOLOGY]
                                (Location 6 Drill Hall)

               ┌────────────────── [HIGH CATWALKS] ──────────────────┐
               │  (15–20 ft elevation; +1d ranged; sweepable by maul)│
               └───────────┬─────────────────────────────┬───────────┘
                           │                             │
                           ▼                             ▼
                  [FALLING STALACTITES]         [WEAPON SCRAP HEAPS]
                  (Lever down: 20 Dmg)          (Hurled for 6 Dmg AoE)
                           │                             │
                           └──────────────┬──────────────┘
                                          ▼
                               [THE MAULER'S ARENA]
                                • 80 Endurance (Might 2)
                                • Scrap-Iron Plating (5d)
                                • Riddle Duel Choke Point
```

---

### 2.1 The Riddle Duel Combat Task

The Mauler is not merely a brute; it is a creature broken by cognitive trauma from the Battle of Azanulbizar. Driven by an overwhelming terror of physical pain, the troll's primitive mind struggles between ferocious rage and paralyzing fear.

```
========================================================================================
                          THE MAULER RIDDLE DUEL MECHANICS
========================================================================================
 • STANCE REQUIREMENT : The negotiating hero must stand in FORWARD STANCE, facing the 
                        troll within arm's reach.
 • ACTION COST        : Executing the Riddle duel consumes the hero's main combat action.
 • TARGET NUMBER      : RIDDLE roll (Favoured), benefiting from The Mauler's 
                        Dull-Witted trait.
 • RESOLUTION TRACK   : 3 Successes required before 3 Failures.
----------------------------------------------------------------------------------------
 SUCCESSES / OUTCOMES :
 • 1st Success : The Mauler halts its swing mid-air, blinking in confusion. It loses 
                 1 Hate (+1 additional Hate per Success icon 6 rolled).
 • 2nd Success : The troll drops its club to clasp its iron-plated ears, weeping in 
                 frustration. It loses 2 Hate and forfeits its next attack round.
 • 3rd Success : Complete cognitive breakdown! The troll cowers against the north wall, 
                 whimpering and covering its head. It is PACIFIED and will not attack 
                 unless struck with weapons.
 • Gandalf (G) : The troll strikes wildly at empty echoes, losing its entire turn in 
                 addition to standard Hate loss.
----------------------------------------------------------------------------------------
 FAILURES / RETALIATION :
 • 1st Failure : The Mauler roars in anger; gains +1d on its next Maul attack.
 • 2nd Failure : The troll sweeps its club across the floor, generating +2 Noise Points.
 • 3rd Failure : The duel collapses! The Mauler enters a blind berserk frenzy, attacking 
                 with +2 Damage on all strikes for the remainder of the encounter.
========================================================================================
```

---

### 2.2 Dynamic Arena Tactics (Location 6)

The Hall of the Mauler is an interactive combat playground featuring four major tactical mechanics:

1. **Catwalk Sweeping (15–20 ft Elevation)**:
   * Heroes scaling the ancient iron inspection ladders can reach the perimeter catwalks, gaining **+1d on ranged attacks** and immunity to ground-level slam attacks.
   * *The Mauler's Reaction*: The troll can use one attack action to sweep its 12-foot tree-trunk club through the iron catwalk struts. All characters on that section must make an **ATHLETICS roll**:
     * *Success*: The hero leaps to an adjacent pillar or ceiling chain, holding fast.
     * *Failure*: The catwalk buckles! The character falls 20 feet to the stone flags below, suffering **10 Falling Damage** and landing Prone.
2. **Stalactite Dropping (20 Direct Damage)**:
   * Colossal mineral stalactites hang from the cracked ceiling vaults directly above the troll's scrap-nest.
   * A hero on the high catwalks can use their action to lever loose a cracked stalactite with a crowbar, spear, or war-mattock (**ATHLETICS roll** or **CRAFT roll**).
   * *Impact*: The falling stone spear crashes into the Mauler, inflicting **20 Direct Damage** (bypassing Armour) and knocking the beast Prone.
3. **Weapon Pile Hurling (Improvised Area Missiles)**:
   * The Mauler frequently digs both hands into heaps of rusted broadswords, iron wedges, and bucklers, hurling a wave of shrapnel at heroes in Open or Rearward stance.
   * *Resolution*: Ranged attack roll (2d) against all heroes in the target arc. Targets hit suffer **6 Damage (Injury 12)** and must pass a **Protection test** (rolling Armour dice + Helm vs Injury 12) or become Weary from shrapnel lacerations.
4. **Siege Engine Integration (Stripping the Carapace)**:
   * If the party managed to aim and prime the heavy Dwarven Torsion Ballista from **Location 5 (The Second Armoury)** into Location 6, firing the harpoon bolt deals **25 Direct Damage** and **permanently strips the troll's scrap-iron plating**, reducing its Armour rating from **5d to 3d**.

---

## 3. Archfoe: Grimnar the Disgraced (Great Orc Chieftain / Stalker)

> *"A towering, whipcord-muscled Great Orc chieftain whose leather harness is adorned with blackened Dwarven beards. He twitches with manic speed, brandishing a notched heavy scimitar in one claw and a gleaming, stolen Dwarven dirk in the other."*

```
========================================================================================
                     GRIMNAR THE DISGRACED (GREAT ORC STALKER)
========================================================================================
 "Humiliated at Durin's Bridge, he will buy his redemption with the blood of the Longbeards."
 Culture: Great Orc of the Misty Mountains | Distinctive Features: Fierce, Vengeful, Cunning
----------------------------------------------------------------------------------------
 ATTRIBUTE LEVEL: 6
 ENDURANCE:       36 (Weary at 0 Hate)
 MIGHT:           2 (Takes 2 Wounds to kill; 2 attacks per combat round)
 HATE:            6
 PARRY:           +2 (+3 when dual-wielding stolen Dwarven dagger)
 ARMOUR:          3d (Scavenged heavy dwarf-mail reinforced with boiled leather)
----------------------------------------------------------------------------------------
 COMBAT PROFICIENCIES:
 • Heavy Scimitar:         3d  (Damage 5, Injury 16, Pierce, Break Shield)
 • Stolen Dwarven Dagger:  3d  (Damage 4, Injury 14, Keen [Pierce on 9–10 or Eye (S)])
 • Broad-headed Spear:     2d  (Damage 5, Injury 16, Pierce, Throwable)
----------------------------------------------------------------------------------------
 FELL ABILITIES:
 • Denizen of the Dark: Attack rolls are Favoured when fighting in subterranean darkness.
 • Craven Ambush: When attacking from darkness or surprise (Alert Tier 2+), his first 
   attack inflicts an automatic Piercing Blow.
 • Fierce Command: Spend 1 Hate to allow 2 nearby Orc Soldiers to make an immediate bonus 
   attack outside their normal turn.
 • Great Leap: Spend 1 Hate to leap over frontline defenders (bypassing the Shield-Wall) 
   to land directly before Rearward heroes or archers.
 • Hate Sunlight: Loses 1 Hate per round if exposed to full direct sunlight.
 • Hatred (Durin's Folk): Attack rolls against Dwarves of Durin's Folk and Dwarf Companions 
   are permanently Favoured (+1d on attack rolls).
 • Hideous Toughness: Unarmed attacks cannot harm him. When an attack reduces Grimnar 
   to 0 Endurance, it causes a Piercing Blow; if he survives the Protection roll, his 
   Endurance immediately resets to 18 (half maximum).
 • Snake-like Speed: Spend 1 Hate when targeted by an incoming melee or ranged attack 
   to make the attacker's roll Ill-favoured.
 • Vengeful Strike: If struck in melee by a Player-Hero, Grimnar may spend 
   1 Hate as an immediate reaction to deliver a free retaliation strike against that hero.
 • Gleaming Dagger: The stolen dagger glints with captured starlight (Famous Dagger 
   of Durin); if Grimnar is slain, the heroes can recover it.
========================================================================================
```

---

### 3.1 Ambush Tactics at Keyed Location 9 (The King's Door)

Grimnar is a cunning hunter who refuses to fight on fair terms. He utilizes the architecture of the Third Deep to stage a multi-phase ambush:

```
========================================================================================
                          GRIMNAR'S AMBUSH DOCTRINE (ROOM 9)
========================================================================================
 PHASE 1: THE CROSSFIRE (Catwalk Overwatch)
 • Position: Grimnar and 2 Udûn Sniffers man the high stone lintels 25 feet above the door.
 • Action: Opens with poisoned broad-headed spear throws targeting heroes working the door 
   mechanisms or signal horns to prevent them from opening the adamant portal.

 PHASE 2: THE GREAT LEAP (Bypassing the Phalanx)
 • Trigger: When the frontline or the Shield-Wall advances to engage his front ranks.
 • Action: Grimnar activates Great Leap, bounding over the locked shields to strike at 
   heroes working the lock mechanisms or in rear ranks.

 PHASE 3: DUAL-WIELDING FRENZY & RETALIATION
 • Stance: Fights in Forward Stance with Heavy Scimitar and Stolen Dwarven Dagger.
 • Tactic: Uses Snake-like Speed to avoid heavy axe blows while triggering Vengeful Strike 
   whenever struck, turning every clash into a bloodbath.
========================================================================================
```

---

### 3.2 Grudge Narrative Dialogue & Combat Taunts

During combat, Grimnar taunts the Player-Heroes with venomous callbacks to prior campaign milestones:

* **Against the Champion**: *"Where is your forge now, hammer-dwarf? Your brothers screamed as their beards burned at the bridge, and your skin will make fine straps for my scabbard!"*
* **Against the Scout**: *"I recognize that broken bit of iron in your hand! Your cousin died clutching it in the slave-pits, and it won't save you from my knife!"*
* **Against the Commander**: *"Blow your brass horn, little herald! Blow it until your lungs burst! Malech's drummers are answering, and your tomb is already sealed!"*
* **On Brandishing the Stolen Dagger**: *"Look at the craft of it! Dwarven steel cuts Dwarven mail sweeter than butter. Shall I carve your name beside your brother's on the hilt?"*

---

### 3.3 Tactical Retreat & Reinforcement Triggers

Grimnar is vengeful but not suicidal. He knows when to break contact:

```
========================================================================================
                           GRIMNAR'S RETREAT PROTOCOL
========================================================================================
 • RETREAT TRIGGER 1 : If Grimnar is reduced to <= 8 Endurance and has 0 Hate remaining.
 • RETREAT TRIGGER 2 : If both accompanying Udûn Sniffers are slain and a hero lands a 
                       Piercing Blow that penetrates his Armour.
 • ESCAPE ROUTE      : Grimnar hurls a smoke-bomb of crushed brimstone into the doorway 
                       and slips through a narrow drainage flue leading toward Location 1.
 • PURSUIT CHECK     : To prevent his escape, a hero must succeed on an ATHLETICS roll 
                       or make a successful ranged attack before he reaches the flue.
========================================================================================
```

---

## 4. Garrison Ranks: Orc Patrols & Sentry Squads

The garrison of the Third Deep consists of standard **Moria Orcs** (cave scavengers, soldiers, guards, drummers, and sniffers) and a small troop of elite **Mordor Orcs** (Grimnar's cutthroats). Udûn Orcs serving Malech's banner do not guard these armouries; they may occasionally come to trade at most.

```
========================================================================================
                          GARRISON ADVERSARY QUICK MATRIX
========================================================================================
 ADVERSARY TYPE      AL  END  MIGHT HATE PARRY ARMOUR  MAIN ATTACK       DAMAGE / INJURY
----------------------------------------------------------------------------------------
 Orc Soldier         3   12     1     3   +1    2d     Orc-axe 2d        Dmg 3, Inj 18 (Break)
 Orc Guard           4   16     1     4   +2    3d     Heavy Scimitar 3d Dmg 4, Inj 16 (Pierce)
 Moria Sniffer       4   16     1     4    —    3d     Iron-shod Staff 3d Dmg 4, Inj 14 (Heavy)
 Moria Orc Drummer   3   12     1     3   +1    2d     Curved Knife 2d   Dmg 3, Inj 14 (Pierce)
 Black Uruk          5   20     1     5   +2    3d     Broadsword 3d     Dmg 4, Inj 16 (Pierce)
 Black Uruk Captain  6   24     2     6   +3    4d     Great Scimitar 3d Dmg 5, Inj 16 (Pierce)
========================================================================================
```

---

### 4.1 Detailed Sentry & Patrol Stat Blocks

#### 1. Moria Orc Soldier (Attribute Level 3)
* **Culture**: Moria Goblin / Scout | **Distinctive Features**: Sullen, Sneaking
* **Endurance**: 12 | **Might**: 1 | **Hate**: 3 | **Parry**: +1 | **Armour**: 2d (Crude leather & iron scrap)
* **Combat Proficiencies**:
  * *Orc-axe*: 2d (Damage 3, Injury 18, Break Shield)
  * *Short Bow*: 2d (Damage 3, Injury 14, Pierce)
* **Fell Abilities**:
  * *Denizen of the Dark*: Attack rolls are Favoured in darkness.
  * *Hate Sunlight*: Loses 1 Hate per round in sunlight.
  * *Craven*: If reduced below half Endurance (6 End) or if their Chieftain falls, must make a Valour test (vs Attribute Level 3) or flee.

#### 2. Moria Orc Guard (Attribute Level 4)
* **Culture**: Moria Orc / Heavy Sentry | **Distinctive Features**: Brutish, Disciplined
* **Endurance**: 16 | **Might**: 1 | **Hate**: 4 | **Parry**: +2 | **Armour**: 3d (Scavenged dwarf-mail)
* **Combat Proficiencies**:
  * *Heavy Scimitar*: 3d (Damage 4, Injury 16, Pierce)
  * *Heavy Spear*: 3d (Damage 4, Injury 14, Pierce)
* **Fell Abilities**:
  * *Denizen of the Dark*: Attack rolls are Favoured in darkness.
  * *Hate Sunlight*: Loses 1 Hate per round in sunlight.
  * *Thick Armour*: Spend 1 Hate on a Protection test to roll +1d Armour.
  * *Shield-Wall*: When fighting in pairs or adjacent to an ally, gains +1 Parry.

#### 3. Moria Sniffer (Attribute Level 4)
* **Culture**: Moria Goblin / Hunter | **Distinctive Features**: Sullen, Alert, Keen Scent
* **Endurance**: 16 | **Might**: 1 | **Hate**: 4 | **Parry**: — | **Armour**: 3d (Soot-caked hides & ash)
* **Combat Proficiencies**:
  * *Iron-shod Staff*: 3d (Damage 4, Injury 14, Heavy Blow)
  * *Poisoned Blowdart / Bow*: 2d (Damage 2, Injury 12, Poison [Black Venom])
* **Fell Abilities**:
  * *Denizen of the Dark*: Attack rolls are Favoured in darkness.
  * *Heartless*: Immune to standard *Intimidate Foe* actions unless triggered by a Gandalf result.
  * *Keen Scent*: Gains +2d on Awareness rolls to detect hidden intruders by scent.
  * *Hate Sunlight*: Loses 1 Hate per round in direct sunlight.

#### 4. Moria Orc Drummer (Attribute Level 3)
* **Culture**: Moria Orc / Signal Corps | **Distinctive Features**: Deformed, Loud, Resonant
* **Endurance**: 12 | **Might**: 1 | **Hate**: 3 | **Parry**: +1 | **Armour**: 2d (Hide vest)
* **Combat Proficiencies**:
  * *Curved Knife*: 2d (Damage 3, Injury 14, Pierce)
  * *Bone Drum-Beater*: 2d (Damage 3, Injury 12, Heavy Blow)
* **Fell Abilities**:
  * *Denizen of the Dark*: Attack rolls are Favoured in darkness.
  * *Hate Sunlight*: Loses 1 Hate per round in sunlight.
  * *Drums in the Deep*: Instead of making an attack action, the Drummer may spend **1 Hate** to beat its massive kettle-drum, instantly raising the Company's **Strategic Eye Awareness by +3** and adding **+2 Alert Points**!

#### 5. Black Uruk of Mordor (Attribute Level 5 — Elite Reinforcement)
* **Culture**: Uruk of Mordor / Shock-Troop | **Distinctive Features**: Fierce, Strong, Relentless
* **Endurance**: 20 | **Might**: 1 | **Hate**: 5 | **Parry**: +2 | **Armour**: 3d (Black iron plate)
* **Combat Proficiencies**:
  * *Broadsword*: 3d (Damage 4, Injury 16, Pierce)
  * *Bow of Horn*: 3d (Damage 3, Injury 14, Pierce)
* **Fell Abilities**:
  * *Horrible Strength*: Spend 1 Hate when scoring a Piercing Blow to make the target hero's Protection roll **Ill-favoured**.
  * *Thick Armour*: Spend 1 Hate on a Protection roll to gain +2d Armour.

#### 6. Black Uruk Captain (Attribute Level 6 — Vanguard Commander)
* **Culture**: Uruk of Mordor / War-Leader | **Distinctive Features**: Cruel, Commanding
* **Endurance**: 24 | **Might**: 2 | **Hate**: 6 | **Parry**: +3 | **Armour**: 4d (Full black plate)
* **Combat Proficiencies**:
  * *Great Scimitar*: 3d (Damage 5, Injury 16, Pierce, Break Shield)
  * *Iron Javelin*: 2d (Damage 5, Injury 14, Pierce)
* **Fell Abilities**:
  * *Horrible Strength*: Spend 1 Hate on a Piercing Blow to make the hero's Protection roll Ill-favoured.
  * *Yell of Triumph*: Spend 1 Hate when an ally or hero falls to immediately restore **1 Hate** to all Orcs within earshot.

---

### 4.2 Weapon Poison Mechanics (Black Orc-Venom)

Many sentry weapons (darts, arrows, and scavenged dagger blades in Locations 1, 3, and 9) are coated in thick, distilled mountain-spider venom mixed with corpse-grease.

```
========================================================================================
                              BLACK ORC-VENOM RULES
========================================================================================
 • TRIGGER           : When an adversary lands a Piercing Blow using a poisoned weapon, 
                       or spends a Success icon (6) to inject venom.
 • INITIAL EFFECT    : Struck hero must make an immediate HEALING roll or PROTECTION test:
   - Success : The venom is resisted; target suffers standard weapon damage only.
   - Failure : The target contracts MODERATE POISON (loses 4 Endurance immediately 
               and gains the Weary condition until treated).
 • SEVERE INFECTION  : If the Feat Die shows the Eye of Sauron (S), the poison is SEVERE 
                       (loses 8 Endurance, becomes Weary, and requires First Aid to purge).
 • TREATMENT         : First Aid treatment requires a HEALING roll during a rest. 
                       Healing rolls are Ill-favoured if Severe.
========================================================================================
```

---

## 5. Subterranean Environmental Hazards

The physical architecture of the Third Deep is an ancient, crumbling ruin beset by geothermal vents, demonic corruption, and centuries of structural fatigue.

```
                  ┌──────────────────────────────────────────────┐
                  │    ENVIRONMENTAL HAZARDS OF THE THIRD DEEP   │
                  └──────────────────────┬───────────────────────┘
                                         │
         ┌───────────────────────────────┼───────────────────────────────┐
         ▼                               ▼                               ▼
 [BALROG TOXIC MIASMA]         [SLAG-WORM TREMORS]           [PITFALLS & CHASMS]
  • Preserved toxic breath      • Seismic tremors & cave-ins  • Rungless vertical drops
  • 1-min vs 1-hr checks        • 30 Dmg keystone collapse    • 30–60 ft falls (Athletics)
  • Crafting respirators        • Athletics / Scan tests      • Band safety rope lines
```

---

### 5.1 Hazard 1: The Balrog Neurotoxic Miasma (*Breath of the Pit*)

* **Location & Context**: Preserved within the depressed vaults of **Location 7 (The Poisoned Halls)** and **Location 8 (The Upper Armoury)**. Unleashed by Durin's Bane during the slaughter of the Dwarven Tunnel-guards, this emerald-tinged vapor pools at floor level and rises to chest height.

```
========================================================================================
                               TOXIC MIASMA EXPOSURE MATRIX
========================================================================================
 EXPOSURE TIER         TESTING INTERVAL           FAILURE CONSEQUENCE
----------------------------------------------------------------------------------------
 1. Unprotected        PROTECTION test /          Ill-favoured Feat Die roll:
    (Raw inhalation,   HEALING roll every         • 1–10: Lose that much Endurance.
     exposed mucous)   turn (or 1 min in combat)  • Eye (S): Severe Poison (lose 8 End, Weary).
----------------------------------------------------------------------------------------
 2. Protected          PROTECTION test /          Standard Feat Die roll:
    (Vinegar cloths,   HEALING roll every hour    • 1–6: Lose that much Endurance.
     chewed herbs)                                • Eye (S): Suffer Severe Poison.
----------------------------------------------------------------------------------------
 3. Masterwork Mask    COMPLETE IMMUNITY          No tests required for 4 continuous 
    (Crafted mask)     FOR 4 HOURS                exploration hours.
========================================================================================
```

* **Skill Tests & Resolutions**:
  * **Primary Exposure Test**: **PROTECTION test** or **HEALING roll**.
  * **Degrees of Success (6 icons)**:
    * **6**: Avoids all Endurance loss and grants **+1d** to an adjacent companion's test.
    * **66**: Completely resists toxic fumes, discovering an intact air pocket or downdraft flue.
    * **Gandalf (G)**: Invigorated by ancestral constitution; restores 1 Hope or clears 1 tier of Fatigue/Weary.
* **Remedies & Engineering Solutions**:
  1. **Constructing Squad Respirators (Skill Endeavour: Resistance 3 / 4)**: Heroes can assemble up to 10 squad respirators utilizing oiled leather, charcoal granules, fine brass mesh, and dwarf-goggles via **CRAFT rolls** (+1d in workshop or with appropriate tools/Traits). Success grants **4 hours of absolute immunity**.
  2. **Emergency Field Mask Repair**: If an Udûn Torch-staff scores a *Fiery Blow* against a hero wearing a mask, the mask ruptures! Repairing it in combat requires a **CRAFT roll** action (1 round).
  3. **Unjamming the Overhead Flue Damper**: In Location 7, a hero can climb the iron chimney wall and force open the ceiling ventilation lever (**ATHLETICS roll** or **CRAFT roll**). Success vents the toxic fog from the chamber in 3 combat rounds, but the shrieking iron generates **+3 Noise Points**.

---

### 5.2 Hazard 2: Slag-Worm Tremors & Structural Collapses

Centuries of mining by subterranean worms and the immense mass of Mount Silvertine have left the granite ribs of the Third Deep precarious.

```
========================================================================================
                                STRUCTURAL COLLAPSE RULES
========================================================================================
 • TRIGGER ACTIONS   : Firing heavy siege engines (Location 5), toppling the stone idol 
                       (Location 4), detonating explosive powders, or pulling the 
                       Upper Gatehouse keystone winch (Location 2).
 • DETECTION CHECK   : A hero can detect shifting keystones via a SCAN roll 
                       (Favoured if using The Broken Key or aided by a Scout).
 • HAZARD RESOLUTION : All characters in the 30-foot collapse zone must make a 
                       PROTECTION test (vs Injury 16) or ATHLETICS roll:
   - Success : The character dives clear, taking 10 Falling Rubble Damage.
   - Failure : The character suffers 20 to 30 CRUSHING DAMAGE and is PINNED under fallen 
               masonry (suffering the Weary condition until extracted).
 • DEGREES OF SUCCESS:
   - 6  : Dives clear taking 0 damage.
   - 66 : Pulls an adjacent companion clear as well, negating their check.
   - G  : Identifies an intact ancient Dwarven arch, preventing further collapse in the zone.
 • CLEARING RUBBLE   : Freeing a pinned ally requires an ATHLETICS roll or a collective 
                       Band WAR (3d vs Band TN 15) test.
========================================================================================
```

---

### 5.3 Hazard 3: Subterranean Pitfalls & Chasm Crossings

Between the Mustering-Yard and the lower arsenals lie deep natural fissures, eroded drainage shafts, and deliberately cut Orc pitfalls.

```
========================================================================================
                             PITFALL & CHASM HAZARD MATRIX
========================================================================================
 HAZARD TYPE           LOCATION          PRIMARY SKILL TEST      CONSEQUENCE ON FAILURE
----------------------------------------------------------------------------------------
 False Flagstone Pit   Location 3        SCAN roll to spot;      Falls 30 ft onto iron spikes: 
 (Orc Scrap-Trap)      (First Armoury)   CRAFT roll to disarm    15 Damage, Moderate Poison, 
                                         (+1d Burglary)          +2 Alert Points.
----------------------------------------------------------------------------------------
 Fractured Chasm       Location 1 -> 2   ATHLETICS roll          Slips into the abyss! Must be 
 (Acoustic Bridge)     (Gatehouse)       (+1d with rope anchor)  held by companion safety rope 
                                                                 (+2 Noise Points).
----------------------------------------------------------------------------------------
 Rungless Ladder       Location 7 -> 8   ATHLETICS roll          Falls 40 ft into toxic pool: 
 (Corroded Shaft)      (Upper Armoury)   (+1d with rigging)      16 Damage, mask compromised.
========================================================================================
```

* **Degrees of Success (6 icons) for Trap & Chasm Tests**:
  * **6**: Action completed silently in half the time; recovers useful scrap parts or secures the anchor point.
  * **66**: Perfectly dismantles mechanism or sets a permanent safety line, allowing all companions to pass without rolling.
  * **Gandalf (G)**: Discovers a concealed floor cache or secret Dwarven ledge containing ancient supplies.

---

### 5.4 Hazard 4: Subterranean Water Perils Table

Whenever a character drinks from an untested pool, well, or cistern in the Third Deep, the GM rolls the **Feat Die**:

```
========================================================================================
                             SUBTERRANEAN WATER PERIL TABLE
========================================================================================
 FEAT DIE ROLL   WATER CONDITION & GAMEPLAY EFFECT
----------------------------------------------------------------------------------------
 Gandalf (G)     Pristine Ancient Dwarven Spring: Crystal-clear mineral water. Restores 
                 2 points of Hope and clears the Weary condition immediately!
 9–10            Icy Snowmelt: Freezing runoff from Silvertine; refreshing, cold, and safe.
 7–8             Orc-Filth: Befouled with offal and tallow grease. Moderate Poison 
                 (4 Endurance loss; cured by Brief Rest or HEALING roll).
 5–6             Bitter Mineral Water: Heavy with alkali salts. Must pass a VALOUR 
                 test to swallow; failure causes violent nausea (-1d on physical 
                 tests for 1 hour).
 3–4             Acidic Mine Runoff: Tainted with nitric acid and heavy metals. Severe 
                 Poison (8 Endurance loss and Weary condition; requires HEALING roll).
 1–2             Malice of Durin's Bane: Tainted by demonic residue. The imbiber gains 
                 2 Shadow Points (Sorcery) and experiences terrifying hallucinations.
 Eye (S)         The Lurker's Pool: The water appears clear, but the cistern is guarded 
                 by a concealed Udûn sentry ambush or a subterranean stone-crawler!
========================================================================================
```

---

## 7. Tactical GM Dashboard & Adversary Combat Matrix

```
=======================================================================================================
                                     RAPID ADVERSARY COMBAT MATRIX
=======================================================================================================
 FOE NAME            AL  END  MIGHT HATE PARRY ARMOUR  MAIN ATTACK       DMG/INJ   KEY FELL ABILITY
-------------------------------------------------------------------------------------------------------
 The Mauler          10  80     2    10    —     5d    Maul / Club 3d    8 / 16    Dull-Witted, Toughness
 Grimnar Disgraced    6  36     2     6   +2     3d    Scimitar/Dagger 3d 5 / 16    Snake-Speed, Retaliate
 Grik the Skulker     3  12     1     2   +3     1d    Jagged Knife 2d   3 / 12    Craven, Sneak Shadows
 Udûn Sniffer         4  16     1     4    —     3d    Torch-staff 3d    4 / 14    Fiery Blow, Keen Scent
 Orc Guard            4  16     1     4   +2     3d    Heavy Scimitar 3d 4 / 16    Denizen Dark, Thick Arm
 Orc Soldier          3  12     1     3   +1     2d    Orc-axe 2d        3 / 18    Craven, Break Shield
 Orc Drummer          3  12     1     3   +1     2d    Curved Knife 2d   3 / 14    Drums in Deep (+3 Eye)
 Black Uruk           5  20     1     5   +2     3d    Broadsword 3d     4 / 16    Horrible Strength
 Black Uruk Captain   6  24     2     6   +3     4d    Great Scimitar 3d 5 / 16    Yell of Triumph
=======================================================================================================
```

---

### 7.1 Band Combat Integration Rules

When running mass skirmishes involving the Player-Heroes and their seven companion Dwarves against these adversary profiles, apply the following squad mechanics:

1. **Interception vs The Mauler**: If The Mauler targets a Player-Hero with a crushing *Maul* strike, **Dolg the Bulwark** or **Dúrmer the Iron-Shouldered** can use their combat reaction to interpose their tower-shield, taking the blow against their own Armour (3d/4d) and Endurance.
2. **Countering Grimnar's Speed**: If Grimnar uses *Snake-like Speed* to make a hero's attack Ill-favoured, a hero can spend 1 point of Band Hope or use a *Leadership* action to coordinate a flanking maneuver with a companion, restoring the attack to standard or Favoured status.
3. **Suppressing Orc Drummers**: When an Orc Drummer begins chanting to use *Drums in the Deep*, an archer or companion can make an overwatch shot from Rearward stance to neutralize the drummer before the kettle-drum sounds.

---

### 7.2 Safety & Escalation Rules for the GM

* **Avoiding Unfair Swarms**: Never spawn additional adversary waves unless the **Alert Tracker** actively advances to a new Tier or a surviving Orc escapes into a side flue.
* **The Mauler Escalation Arc**: If the party engages The Mauler in straight melee without using the catwalks, stalactites, or Riddle duel, allow a hero invoking *Enemy-lore (Orcs)* or making a **SCAN roll** to highlight the loose ceiling stones or the beast's weeping fear of pain.
* **Rewarding Stealth**: Defeating sentries silently in 1 round generates **+0 Noise Points**, allowing tactical parties to clear outposts in Locations 1, 3, and 5 without raising the dungeon alarm.



<!-- PAGE BREAK: Chapter 6 -->

---


# Chapter 6: Relics, Rewards, Scavenge Tables & Royal Vault Loot

> *"There lies the axe of Durin, hewn when the earth was young,*  
> *Upon its blade the ancient stars in silver cold are hung.*  
> *No rust shall bite its adamant, no orc-fire dull its gleam,*  
> *Till Durin walks beneath the peaks and wakes from stone and dream."*  
> — *Lay of the Lost Armouries, stanzas recovered in the Iron Hills*

---

## 1. Overview: The Wealth of the Third Deep

The Armouries of the Third Deep represent the supreme martial repository of ancient Khazad-dûm. Situated below the primary residential mansions of the Second Deep and above the fathomless smelters and mines of the Lower Deeps, this fortified level was designed to outfit an entire Dwarven army during the glory of the Second Age.

When the Balrog (*Durin's Bane*) awoke in 1980 TA and slew King Durin VI and King Náin I, the royal marshals sealed the Third Deep's innermost sanctuaries behind runic wards, toxic flues, and blast-portals to prevent the Great Enemy from despoiling the heritage of the Longbeards.

```
========================================================================================
                          REWARDS MATRIX & STRATEGIC VALUE
========================================================================================
 REWARD CATEGORY            PRIMARY LOCATION        CAMPAIGN & MECHANICAL IMPACT
----------------------------------------------------------------------------------------
 1. Durin's Axe             Location 10 (Vault)     Royal Artifact; +4 Eye Awareness; 
                                                    Key to convincing King Dáin Ironfoot.
 2. Tunnel-Guard Wargear    Locations 8 & 10        Masterwork Mithril & Gromril Gear; 
                                                    Favoured combat bonuses & Virtues.
 3. Mithril & King's Keys  Rooms 1/5/Last Redoubt   Unlocks Location 9 (The King's Door); 
                                                    Vital puzzle key / bypass mechanic.
 4. Garrison Armament Haul  Location 8 (Upper Arm.) 40+ Mail, 30 Shields, Crates of Axes; 
                                                    Equips 50 frontline Dwarves for Balin.
 5. Custom Greater Hoard    Location 10 (Vault)     Royal gold, Mirrormere gems, gromril 
                                                    ingots; 120+ Treasure Points.
 6. D66 Scavenge Table      Throughout Third Deep   36 Unique subterranean items, lore curios, 
                                                    tactical consumables, and relics.
========================================================================================
```

---

## 2. Durin's Axe (The Royal Artifact of Khazad-dûm)

```
       ▲               =============================================
      / \                             DURIN'S AXE
     /   \                 (The Heirloom of the First King)
    /=====\            =============================================
   / |   | \           "Hewn by Mahal in the elder morning of Arda."
  |  |   |  |
  |  |   |  |          • Weapon Category: Great Axe (Two-handed)
   \ |   | /           • Damage: 9 (Base 7 + Superior Grievous +2)
    \=====/            • Injury: 20
      | |              • Load: 4
      | |              • Craft Qualities: Rune-scored, Superior Grievous,
      | |                                 Superior Keen (Pierce on 8+)
      | |              • Cultural Traits: Flame of Hope, Gleam of Terror
      | |              • Doom of the Deeps: +4 Eye Awareness upon claiming
      | |              =============================================
```

### 2.1 Historical Lore & Provenance
Forged in the Elder Days before the rising of the Sun and Moon, **Durin's Axe** is whispered to have been struck on the very anvil of **Aulë the Maker** (whom the Dwarves call *Mahal*). Borne by **Durin the Deathless** when he first founded the mansions of Khazad-dûm, the axe was passed down through fifty generations of Longbeard kings.

Its broad, crescent-shaped head is wrought of an adamantine silver-iron alloy that neither ages, rusts, nor notches. Along the spine of the blade run twenty-four radiant Khuzdul runes of striking, inlaid with pure ithildin that glows with a pale, cold azure light whenever orcs, trolls, or creatures of the ancient shadow draw near.

When Khazad-dûm fell in 1981 TA, the dying royal guards placed the axe upon the adamant Dais of the High King in the sealed Lower Armoury, sealing the portal with the King's Door so that no defiler's hand might touch it. For over one thousand years, it has rested in the silent dark, awaiting a true son of Durin to take up the heirloom of the mountain.

---

### 2.2 Complete TOR 2e Profile

```
========================================================================================
                     ARTIFACT PROFILE: DURIN'S AXE (TOR 2e)
========================================================================================
 WEAPON TYPE:          Great Axe (Two-handed)
 DAMAGE:               9 (Base 7 + Superior Grievous +2)
 INJURY:               20
 LOAD:                 4
 CRAFT QUALITIES:      • Rune-scored (Indestructible; all attack rolls made with 
                         Durin's Axe are Favoured)
                       • Superior Grievous (+2 Damage)
                       • Superior Keen (Scores Piercing Blow on 8, 9, 10, or Gandalf Rune)
----------------------------------------------------------------------------------------
 PRE-UNLOCKED DWARF CULTURAL TRAITS (Manifested when wielded by any Dwarf):
 
 1. FLAME OF HOPE:
    • The azure runes blaze like mountain sunlight in darkness, illuminating a 30-foot 
      radius. All subterranean stealth penalties caused by total darkness are negated for 
      the Company.
    • In combat, the wielder may spend 1 point of Hope to inspire all allies: every 
      companion and Player-Hero gains +1d on all attack rolls and Protection tests for the 
      next combat round, or clears the Weary condition immediately.
 
 2. GLEAM OF TERROR:
    • When the axe is drawn in battle, its keen edge rings with the thunder of the forge.
    • Intimidate Foe combat tasks (AWE) made by the wielder are automatically FAVOURED.
    • On a success, the targeted adversary (or all minions of Attribute Level <= 4 in the 
      opposing front rank) loses 2 Hate/Resolve instead of 1, and must make an immediate 
      Dread test (**VALOUR roll**, Ill-favoured) or flee to the rear rank.
----------------------------------------------------------------------------------------
 SHADOW ATTRACTION & THE WEIGHT OF DOOM:
 • The ancient malice of Moria is bound to the axe. The moment a hero lifts Durin's Axe 
   from the granite plinth in Location 10, the mountain groans in resonance.
 • The Company's STRATEGIC EYE AWARENESS IMMEDIATELY INCREASES BY +4 POINTS.
 • If this increase causes Eye Awareness to meet or exceed the Hunt Threshold (14), a 
   REVELATION EPISODE is triggered at the worst possible tactical moment during the 
   extraction (see Chapter 3, Section 2.2).
========================================================================================
```

---

### 2.3 Strategic & Narrative Impact in the Campaign
1. **The Mandate of King Dáin Ironfoot**: Presenting Durin's Axe to the emissaries of Erebor provides irrefutable royal proof that Khazad-dûm can be reclaimed. It guarantees that King Dáin will mobilize **five hundred heavy Dwarf veterans** from the Iron Hills and Lonely Mountain to reinforce Balin's colony in 2990 TA.
2. **Heir of Durin's Destiny**: If a Dwarf hero of Durin's Folk (such as a Champion with Great Axe Mastery) takes up the axe, their resolve is both empowered and imperiled. When fighting Orcs with Durin's Axe, a wielder with Great Axe Mastery scores Piercing Blows on a Feat die roll of **7, 8, 9, 10, or Gandalf Rune** (stacking Mastery with Superior Keen). However, failing a **VALOUR roll** while wielding the axe adds **+2 Shadow (Dread)** instead of 1.

---

## 3. Tunnel-Guard Wargear (The Citadel Armaments)

Forged in the legendary deep-smithies of the Third Age, these masterwork pieces were issued exclusively to the *Wardens of the Deeps*—the elite royal bodyguards who patrolled the subterranean gates and defended the King's person.

```
========================================================================================
                            TUNNEL-GUARD WARGEAR MATRIX
========================================================================================
 ITEM NAME                CATEGORY           MECHANICAL PROFILE & CRAFT VIRTUE
----------------------------------------------------------------------------------------
 Shield of the Deep Gate  Reinforced Shield   Parry +3, Load 3. Virtue: *Unyielding*
 Mattock of Moria-Silver  Two-handed Mattock Damage 8, Injury 18, Load 3. Virtue: *Gleaming Edge*
 Mail of Unyielding Stone Coat of Mail       Protection 5d, Load 12. Virtue: *Impenetrable*
 Helm of the Iron Watch   Dwarven Helm       Protection +1d, Load 1. Virtue: *Vigilant Sentinel*
 Pike of the Under-Gate   Heavy Spear/Pike   Damage 5, Injury 16, Load 3. Virtue: *Foe-Piercer*
========================================================================================
```

---

### 3.1 Shield of the Deep Gate (Enchanted Great Shield)
* **Item Type**: Reinforced Dwarf-Shield (Great Shield)
* **Parry Modifier**: **+3** | **Load**: 3 | **Reward Qualities**: *Reinforced* (Unbreakable), *Cunning Make*
* **Craft Virtue — Unyielding**:
  * **Anti-Crush & Anti-Seize**: The bearer cannot be knocked down, pushed back, or trapped in the *Seized* condition by trolls, cave-beasts, or adversary Special Damage triggers while braced in the Defensive or Forward stance.
  * **Shield-Wall Bulwark**: When the bearer forms part of a *Shield-Wall Phalanx* (see Chapter 2), the Band gains an automatic **+1d on all Band War rolls** and absorbs 1 additional point of enemy Resistance damage per round.
  * **Adversary Weapon Sunder**: When an adversary rolls a failure on an attack against the bearer while showing an Eye of Sauron ($\mathbf{S}$) on the Feat die, their mundane weapon (scimitar, spear, club) shatters against the adamant rim.

---

### 3.2 Mattock of Moria-Silver (True Mithril-Alloy Great Weapon)
* **Item Type**: Two-handed Mattock
* **Damage Rating**: **8** (Base 7 + Grievous +1) | **Injury Rating**: **18** | **Load**: 3 (Forged of lightweight Mithril-steel)
* **Craft Qualities**: *Grievous*, *Close Fitting* (Load reduced from 5 to 3), *Superior Craftsmanship*
* **Craft Virtue — Gleaming Edge**:
  * **Subterranean Superiority**: Attacks made with this mattock against underground adversaries (Orcs, Goblins, Cave-trolls, Spiders) in dark or subterranean environments are permanently **FAVOURED**.
  * **Armour Penetration**: On a Piercing Blow, the target adversary's Protection roll suffers a **-1d penalty** due to the razor-keen, high-density Moria-silver pick-head.
  * **Breaching Utility**: Adds **+2d** on all Craft, Athletics, or Band War checks made to shatter stone doors, breach Orc barricades, or smash siege masonry.

---

### 3.3 Mail of Unyielding Stone (Gromril-Iron Weave Coat of Mail)
* **Item Type**: Coat of Mail
* **Protection Rating**: **5d** | **Load**: **12** (Specially treated dwarf-iron rings laced with gromril wire; Load reduced from 16 to 12)
* **Craft Qualities**: *Close Fitting* (-4 Load), *Reinforced*
* **Craft Virtue — Impenetrable**:
  * **Injury Reduction**: When the wearer suffers a Piercing Blow and fails their Protection test, they may spend **1 point of Hope** to immediately reduce the severity of the resulting Injury by one step:
    * *Grievous Injury $\rightarrow$ Severe Injury*
    * *Severe Injury $\rightarrow$ Moderate Injury*
    * *Moderate Injury $\rightarrow$ Fleeting Injury*
  * **Crush & Tremor Resistance**: The wearer takes half damage (rounded down) from environmental hazards involving falling rocks, ceiling collapses, or boulder strikes.

---

### 3.4 Helm of the Iron Watch (Runed Visored Sallet)
* **Item Type**: Dwarven Helm
* **Protection Modifier**: **+1d** | **Load**: 1 (Forged of fluted, blackened steel with electrum inlays)
* **Craft Virtue — Vigilant Sentinel**:
  * **Darkvision & Whispering Stone**: Grants the wearer **Favoured** status on all **Awareness**, **Vigilance**, and **Scan** rolls made while underground.
  * **Immunity to Drop Ambushes**: The wearer cannot be surprised or ambushed by foes attacking from catwalks, ceiling fissures, or overhead shadows (such as Grimnar’s stalkers in Room 9 or cave bats).
  * **Tremor-Sense**: The helm hums faintly when structural cave-ins or heavy beast footsteps (such as The Mauler) approach within 100 yards, granting the Company **+1 round of preparation** before encounters.

---

### 3.5 Pike of the Under-Gate (Masterwork Vault-Spear)
* **Item Type**: Long Spear / Pike (Two-handed)
* **Damage Rating**: **5** | **Injury Rating**: **16** | **Load**: 3
* **Craft Qualities**: *Keen* (Pierce on 9–10), *Grievous* (+1 Damage)
* **Craft Virtue — Foe-Piercer**:
  * **Phalanx Reach**: When fighting in Forward or Open stance, the wielder may attack from behind an ally who is in Defensive stance without penalty.
  * **Anti-Charge Impale**: If an adversary charges the wielder (such as The Mauler or Orc Guards), the wielder strikes first in the round; if a Piercing Blow is scored, the adversary's charge is cancelled and they lose 2 Hate immediately.

---

## 4. The Marshal's Key & Vault Penetration Subsystem

To open **The King's Door** (Keyed Location 9) and access the treasures of the Lower Armoury (Keyed Location 10), the ancient Dwarven security system requires two distinct master keys:
1. **The King's Key**: Canonical heirloom residing in the skeletal hand of King Náin I in the *Last Redoubt of the Dwarves* (Fifth Hall).
2. **The Marshal's Key**: The master bypass key carried by the Marshal of the Third Deep during the final evacuation, lost in the upper Deeps.

The Player-Heroes have three distinct operational pathways to secure or bypass the Marshal's Key.

```
                               ┌─────────────────────────────────┐
                               │   UNLOCKING THE KING'S DOOR     │
                               │   (Keyed Location 9 Resolution) │
                               └────────────────┬────────────────┘
                                                │
         ┌──────────────────────────────────────┼──────────────────────────────────────┐
         ▼                                      ▼                                      ▼
  [PATHWAY 1: COMBAT AMBUSH]           [PATHWAY 2: SOCIAL DEAL]              [PATHWAY 3: CRAFT ENDEAVOUR]
   • Intercept Mordor Patrol            • Negotiate with Grik                 • Masterwork Lock Bypass
   • Slay Captain in Room 3 or 5        • Trade: Silver / Tobacco / Revenge   • Resistance 6 / Time Limit 3
   • Alert: +1 Tier on noise            • Alert: +0 (Completely Silent)       • Failure: Alarm & Broken Tools
```

---

### 4.1 Pathway 1: Defeating the Mordor Patrol Captain (Combat Infiltration)
* **Context**: Centuries after the fall of the Third Deep, a Mordor Orc scouting patrol ransacking the ruins near the Dimrill Stairs discovered the shining Mithril Key where Marshal Thrain fell. The key is currently worn as an ostentatious war-talisman around the neck of **Captain Grashnak** (a Mordor Uruk Champion). It lets the Mordor Orcs slip secretly in and out of the mountain via the pillar at the Dimrill Stairs.
* **Location**: Wandering between **Location 3 (First Armoury)** and **Location 5 (Second Armoury)**.
* **Encounter Breakdown**:
  * **Enemy Roster**: Captain Grashnak (Attribute Level 6, End 24, Might 2, Hate 6, Armour 3d, Heavy Scimitar 3d [5/16, Pierce], *Horrible Strength*) leading 3 **Moria Sniffers** (AL 4, End 16, Hate 4).
  * **Tactical Conditions**: The patrol moves with smoky torches, casting wild shadows. If the Company uses the *Forward Scout Screen* (Austri & Bróga), they spot the patrol 40 paces away.
  * **Combat Resolution**:
    * If ambushed and eliminated within **2 combat rounds**, the key is claimed with **+0 Noise Points** generated.
    * If combat extends to **3+ rounds**, or if Grashnak bellows a challenge, the clash generates **+2 Noise Points** (advancing the Alert Tracker by +1/2 Tier).
    * If any Orc flees into the vents, the escape immediately adds **+2 to Strategic Eye Awareness**.
* **Reward**: The physical **Mithril Key**, forged of shining dwarf-mithril and star-iron, etched with the secret rune of the Dimrill Stairs pillar.

---

### 4.2 Pathway 2: Bargaining with Grik the Skulker (Social Parley)
* **Context**: **Grik the Skulker** (the cunning goblin informant lurking near Location 1's Mustering-Yard) has watched Grashnak's patrol for days. Grik knows Grashnak carries the "bright mithril key" that lets the Mordor Orcs enter secretly via the pillar at the Dimrill Stairs. Crucially, Grik does **not** know this key also unlocks the left ward of the King's Door to the treasure chamber (he only knows it operates the secret stairs entrance). Grik knows where the patrol sleeps in the ventilation flues above Location 3. Alternatively, if Grik managed to pilfer it during a drunken brawl, he has it stashed in a bone-filled crevice near the Upper Gatehouse.
* **Negotiation Encounter**:
  * **Grik’s Disposition**: Opportunistic, terrified of the Mordor Orcs, greedy for luxuries.
  * **Grik’s Demands (The Price)**: Grik will deliver the key (or lead the heroes to its exact hiding spot) in exchange for any **two** of the following:
    1. **Bright Silver / Dwarf Coin**: 5 Treasure Points / 30 Silver Pennies.
    2. **Fragrant Surface Tobacco**: A pouch of *Old Toby* or Longbottom Leaf (carried by a hero or companion).
    3. **Blood Revenge**: The heroes must ambush and slaughter Grashnak's brute squad without implicating Grik.
    4. **Dwarven Strong Spirits**: An iron flask of fiery Mountain Spirits.
  * **Skill Resolution**:
    * **RIDDLE roll** or **PERSUADE roll**: Success convinces Grik to accept a single offering and provide a crude charcoal map of the patrol's blind spot.
    * **ENHEARTEN roll** or **AWE roll**: A hero can terrify Grik into surrendering the key immediately. However, an intimidated Grik will alert Grimnar at Alert Tier 2 out of spite (+1 Eye Awareness).
* **Benefit**: A completely silent acquisition (**+0 Noise Points, +0 Alert increase**).

---

### 4.3 Pathway 3: Masterwork Craft Skill Endeavour (The Lockbreaker Bypass)
* **Context**: If the Company fails to find the physical key or wishes to bypass the search entirely, a hero or companion can attempt to crack the ancient triple-tumbler adamant lock of the King's Door using masterwork lockpicks, stone-wedges, counterweights, and cunning tools.

```
========================================================================================
                  SKILL ENDEAVOUR: CRACKING THE KING'S DOOR
========================================================================================
 RESISTANCE RATING:   6 Successes Required
 TIME LIMIT:          3 Endeavour Turns (Each turn = 10 minutes of intense work)
 KEYED HAZARD:        Internal Deadfall Trap & Acoustic Chime
 PARTICIPANTS:        Primary: Lockbreaker (Craft / Scan / Riddle)
                      Support: Company Helpers (Anchor / Brace / Lookout)
========================================================================================
```

#### Turn-by-Turn Skill Options & Mechanics

```
Turn 1: Tumbler Alignment ──► Turn 2: Tension & Deadfall Bypass ──► Turn 3: Forcing the Bolt
 (Scan / Craft)                  (Craft / Riddle / Athletics)         (Craft / Athletics / Band Expertise)
```

1. **Turn 1: Aligning the Internal Wards**:
   * *Eligible Skills*: **SCAN roll** or **CRAFT roll**.
   * *Tool & Trait Synergy*: Invoking a Trait such as *Burglary* or using keen locksmith tools grants $+1\text{d}$ (or Favoured status).
   * *Companion Synergy*: A companion invoking *Vaultbreaker* grants $+1\text{d}$.
   * *Degrees of Success (6 icons)*: Each standard success adds 1 point toward Resistance; each Success icon ($\mathbf{6}$) adds 1 additional point (2 points total). Gandalf Rune ($\mathbf{G}$) contributes 2 points and reveals the internal gear sequence.
2. **Turn 2: Disarming the Counterweight Deadfall**:
   * *Eligible Skills*: **CRAFT roll** ($+1\text{d}$ if invoking the *Burglary* Trait) or **RIDDLE roll** (deciphering the runic puzzle sequence).
   * *Complication on Failure*: If no successes are rolled on Turn 2, an internal stone needle snaps: the lock releases a loud harmonic chime, generating **+2 Noise Points** (advancing the Alert Tracker).
3. **Turn 3: Forcing the Final Gromril Tumbler**:
   * *Eligible Skills*: **CRAFT roll**, **ATHLETICS roll** (turning heavy iron levers), or Band **EXPERTISE roll** (2d vs **Band TN 15**).
   * *Bracing Support*: A supporting hero can spend 1 Hope to brace the torsion bar, granting $+1\text{d}$ to the roller.

#### Endeavour Outcomes
* **Total Success ($\ge 6$ Successes within 3 Turns)**: The massive adamant doors slide back silently along oiled basalt tracks. The King's Door opens without triggering the alarm (+0 Noise).
* **Marginal Success (Reached 6 Successes on Turn 3 with failures)**: The door opens, but the internal counterweight crashes into the floorpit, generating **+3 Noise Points** (+1 Alert Tier, +1 Eye Awareness).
* **Failure ($< 6$ Successes after 3 Turns)**: The lock jams permanently in emergency lockdown! To open the door now requires a violent battering operation with a siege ram from Location 5 (generating **+5 Noise Points** and triggering an instant **Alert 3 / Drums in the Deep** alarm).

---

## 5. The Royal Greater Hoard & Garrison Logistics

Beyond the legendary *Durin's Axe*, the Lower Armoury (Keyed Location 10) holds the pristine treasures of the royal house of Khazad-dûm, unlooted since 1981 TA.

```
========================================================================================
                          THE GREATER HOARD OF THE THIRD DEEP
========================================================================================
 HOARD VALUATION:       120+ TREASURE POINTS (Royal Vault Category)
 MAGICAL REWARD DICE:   Roll 3d6 on the Magical Treasure Index (TOR 2e Core pp. 158–162)
 SPECIAL METALS:        12 Crated Ingots of Refined Moria-Silver (True Mithril / Gromril)
 GARRISON ARSENAL:      40+ Dwarf-forged Mail-shirts, 30 Tower Shields, 60 War-axes
 STRATEGIC PAYOFF:      EQUIPS 50 FRONTLINE DWARVES & SECURES UPPER MANSIONS
========================================================================================
```

---

### 5.1 Detailed Hoard Inventory
1. **The Royal Regalia of the Third Deep**:
   * *The Mithril Circlet of the High Warden*: A delicate crown of interwoven ithildin wire set with three radiant starlight-diamonds from the depths of Celebdil. Worth **30 Treasure Points** or grants **+1d to all ENHEARTEN and PERSUADE rolls** when worn by a Dwarf of noble blood.
   * *The Golden Casket of Mirrormere Pearls*: A cedar box bound in platinum containing forty-eight lustrous, black subterranean river-pearls dredged from Kheled-zâram before the Shadow fell. Worth **25 Treasure Points**.
   * *Ceremonial Gorget of King Náin I*: Heavy electrum neck-guard chased with scenes of the founding of the Great Gates. Worth **15 Treasure Points**.
2. **Refined Gromril (Mithril) Ingots**:
   * Twelve stamped ingots of pure, refined Moria-silver, each bearing the royal seal of the Crown and Seven Stars.
   * *Weight & Value*: Each ingot weighs 2 pounds and is worth **10 Treasure Points** (120 Treasure Points total for all twelve).
   * *Crafting Power*: Delivering these ingots to master smiths (such as Hjoldring or the forges of Erebor) allows the forging of **up to three custom enchanted Dwarven weapons or armor pieces** during future Fellowship Phases.
3. **Ancient Royal Cartography**:
   * Preserved slate maps showing the forgotten aqueducts, ventilation flues, and hydraulic lift shafts connecting the Third Deep to the *Sixth Deep* and the *Endless Stair*. Grants **Favoured status on all EXPLORE rolls** in deeper sectors of Moria.

---

### 5.2 Garrison Logistics: Arming Balin's 50 Soldiers
The primary operational goal of the delve is salvaging enough heavy wargear to outfit Balin's vulnerable frontline garrison at the East-Gate Camp.

```
========================================================================================
                           GARRISON SALVAGE LOGISTICS
========================================================================================
 RECOVERED ARSENAL:    • 40 Suits of Dwarf-forged Mail-shirts (Standard Protection 3d/4d)
                       • 30 Heavy Iron-rimmed Oak-and-Steel Shields (+2 Parry)
                       • 60 Masterwork Broad-axes, War-mattocks, and Heavy Spears
                       • 10 Crates of Crossbow Bolts and Horn-bow Staves
----------------------------------------------------------------------------------------
 TRANSPORT REQUIREMENTS:
 • Requires the entire Company and the 7 Companion Dwarves operating as the 
   HEAVY SALVAGE PORTER SQUAD (Hjoldring & Dúrmer leading).
 • Shifts Band Burden rating from Medium to HEAVY / OVERBURDENED:
   - Band Manoeuvre rolls suffer -1d.
   - Marching Fatigue tests suffer -1d (or -2d if resting is skipped).
   - Moving between keyed locations generates +1 Noise Point unless muffled with cloth.
----------------------------------------------------------------------------------------
 STRATEGIC PAYOFF:
 • Delivering the arsenal back to Commander Fróra at the East-Gate Camp awards:
   1. GARRISON ARMAMENT: Fully equips 50 frontline Dwarves of Balin's colony with 
      dwarf-mail and masterwork weapons, securing the Upper Mansions permanently.
   2. REPUTATION & RENOWN: The Player-Heroes gain +2 Fellowship Score, +4 Adventure 
      Points, +3 Skill Points, and are declared *Champions of the Deeps* by Lord Balin.
   3. ROYAL REINFORCEMENTS: Convinces King Dáin Ironfoot in Erebor to dispatch 500 
      veteran Dwarf reinforcements to Moria in 2990 TA.
========================================================================================
```

---

## 6. The Complete D66 Moria Scavenge Table

Whenever Player-Heroes or companion scouts spend time searching ransacked lockers, ancient corpses, rubble piles, or forge recesses throughout the Third Deep, the GM rolls **D66** (roll two 6-sided dice, the first die representing tens and the second die representing units, yielding numbers from 11 to 66).

Every single one of the 36 entries is detailed below with evocative lore, immediate mechanical utility, and trade/salvage value.

```
========================================================================================================================
                                             THE D66 MORIA SCAVENGE TABLE
========================================================================================================================
 D66  ITEM NAME                   HISTORICAL LORE & DESCRIPTION             MECHANICAL UTILITY & IN-GAME EFFECT    VALUE
------------------------------------------------------------------------------------------------------------------------
 11   Cured Dwarf-Tobacco         A sealed lead pouch of "Long-Beard Leaf"  Consuming a pipeful during a rest      2 Silver
      (Second Age Vintage)        grown in ancient mountain terraces.       restores +1 Hope to a Dwarf.           Pennies
------------------------------------------------------------------------------------------------------------------------
 12   Gromril-Tipped Pitons       Four masterwork climbing spikes forged    Grants +1d on all ATHLETICS rolls      1 Treasure
      (Set of 4)                  of blackened steel with mithril points.   made to scale walls or secure ropes.   Point
------------------------------------------------------------------------------------------------------------------------
 13   Scribe's Luminescent Ink    A crystal vial of glowing squid-bile      Can be used to write secret messages   5 Silver
                                  mixed with crushed phosphorescent moss.   visible only in pitch darkness.        Pennies
------------------------------------------------------------------------------------------------------------------------
 14   Runic Whetslate of Erebor   An abrasive whetstone etched with runes   Sharpening a bladed weapon grants +1   1 Treasure
                                  of sharpening by ancient weapon-smiths.   Damage on its very next strike.        Point
------------------------------------------------------------------------------------------------------------------------
 15   Lead-Sealed Rations         A flat tin of spiced mountain goat pem-   Provides 3 hearty meals; removes the   3 Silver
      (Hard Pemmican)             mican cured in honey and rock-salt.       Hungry condition without making fire.  Pennies
------------------------------------------------------------------------------------------------------------------------
 16   Notched Orc Scimitar        A crude, blackened blade bearing the      Can be shattered to yield +1 Noise or  0 Pennies
      (Udûn Scrap)                jagged eye-brand of Malech's horde.       wielded as a disposable 3-Dmg weapon.  (Scrap)
------------------------------------------------------------------------------------------------------------------------
 21   Silver Runic Dice           A pair of weighted knucklebone dice       Grants +1d on RIDDLE tests or social   2 Treasure
      (Set of 2)                  inlaid with Khuzdul numerals.             gambling checks against foes/allies.   Points
------------------------------------------------------------------------------------------------------------------------
 22   Flask of Dragon-Fire Oil    A ceramic flask of pressurized dwarven    Throwable weapon (Ranged, using ATH-   2 Treasure
                                  naphtha used for forge bellows.           LETICS). Short range 10y, Medium 20y   Points
                                                                            (-1d), Long 30y (-2d). On a hit, deals
                                                                            6 Endurance damage. If a Piercing
                                                                            Blow is scored, or if the target
                                                                            fails a PROTECTION test (Injury TN 16),
                                                                            they suffer a Wound and are set ablaze
                                                                            (takes 2 Endurance damage per round
                                                                            until extinguished by Athletics/Craft
                                                                            roll vs TN 10). Ignites wood/flammables.
------------------------------------------------------------------------------------------------------------------------
 23   Acoustic Listening Horn     A folding brass cone crafted to magnify   Grants Favoured rolls on AWARENESS     1 Treasure
                                  vibrations traveling through solid rock.  tests made to detect approaching foes. Point
------------------------------------------------------------------------------------------------------------------------
 24   Chiseled Blood-Agate        A thumb-sized crimson gemstone dropped    Can be socketed into a weapon hilt or  3 Treasure
                                  by a fleeing jeweler during the Fall.     traded to Grik for secret intel.       Points
------------------------------------------------------------------------------------------------------------------------
 25   Hard-Tallow Scent Candles   Three beeswax tapers infused with pine    Burning one masks the Company's scent, 1 Treasure
      (Bundle of 3)               pitch and dried thyme.                    giving Moria sniffers -1d on Awareness. Point
------------------------------------------------------------------------------------------------------------------------
 26   Shattered Mail Rings        A handful of severed mithril-iron rings   Hjoldring can use these to repair a    2 Treasure
                                  torn from an ancient captain's hauberk.   damaged suit of mail (+1 Protection).  Points
------------------------------------------------------------------------------------------------------------------------
 31   Dwarven Salve of Stone-Skin A clay jar of pungent mineral paste       Applying the paste grants +1d to a     2 Treasure
                                  blended from sulfur and cave-tallow.      Protection roll against crush damage.  Points
------------------------------------------------------------------------------------------------------------------------
 32   Masterwork Pick-Head        A heavy, drop-forged steel pick-head      Grants +1d on CRAFT or ATHLETICS checks 1 Treasure
                                  bearing the hallmark of King Durin IV.    made to undermine walls or dig flues.  Point
------------------------------------------------------------------------------------------------------------------------
 33   Torn Page of the Armouries  A scorched parchment leaf recording the   Grants +1d on SCAN rolls to locate the 2 Treasure
                                  storage location of gromril mail-shirts.  hidden vault chests in Location 8.     Points
------------------------------------------------------------------------------------------------------------------------
 34   Black Orc-Poison Phial      A glass vial of thick, curdled viper-     Coating a blade inflicts Severe Poison 1 Treasure
                                  venom taken from a Mordor stalker.        on the next successful Piercing Blow.  Point
------------------------------------------------------------------------------------------------------------------------
 35   Carved Bone Whistle         A hollowed mountain-eagle wing bone       Sounds a silent, high-pitch frequency  5 Silver
                                  tuned to canine hearing.                  that distracts and confuses cave bats. Pennies
------------------------------------------------------------------------------------------------------------------------
 36   Electrum Belt Clasp         A heavy oval buckle depicting the Crown   A noble heirloom; worth immediate      4 Treasure
                                  and Seven Stars of Durin's Folk.          Treasure points or boosts ENHEARTEN.   Points
------------------------------------------------------------------------------------------------------------------------
 41   Folded Silk Rope            A 50-foot coil of gossamer-light spider-  Weighs only 1 Load; grants +1d on all  3 Treasure
      (Dwarf-Weave)               silk and silver thread rope (Load 1).     ATHLETICS rolls for climbing/ropes.    Points
------------------------------------------------------------------------------------------------------------------------
 42   Charcoal Filtration Mask    A primitive leather mask packed with      Protects the wearer against the toxic  2 Treasure
                                  activated charcoal and dried mint leaves. Balrog miasma for 2 continuous hours.  Points
------------------------------------------------------------------------------------------------------------------------
 43   Engraved Bronze Mirror      A polished bronze disc etched with the    Allows checking around corridor cor-   1 Treasure
                                  constellation of the Plough.              ners without exposing head or torso.   Point
------------------------------------------------------------------------------------------------------------------------
 44   Iron Hills Field Flask      A heavy pewter canteen containing aged,   Drinking a draught during battle im-   1 Treasure
                                  fiery plum brandy from the East.          mediately removes the Weary condition. Point
------------------------------------------------------------------------------------------------------------------------
 45   Runic Key-Blank             An uncut brass key-blank stamped with     Can be filed into a skeleton key       2 Treasure
                                  the cipher of the Royal Locksmiths.       (+1d to CRAFT when picking locks).     Points
------------------------------------------------------------------------------------------------------------------------
 46   Severed Troll-Claw Amulet   A fossilized cave-troll talon strung on   Wearing it grants +1d to AWE on        1 Treasure
                                  a thong of dried wolf-sinew.              Intimidate Foe against goblins/Orcs.   Point
------------------------------------------------------------------------------------------------------------------------
 51   Vial of Mirrormere Water    A lead-stoppered crystal flask containing Imbibing this water purges all poison  5 Treasure
                                  pure, blessed water from Kheled-zâram.    and restores 2 points of Hope.         Points
------------------------------------------------------------------------------------------------------------------------
 52   Mithril Wire Filigree       A spool of hair-thin True Mithril wire    Can be used to craft delicate snares,  3 Treasure
      (10 Feet)                   used for binding luxury wargear.          tripwires, or repair magical gear.     Points
------------------------------------------------------------------------------------------------------------------------
 53   Ancient Stone Mason's Wedge A hardened steel wedge stamped with the   Hammering it into a doorframe perma-   1 Treasure
                                  rune of the Master Builders of Khazad.    nently jams a door against pursuit.    Point
------------------------------------------------------------------------------------------------------------------------
 54   Preserved Herbal Poultice   A linen wrap impregnated with dried       Grants +1d on a HEALING test to treat  2 Treasure
                                  Athelas and crushed wintergreen roots.    Severe or Grievous wounds.             Points
------------------------------------------------------------------------------------------------------------------------
 55   Serrated Goblin Scalp Knife A rusted, jagged iron blade crusted with Gives +1d on HUNTING tests in the dark; 0 Pennies
                                  dried black blood.                        despised by Dwarves.                   (Scrap)
------------------------------------------------------------------------------------------------------------------------
 56   Gold Ingot of the Royal Mint A stamped 1-pound ingot bearing the      Pure currency; instantly convertible   5 Treasure
                                  anvil mark of King Náin I.                into royal wealth in Dale or Erebor.   Points
------------------------------------------------------------------------------------------------------------------------
 61   Ithildin Lens               A ground crystal lens rimmed in silver;   Reveals hidden moon-runes, secret door 4 Treasure
                                  catches faint ambient starlight.          seams, and concealed writing in dark.  Points
------------------------------------------------------------------------------------------------------------------------
 62   Dwarven War-Horn Mouthpiece A carved silver mouthpiece salvaged from  Fitting it to a war-horn grants +1d to 2 Treasure
                                  a shattered muster-horn of the Guard.     subsequent BATTLE and RALLY rolls.     Points
------------------------------------------------------------------------------------------------------------------------
 63   Black Iron Caltrops         A leather bag containing twenty four-     Throwing them behind the Company slows 1 Treasure
      (Bag of 20)                 pointed iron spikes.                      pursuing Orcs and inflicts 4 Damage.   Point
------------------------------------------------------------------------------------------------------------------------
 64   Seal of the Third Marshal   A heavy bronze signet ring bearing the    Grants Favoured rolls on COURTESY and  3 Treasure
                                  crest of the Lord Marshal of the Armies.  PERSUADE with Balin’s officers/kin.    Points
------------------------------------------------------------------------------------------------------------------------
 65   Ancient Dwarven Compass     A gimballed magnetic lodestone set in a   Functions flawlessly underground,      3 Treasure
                                  case of carved serpentine stone.          preventing Company from getting lost.  Points
------------------------------------------------------------------------------------------------------------------------
 66   True Gromril Chain Link     A single link of pristine, gleaming       Radiates faint starlight; grants the   10 Treasure
                                  True Mithril, cold to the touch.          bearer +1 Hope and cleanses 1 Shadow.  Points
========================================================================================================================
```

---

## 7. Integration with Other Chapters

```
┌───────────────────────────────────────────────────────────────────────────────────────┐
│                           CROSS-CHAPTER INTEGRATION MAP                               │
└───────────────────────────────────┬───────────────────────────────────────────────────┘
                                    │
     ┌──────────────────────────────┼──────────────────────────────┐
     ▼                              ▼                              ▼
 [CHAPTER 4: LOCATIONS]         [CHAPTER 5: ADVERSARIES]       [CHAPTER 7: PLAYBOOK]
  • Room 3/5: Marshal's Key      • Grimnar: Stolen Dagger       • Session 2: Poison Halls
  • Room 7: Scribe Prop Clue     • The Mauler: Iron Carapace    • Session 3: Vault Climax
  • Room 8: Garrison Mail Haul   • Grashnak: Patrol Ambush      • Fighting Withdrawal Rules
  • Room 9: The King's Door      • Udûn Sniffers: Scent tests   • D66 Table Loot Rolls
  • Room 10: Durin's Axe Plinth  • Eye Awareness: +4 on Axe     • Balin's 50 Garrison Reward
```

* **Chapter 4 (Keyed Locations)**:
  * Location 7 houses the skeletal remains of Scribe Frár holding the *Dying Scribe's Letter* prop.
  * Location 8 provides the 40+ suits of Dwarf-mail that trigger the *Heavy Salvage Porter Squad* rules from Chapter 2.
  * Location 9 tests the *Marshal's Key* or *Craft Skill Endeavour* detailed in Section 4.
  * Location 10 houses the plinth bearing *Durin's Axe* and the *Greater Hoard*.
* **Chapter 5 (Adversaries & Hazards)**:
  * Slaying Captain Grashnak in Room 3 or 5 provides the physical *Marshal's Key*.
  * Claiming *Durin's Axe* triggers the +4 Eye Awareness spike, potentially summoning Grimnar’s ambush or triggering a Revelation Episode.
* **Chapter 7 (GM Playbook & Pacing)**:
  * Provides session guidance for distributing D66 scavenge rolls during exploration turns.
  * Details the fighting withdrawal mechanics as the Band lugs the heavy wargear back through the Mustering-Yard to Thrym's Safe Haven.



<!-- PAGE BREAK: Chapter 7 -->

---


# Chapter 7: GM Playbook, Session Pacing & The Fighting Withdrawal

> *"A dwarf may delve a hundred fathoms in search of silver, but it is the road back to the sky that tests his marrow."*  
> — *Proverb of the Iron Hills*

---

## 1. The 3-Act Narrative Architecture

The *Armouries of the Third Deep* is structured as a high-tension, squad-level tactical subterranean operation designed for **2 to 3 tabletop sessions** (each lasting approximately 3.5 to 4 hours). Rather than a static dungeon crawl where adversaries wait passively in disconnected rooms, the delve unfolds as a dynamic military operation governed by player choices, noise discipline, and environmental escalation.

```
========================================================================================================================
                                           3-ACT NARRATIVE ARCHITECTURE
========================================================================================================================

  ACT I: THE LATERAL ADVANCE & FORWARD REDOUBT (Session 1: ~3.5 Hours)
  ┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
  │ • Staging from The Caves of Thrym Thistlebeard (Safe Haven) along the Westward Drift (Level 3A).                 │
  │ • Infiltration of Location 1 (The Mustering-Yard): Stealth reconnaissance, Moria sentries, Grik social parley.   │
  │ • Fortifying Location 2 (The Upper Gatehouse): Rigging the keystone collapse trap and stationing the rearguard.  │
  │ • Breaching Location 3 (The First Armoury): Disarming venom-coated scrap-traps and securing tactical routes.    │
  │ • CLIFFHANGER: Shadowy sightings of Grimnar the Disgraced stalking the lower galleries / Grik's ominous warning. │
  └────────────────────────────────────────────────────────┬─────────────────────────────────────────────────────────┘
                                                           │
                                                           ▼
  ACT II: THE DESPOILED HALLS & TOXIC HAZARDS (Session 2: ~3.5 Hours)
  ┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
  │ • Location 4 (The Broken Hall): Facing the Balrog Idol's Dread aura, deciphering the Old Moria Royal Cartouche. │
  │ • Location 5 (The Second Armoury): Tactical sandbox with siege engines (Grond-ram, torsion ballistas).          │
  │ • Locations 7 & 8 (The Poisoned Halls & Upper Armoury): Braving the Balrog Neurotoxic Miasma with respirators,   │
  │   recovering Scribe Frár's Slate (Handout #1), salvaging 50 suits of garrison wargear for Balin's colony.       │
  │ • Acquiring the Mithril Key: Ambushing Captain Grashnak's patrol or completing Grik's trade.                   │
  │ • CLIFFHANGER: Standing before the iron blast-doors of Location 6 as The Mauler's floor-shaking snores rumble.  │
  └────────────────────────────────────────────────────────┬─────────────────────────────────────────────────────────┘
                                                           │
                                                           ▼
  ACT III: THE APEX DUEL, ROYAL VAULT & WITHDRAWAL (Session 3: ~3.5–4 Hours)
  ┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
  │ • Location 6 (The Hall of the Mauler): Tactical boss confrontation, Riddle duel, catwalk gymnastics, stalactites.│
  │ • Location 9 (The King's Door): Star-iron runic puzzle, cracking the dual locks under Grimnar's parapet ambush. │
  │ • Location 10 (The Lower Armoury): Claiming Durin's Axe (+4 Eye Awareness trigger!) and the Greater Hoard.      │
  │ • THE FIGHTING WITHDRAWAL: Escorting the overburdened Band across Level 3C -> 3B -> 3A while holding chokes.     │
  │ • GATEHOUSE STAND: Triggering the Keystone Collapse at Location 2 to crush Malech's pursuit vanguard.           │
  │ • EPILOGUE: Retreat to Thrym's Haven, East-Gate debrief with Lord Balin, Royal Proof for King Dáin in Erebor.    │
  └──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
========================================================================================================================
```

---

## 2. Session-by-Session Facilitator Playbook

This section provides granular, turn-by-turn facilitator notes for running the module across either a **3-Session Format** (recommended for full narrative immersion and tactical sandbox exploration) or a condensed **2-Session Format** (ideal for convention or fast-paced campaign play).

---

### 2.1 Three-Session Master Schedule

```
========================================================================================================================
                                     3-SESSION RUNNING MASTER MATRIX
========================================================================================================================
 SESSION   ACT     EST. DURATION   LOCATIONS COVERED       CORE OBJECTIVES & KEY CLIMAX
------------------------------------------------------------------------------------------------------------------------
 Session 1 Act I   3.5 Hours       Loc 1, Loc 2, Loc 3     Advance along Westward Drift; neutralize sentries; fortify 
                                                           Gatehouse redoubt; disarm scrap-traps; parley with Grik.
                                                           Climax: Grimnar sighted in the lower flues.
------------------------------------------------------------------------------------------------------------------------
 Session 2 Act II  3.5 Hours       Loc 4, Loc 5, Loc 7, 8  Overcome Balrog Idol Dread; prime siege engines; craft 
                                                           respirators for toxic halls; salvage 50 suits of mail; 
                                                           secure Marshal's Key. Climax: Door to Mauler's lair.
------------------------------------------------------------------------------------------------------------------------
 Session 3 Act III 4.0 Hours       Loc 6, Loc 9, Loc 10,   Defeat/pacify The Mauler; unlock King's Door; survive 
                                   Withdrawal (10 -> 1)    Grimnar's ambush; claim Durin's Axe; execute Fighting 
                                                           Withdrawal; trigger Gatehouse Cave-in; East-Gate Debrief.
========================================================================================================================
```

---

### 2.2 Session 1: The Descent & The Forward Redoubt (Act I)

* **Real-Time Duration**: ~3.5 Hours (210 Minutes)
* **Pacing Arc**: Low-to-Moderate Tension (Alert Tier 0 $\rightarrow$ Alert Tier 1).
* **Primary Theme**: Stealth, tactical preparation, spatial orientation, establishing lines of communication.
* **System Reference**: Hero Attribute TNs (20 - Attribute) | Band TN 15 (20 - Readiness 5)

```
+----------------------------------------------------------------------------------------------------------------------+
| TIME (MIN)  SCENE & LOCATION               GM RUNNING GOALS & CRITICAL DECISIONS                                     |
+----------------------------------------------------------------------------------------------------------------------+
| 000–020     Prologue & Haven Staging       • Set the mood in The Caves of Thrym Thistlebeard. Reiterate Balin's      |
|             (Second Deep Safe Haven)         mandate: salvage arms for 50 Dwarves and find royal proof for King Dáin.|
|                                            • Review Band roster (7 companions), Band Readiness 5 (Band TN 15), roles.|
|                                            • Confirm The Eye of Thrym is inert past the threshold.                   |
+----------------------------------------------------------------------------------------------------------------------+
| 020–050     Lateral Ingress                • Describe the march along the Westward Drift (Great Transverse Gallery), |
|             (The Westward Drift)             transitioning from natural karst to ancient dressed basalt masonry.     |
|                                            • The Guide makes a **TRAVEL roll** or Band **MANOEUVRE roll**            |
|                                              (2d vs **Band TN 15**); a scout ranges ahead on the Forward Screen.     |
|                                            • Arrive at the high overlook portal of Location 1 (The Mustering-Yard).  |
+----------------------------------------------------------------------------------------------------------------------+
| 050–100     The Mustering-Yard             • Boxed read-aloud text. Establish sensory cues: Balrog scorches,        |
|             (Keyed Location 1)               cold drafts, extreme acoustic echo.                                     |
|                                            • A scouting hero makes a **SCAN roll (Favoured)** to spot the            |
|                                              Moria sentry post behind Pillar #4 and Grik in the north drainage slit.  |
|                                            • Social or Stealth challenge: Bypass (**STEALTH roll**) or ambush        |
|                                              (**BATTLE roll**) sentries (1-round kill = 0 Noise). Optional parley    |
|                                              with Grik (**PERSUADE roll** or **RIDDLE roll**).                       |
+----------------------------------------------------------------------------------------------------------------------+
| 100–150     The Upper Gatehouse            • Boxed read-aloud text. Highlight the buckled adamant doors.            |
|             (Keyed Location 2)             • Strategic Decision: The party fortifies the Gatehouse (**CRAFT roll**)  |
|                                              and decides whether to leave 2 companions as a Rearguard.               |
|                                            • Rigging the Keystone Winch Trap (**CRAFT roll** or **BATTLE roll**)     |
|                                              for a controlled 30-damage cave-in during the future escape.            |
+----------------------------------------------------------------------------------------------------------------------+
| 150–195     The First Armoury              • Boxed read-aloud text. Scent of rot and black Orc-venom.               |
|             (Keyed Location 3)             • A hero or companion detects and disarms the scythe tripwires            |
|                                              (**SCAN roll** or **CRAFT roll**, $+1\text{d}$ invoking *Burglary*).   |
|                                            • Harvest 3 doses of Black Orc-Venom; search loose flagstone cache.       |
+----------------------------------------------------------------------------------------------------------------------+
| 195–210     Session 1 Cliffhanger          • A low whistle sounds from the southern ramp. Austri whispers that heavy |
|             (Corridor to Location 4)         hobnailed boots are pacing in the dark. In the gloom below, the gleam   |
|                                              of a stolen Dwarven blade flashes—Grimnar is watching!                  |
+----------------------------------------------------------------------------------------------------------------------+
```

#### Detailed GM Session 1 Running Tips
1. **Emphasize the Sound Economy Early**: Give players physical tokens representing Noise Points. Whenever a character speaks loudly, drops gear, or prolongs combat, hand them a token. Showing the Alert Tracker gauge moving from Alert 0 toward Alert 1 creates visceral tension without rolling dice.
2. **Reward Companion Deployment**: If the players assign Austri and Bróga to the *Forward Scout Screen*, explicitly tell them: *"Austri spots the tripwire before your boots touch the wire."* This builds immediate player trust in the Band system.
3. **The Gatehouse Choice**: Strongly encourage the Company's leader to deliberate over stationing a Rearguard at Location 2. Remind them of the Moria rule: unheld corridors in Khazad-dûm do not stay empty.

---

### 2.3 Session 2: The Despoiled Halls & Toxic Hazards (Act II)

* **Real-Time Duration**: ~3.5 Hours (210 Minutes)
* **Pacing Arc**: Rising Tension & Environmental Peril (Alert Tier 1 $\rightarrow$ Alert Tier 2).
* **Primary Theme**: Psychological horror, Dwarven sorrow, engineering problem-solving, logistics.

```
+----------------------------------------------------------------------------------------------------------------------+
| TIME (MIN)  SCENE & LOCATION               GM RUNNING GOALS & CRITICAL DECISIONS                                     |
+----------------------------------------------------------------------------------------------------------------------+
| 000–015     Recap & Awakening              • Quick recap of Session 1. Remind players of their fortified Gatehouse   |
|             (Location 3 Threshold)           and the lurking shadow of Grimnar. Review Hope and Fatigue.             |
+----------------------------------------------------------------------------------------------------------------------+
| 015–060     The Broken Hall                • Boxed read-aloud text. Present the 12-foot jagged Balrog Idol.         |
|             (Keyed Location 4)             • All characters make Dread Tests (**VALOUR roll**).                      |
|                                              - Heroes with flaws such as Vengeance risk wrathful impulses on fail.   |
|                                              - Heroes with flaws such as Greed covet gold veins in the idol base.    |
|                                            • Deciphering the Secret Cartouche (**LORE roll** or **RIDDLE roll**):    |
|                                              Discovers the dual King's Key & Marshal's Key requirement.             |
|                                            • Optional: Topple the idol (**ATHLETICS roll**; muffle with canvas).     |
+----------------------------------------------------------------------------------------------------------------------+
| 060–105     The Second Armoury             • Boxed read-aloud text. Smells of dry cedar and linseed oil.            |
|             (Keyed Location 5)             • Explore the siege sandbox: Grond-ram, torsion ballistas, crane hoist.   |
|                                            • A hero or companion tests **CRAFT roll** to prime the ballista.         |
|                                            • Intercepting Captain Grashnak's Udûn patrol (Pathway 1 for the Key) or  |
|                                              preparing an artillery ambush.                                          |
+----------------------------------------------------------------------------------------------------------------------+
| 105–150     The Poisoned Halls             • Boxed read-aloud text. Unveil the emerald Balrog miasma.               |
|             (Keyed Location 7)             • Crafting squad respirators (**CRAFT roll**) using leather and charcoal. |
|                                            • Discover the petrified Dwarven captains and Scribe Frár at his desk.    |
|                                            • PRESENT HANDOUT #1: The Dying Scribe's Slate. Let players read the prop!|
+----------------------------------------------------------------------------------------------------------------------+
| 150–195     The Upper Armoury              • Boxed read-aloud text. Unseal pristine Dwarven storehouse.             |
|             (Keyed Location 8)             • Salvage Operation: Packing 40 mail-shirts, 30 shields, 50 mattocks.     |
|                                            • Shift Band status to Heavy Burden (-1d Manoeuvre, -1d Fatigue).         |
|                                            • Claim 6 flasks of Dwarven Liquid Fire from lead munitions chest.        |
+----------------------------------------------------------------------------------------------------------------------+
| 195–210     Session 2 Cliffhanger          • The Company returns to Location 5 and approaches the massive bronze     |
|             (Threshold of Location 6)        doors of Location 6. Through the iron seams pulses a rhythmic, guttural |
|                                              snore that rattles dust from the ceiling—The Mauler is right behind it! |
+----------------------------------------------------------------------------------------------------------------------+
```

#### Detailed GM Session 2 Running Tips
1. **Play Up the Emotional Weight of Location 7**: Scribe Frár's letter is the emotional anchor of the module. Take time to describe the calcified Dwarves standing in silent vigil. When handing the physical Handout #1 to the players, dim the room lighting or read it in a solemn, archaic voice.
2. **Manage the Burden Shift**: When the Band packs the wargear in Location 8, explicitly inform the players that they are now carrying the physical future of Balin's colony. Every step back will be slower and noisier, raising the tactical stakes for Act III.
3. **Sandbox Versatility in Location 5**: Let players experiment with the siege engines. If they spend time winching the ballista or aiming the Grond-ram south toward Location 6, ensure it pays off dynamically in Session 3.

---

### 2.4 Session 3: The Apex Duel, Royal Vault & The Fighting Withdrawal (Act III)

* **Real-Time Duration**: ~4.0 Hours (240 Minutes)
* **Pacing Arc**: Climactic Action & High-Tension Escape (Alert Tier 2 $\rightarrow$ Alert Tier 3 / General Alarm).
* **Primary Theme**: Heroic combat, ancient majesty, disciplined squad extraction, sacrifice.

```
+----------------------------------------------------------------------------------------------------------------------+
| TIME (MIN)  SCENE & LOCATION               GM RUNNING GOALS & CRITICAL DECISIONS                                     |
+----------------------------------------------------------------------------------------------------------------------+
| 000–015     Preparation & Stance Briefing  • Quick recap of Session 2. Players confirm their squad marching order,   |
|             (Location 5 Staging)             equipped liquid fire, and tactical plan for The Mauler.                 |
+----------------------------------------------------------------------------------------------------------------------+
| 015–075     The Hall of the Mauler         • Boxed read-aloud text. The Armoured Cave-Troll (AL 10, End 80, Might 2).|
|             (Keyed Location 6)             • Dynamic Arena Tactics: Scaling catwalks (High Ground), dropping 20-Dmg  |
|                                              stalactites, called shots to sever scrap-armor copper wires.            |
|                                            • The Riddle Duel: Hero in Forward Stance exploits Dull-Witted trait via  |
|                                              **RIDDLE roll**; 3 successes pacify or panic the troll!                 |
|                                            • Companions use Shield-Wall to intercept crushing maul strikes.          |
+----------------------------------------------------------------------------------------------------------------------+
| 075–125     The King's Door & Ambush       • Boxed read-aloud text. Ithildin runes pulse with blue starlight.        |
|             (Keyed Location 9)             • Unlocking the portal: Marshal's Key (1 round) vs Lockbreaker Skill      |
|                                              Endeavour (Resistance 6, Time Limit 3 Turns: SCAN, CRAFT, or RIDDLE).   |
|                                            • Grimnar's Ambush: Grimnar and 4 Udûn stalkers attack from the parapet!  |
|                                            • Single Combat vs Grimnar: A champion may challenge Grimnar to negate    |
|                                              Might penalties.                                                        |
|                                            • Companion overwatch suppresses javelin-throwers. Slay or rout Grimnar!  |
+----------------------------------------------------------------------------------------------------------------------+
| 125–155     The Royal Vault (Location 10)  • Boxed read-aloud text. Pure starlight, mithril filigree.                |
|             & Durin's Axe Claiming         • Bolting the Vault Barricade: 30-minute rest, restore Hope.              |
|                                            • Claiming Durin's Axe from black marble dais:                            |
|                                              - +4 STRATEGIC EYE AWARENESS TRIGGER!                                   |
|                                              - Alert Tracker surges to ALERT TIER 3 (Drums in the Deep!).            |
|                                            • Claiming the Three Relic Coffers & Mithril Ingots.                      |
+----------------------------------------------------------------------------------------------------------------------+
| 155–215     THE FIGHTING WITHDRAWAL        • The escape begins! Traverse backwards: Loc 9 -> 6 -> 5 -> 4 -> 3 -> 2.  |
|             (Level 3C -> 3B -> 3A)         • Full Fighting Withdrawal Rules (Section 5):                             |
|                                              - Porter squad hauls 50 suits of mail under heavy pursuit.              |
|                                              - Band Clashes against pursuing Orc warbands (Band WAR vs Band TN 15).  |
|                                              - Desperate Stand options for heroic companions.                        |
|                                            • GATEHOUSE STAND (Location 2): Pulling the keystone winch to collapse    |
|                                              the archway (30 Dmg), crushing Malech's pursuit vanguard!               |
+----------------------------------------------------------------------------------------------------------------------+
| 215–240     Transverse Retreat & Epilogue  • Hauling wargear and wounded along the Westward Drift to Thistlebeard    |
|             (Safe Haven & East-Gate Camp)    Caves Safe Haven.                                                       |
|                                            • Grand Council with Lord Balin and Commander Fróra at the East-Gate.     |
|                                            • Distribute XP, AP, SP, and calculate garrison armament rewards.         |
|                                            • Emissary dispatched to King Dáin Ironfoot in Erebor!                    |
+----------------------------------------------------------------------------------------------------------------------+
```

---

### 2.5 Two-Session Condensed Schedule (Fast-Paced / Convention)

If running the adventure across **2 sessions** (4 hours each):
* **Session 1 (Infiltration & Arsenals, 4.0 Hrs)**: Ingress $\rightarrow$ Locations 1, 2, 3 $\rightarrow$ Location 4 (Idol) $\rightarrow$ Location 5 (Siege Engines) $\rightarrow$ Location 7 & 8 (Gas & Salvage). *Cliffhanger*: Unlocking the double doors into The Mauler's Hall.
* **Session 2 (Boss, Heist & Extraction, 4.0 Hrs)**: Location 6 (The Mauler) $\rightarrow$ Location 9 (King's Door & Grimnar Ambush) $\rightarrow$ Location 10 (Durin's Axe) $\rightarrow$ Full Fighting Withdrawal across all levels $\rightarrow$ Keystone Collapse & Debrief.

---

## 3. Character Spotlights & Flaw Management

Each Player-Hero possesses unique motivations, signature masteries, and psychological Shadow vulnerabilities that should be highlighted during specific encounters:

```
========================================================================================================================
                                     PLAYER-HERO SPOTLIGHT & SHADOW MATRIX
========================================================================================================================
 HERO       SIGNATURE UTILITY            KEY SPOTLIGHT SCENES             SHADOW FLAW & VULNERABILITY TRIGGERS
------------------------------------------------------------------------------------------------------------------------
 Champion   Great Axe Mastery;           • Smashes Balrog Idol in Room 4  • Curse of Vengeance: Triggered by Orc
 (Frontline Frontline Breacher;          • Catwalk duel with The Mauler     taunts, defaced king murals, or seeing
 Vanguard)  Redoubtable Cultural Virtue  • Single combat vs Grimnar (Rm 9)  allies wounded. Must make a VALOUR roll
                                                                            or succumb to wrathful impulses!
------------------------------------------------------------------------------------------------------------------------
 Treasure   Masterwork Lockpicks / Tools • Disarms scrap-traps in Room 3  • Dragon-sickness: Triggered by gold-leaf
 Hunter     Trait *Burglary*;            • Deciphers Scribe Slate (Rm 7)    in idol (Rm 4) and Greater Hoard (Rm 10).
 (Scout)    Keen SCAN & STEALTH          • Cracks King's Door locks (Rm 9)  Must make a VALOUR roll or hoard
                                                                            recklessly.
------------------------------------------------------------------------------------------------------------------------
 Scholar /  Battle-horn (+1d BATTLE);    • Coordinates Gatehouse Redoubt  • Lure of Power: Obsessed with ancient
 Captain    Trait *Leadership*;          • Directs siege engines (Room 5)   ciphers and the Dimrill Stair Key. Must
 (Leader)   Inspiring ENHEARTEN          • Commands Fighting Withdrawal     make a COURTESY roll or VALOUR roll or
                                                                            refuse tactical retreat!
========================================================================================================================
```

### 3.1 Managing the Companion Band's Personal Arcs
* **Bláin the Cleaver**: Sullen, scarred survivor. Yearns to avenge his Bridge wound against Grimnar. If a champion duels Grimnar, Bláin guards their flank against interfering sniffers with ferocious loyalty.
* **Fáin Iron-Visor**: Refuses to remove his enclosed iron helm. His keen eyesight in darkness makes him the ultimate counter-sniper against Grimnar's parapet javelin-throwers.
* **Dúrmer the Hardened**: The moral and physical rock. Whenever the Band's resolve wavers during the withdrawal, Dúrmer takes a swig from his iron flask and shoulders the heaviest crate of gromril-mail without a word.
* **Hjoldring the Armourer**: Deeply emotional when inspecting pristine First Age craft in Locations 8 and 10. He treats each suit of mail as a living Dwarven ancestor.
* **Bróga & Austri**: The infiltration duo. They move like smoke along the cornices. If a hero is working the locks on the King's Door, Bróga provides the vital tension wrench via his *Vaultbreaker* Trait.
* **Dolg the Bulwark**: The selfless protector. When The Mauler swings its tree-trunk club or Grimnar lunges at an ally, Dolg uses his *Shield-Bearer* reaction to take the blow on his notched tower-shield.

---

## 4. Pacing Dials & Emergency GM Interventions

Subterranean adventures can occasionally run ahead of schedule (if players bypass hazards with lucky rolls) or bog down in analysis paralysis. Use the following "Pacing Rescue Dials" to regulate table tempo:

```
========================================================================================================================
                                           PACING RESCUE DIALS
========================================================================================================================

  DIAL 1: RUNNING TOO FAST / LOW TENSION (Party breezing through stealth)
  ┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
  │ • DISPATCH MORIA SNIFFER PATROL: Two sniffers with iron-shod staves enter the room, tracking scent.              │
  │ • ACOUSTIC ECHO: A sudden tremor in Mount Silvertine causes loose stones to tumble, adding +2 Noise Points.      │
  │ • GRIMNAR'S TAUNT: Grimnar hurls a black dagger into the lintel above them and vanishes into the vents.         │
  └──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘

  DIAL 2: BOGGED DOWN / ANALYSIS PARALYSIS (Players over-planning or stuck)
  ┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
  │ • GRIK INTERVENES: Grik the Skulker pokes his head from a drain, offering a direct tip for 1 silver coin.       │
  │ • DÚRMER'S COMMON SENSE: Dúrmer points out an obvious structural wedge or lever with an exasperated grunt.      │
  │ • DISTANT DRUMS: A single hollow drum beat (*thum... thum*) echoes in the distance, establishing urgency.       │
  └──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘

  DIAL 3: NEAR-TPK / CRITICAL COMBAT CRISIS (Party overwhelmed in combat)
  ┌──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐
  │ • TRIGGER DESPERATE STAND: An uninjured companion (e.g. Dolg or Dúrmer) steps in front of the lethal blow!       │
  │ • SIEGE COLLAPSE: An errant strike hits a cracked pillar, dropping rubble between the party and the horde.       │
  │ • TROLL DISTRACTION: The Mauler becomes confused by echoing gunfire/horns and attacks Orc reinforcements instead. │
  └──────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘
========================================================================================================================
```

---

## 5. The Fighting Withdrawal Subsystem

The climax of the adventure is not merely slaying an adversary, but the **disciplined extraction of 50 suits of heavy garrison wargear, royal relics, and wounded companions** back through enemy-infested territory while the full garrison mobilizes.

```
========================================================================================================================
                                      FIGHTING WITHDRAWAL FLOWCHART
========================================================================================================================

  [PHASE 1: THE ALARM SOUNDS] ──► [PHASE 2: LEVEL 3C EXTRACTION] ──► [PHASE 3: LEVEL 3B CHOKE POINTS]
   • Lifting Durin's Axe           • Exit Room 10 -> Room 9          • Traverse Room 5 -> Room 4 -> Room 3
   • +4 Strategic Eye Awareness    • Rearguard holds Avenue          • Deploy caltrops / barricades
   • Alert 3: Drums in Deep!       • Porters haul wargear            • Band Clash vs Orc Pursuit Packs
                                                                                    │
                                                                                    ▼
  [PHASE 6: HAVEN REUNION]    ◄── [PHASE 5: TRANSVERSE RETREAT] ◄── [PHASE 4: GATEHOUSE STAND (ROOM 2)]
   • Secure in Thrym's Haven       • Phalanx covers Westward Drift   • Hold the adamant blast-doors
   • First Aid for Wounded         • Porters wheel wargear sledges   • TRIGGER KEYSTONE CAVE-IN TRAP!
   • Return to East-Gate Camp      • Haven gate slammed and barred   • 30 Crushing Dmg seals the chasm!
========================================================================================================================
```

---

### 5.1 Step-by-Step Fighting Withdrawal Rules

1. **Triggering the General Alarm**:
   * The moment *Durin's Axe* is lifted from the Dais in Location 10, the mountain resonates with a deep harmonic hum. The Company's **Strategic Eye Awareness increases by +4**, immediately triggering **Alert Tier 3 (*Drums in the Deep*)**.
   * The great kettle-drums of Malech's Fortress begin their thunderous cadence (*Doom, doom, roll-and-go*).
2. **The Heavy Salvage Burden**:
   * Hauling the 50 suits of mail, 30 shields, crates of weapons, and the 3 Relic Coffers requires **4 Companion Dwarves** operating as the *Heavy Salvage Porter Squad* (Hjoldring & Dúrmer leading).
   * The Band operates under **Heavy Burden**:
     * All **Band Manoeuvre** rolls suffer **-1d**.
     * Marching Fatigue tests suffer **-1d**.
     * Moving between keyed locations generates **+1 Noise Point** unless muffled.
3. **Room-by-Room Backward Traversal & Clashes**:
   * The Company must traverse the rooms in reverse sequence: **Location 10 $\rightarrow$ Location 9 $\rightarrow$ Location 6/5 $\rightarrow$ Location 4 $\rightarrow$ Location 3 $\rightarrow$ Location 2 $\rightarrow$ Location 1**.
   * In each major sector, the GM resolves one **Band Clash** or **Tactical Choke Encounter**:
     * **Sector 1 (Locations 9 & 6)**: An advance squad of 6 Orc Guards attempts to cut off the colonnade. The Company commands the Band in *Guarded Stance* while the vanguard clears the path.
     * **Sector 2 (Locations 5 & 4)**: A pack of 8 Udûn Sniffers with torch-staves pours from side flues. If the party pre-assembled the cedar barricades or rigged the Grond-ram, they bypass this clash automatically!
     * **Sector 3 (Location 3)**: Orc scrap-traps previously re-armed by the Company catch the pursuing vanguard, delaying them by 2 rounds.
4. **The Gatehouse Stand (Keyed Location 2)**:
   * When the Company reaches the Upper Gatehouse, the pursuing main host (a warband of 20+ Black Uruks and Orc Guards) is less than 50 paces behind.
   * **If a Rearguard was stationed at Location 2** (e.g. Bláin & Fáin):
     * The rearguard covers the porters as they pass through the adamant doors.
     * The Company gains an automatic **+2 bonus to Band Readiness** (reducing Band TN from 15 to 13).
   * **Triggering the Keystone Collapse Trap**:
     * As the lead ranks of Black Uruks surge beneath the gatehouse archway, a hero or companion pulls the quick-release winch lever (**CRAFT roll** or **ATHLETICS roll**).
     * The three-ton granite keystone crashes down, unleashing a catastrophic **Controlled Cave-In**!
     * **Mechanical Effect**: Deals **30 Crushing Damage** to the pursuing vanguard and buries the southern rampway under ten tons of solid granite rubble, completely severing the pursuit!
5. **The Transverse Withdrawal (Location 1 $\rightarrow$ Safe Haven)**:
   * Arriving at the Mustering-Yard, the porters secure the heavy salvage on iron-wheeled sledges and pack-litters.
   * Dúrmer and the vanguard anchor the rearguard shield-wall across the Mustering-Yard portals while archers form a perimeter overwatch with bows.
   * The Company falls back steadily along the paved avenue of the *Westward Drift* (Great Transverse Gallery), entering the fortified gates of *The Caves of Thrym Thistlebeard* and barring the massive iron portals as the distant war-drums rumble harmlessly against the sealed stone defenses.

---

## 6. Campaign Debrief, Fellowship Phase & Royal Recognition

Successfully returning from the Third Deep represents a monumental turning point in the history of Balin's colony.

```
========================================================================================================================
                                     CAMPAIGN REWARDS & RECOGNITION SUMMARY
========================================================================================================================
 REWARD CATEGORY           MECHANICAL BENEFIT                      NARRATIVE & CAMPAIGN IMPACT
------------------------------------------------------------------------------------------------------------------------
 1. Garrison Wargear Haul  Equips 50 Frontline Dwarves             Equips 50 frontline Dwarves in Balin's vanguard;
    (40 Mail, 30 Shields)                                          secures the First and Second Halls permanently.
------------------------------------------------------------------------------------------------------------------------
 2. Royal Proof for Erebor Convincing Proof for King Dáin         Emissary dispatched to the Lonely Mountain; Dáin 
    (Durin's Axe & Mithril)                                        mobilizes 500 veteran Dwarf reinforcements (2990 TA).
------------------------------------------------------------------------------------------------------------------------
 3. Experience & Renown    +4 Adventure Points (AP)                Heroes declared *Wardens of the Third Deep*; gain 
                           +3 Skill Points (SP)                    +2 Fellowship Score and high renown among Durin's Folk.
------------------------------------------------------------------------------------------------------------------------
 4. Relic Attunements      Durin's Axe, Deep Gate Shield,          Heroes may permanently attune their recovered wondrous
                           Moria-Silver Mattock, Stone Mail        relics during the subsequent Fellowship Phase.
------------------------------------------------------------------------------------------------------------------------
 5. Safe Haven Elevation   Thrym's Haven Elevated to Sanctuary     The Caves of Thrym become a permanent underground town,
                           (Free Prolonged Rests & Triage)         allowing major Craft Endeavours and weapon forging.
========================================================================================================================
```

---

### 6.1 The High Council at the East-Gate
Upon emerging into the fresh mountain air of the East-Gate Camp, the Company is received by **Lord Balin** and **Commander Fróra** amidst tears and triumphant horn-blasts:
* **Lord Balin's Speech**: Holding *Durin's Axe* aloft before the assembled colony, Balin proclaims:
  > *"Look upon the handiwork of the First King! While the steel of Durin remains in the hands of his sons, the shadow shall never claim the Mountain! To the brave heroes of the Company and their seven companions, the House of Durin owes a debt that all the gold of Erebor cannot repay!"*
* **Commander Fróra's Logistics Report**: Fróra immediately puts Hjoldring and the armourers to work distributing the 40 suits of gromril-mail and 30 shields. Balin's frontline soldiers are transformed from a ragtag militia into an iron-clad phalanx capable of holding the Deeps against any assault.

---

### 6.2 Fellowship Phase Undertakings
During the subsequent Fellowship Phase at *The Caves of Thrym Thistlebeard* or the *East-Gate Camp*, the heroes may choose from the following specialized Undertakings:

1. **Attune Ancient Relic**: A hero who claimed one of the wondrous relics (*Durin's Axe*, *Shield of the Deep Gate*, *Mattock of Moria-Silver*, or *Mail of Unyielding Stone*) spends the phase communing with the weapon's history, cementing its virtues into their character sheet.
2. **Refine Moria-Silver (Craft Undertaking)**: Working with Hjoldring at the newly rekindled forges of Thrym's Haven, a hero uses the recovered Mithril ingots to upgrade a standard weapon or armor piece with a permanent Craft Quality (*Keen*, *Grievous*, or *Close Fitting*).
3. **Study Scribe Frár's Schematics (Lore Undertaking)**: A hero studies the architectural maps found in the lead scroll tube, gaining **Favoured status on all EXPLORE rolls** in future delves into the *Fifth Deep* and *The Endless Stair*.
4. **Heal the Wounded Veterans**: Tend to any companion Dwarves who suffered *Severe* or *Lingering Injuries*, restoring the Band roster to full readiness before the next campaign milestone.

---

### 6.3 Future Campaign Hooks
* **The Emissary to Erebor**: The heroes are chosen to escort the courier bearing *Durin's Axe* (or a royal coffer of Mithril) across Wilderland to King Dáin Ironfoot in the Lonely Mountain.
* **The Wrath of Malech**: Enraged by the loss of the Third Deep armouries, Great Orc Lord Malech orders the mobilization of his fire-touched hordes from the *Palace of Stairs*.
* **The Shadow of Bildor**: The traitorous Dwarf *Bildor* is reported leading an Orc raiding party toward the *Dimrill Stair*—setting the stage for the next high-stakes adventure module!



<!-- PAGE BREAK: APPENDICES -->

---

# APPENDICES: TABLETOP PLAY AIDS & HANDOUTS




<!-- PAGE BREAK: Appendix A -->

---

## Appendix A: Operational Node Map & Schematic


# Node Map, Tactical Floorplans & Spatial Topology
### *The Armouries of the Third Deep* — Spatial Navigation Guide

---

## 1. Master Three-Tier Elevation Cross-Section

```
========================================================================================================================
                                     THREE-TIER ELEVATION CROSS-SECTION
========================================================================================================================

  [SURFACE / UPPER LEVELS]
  • The Great Gates of the Dimrill Dale (East-Gate Encampment & Lord Balin)
     │
     ▼ (Wild Land Travel — **TRAVEL roll**)
  • THE CAVES OF THRYM THISTLEBEARD (Safe Haven - Location 18)
     │
     └──► [The Westward Drift / Great Transverse Gallery: Lateral Paved Ingress]
           │
           ▼
  ====================================================================================== [LEVEL 3A: UPPER STAGING TIER]
  ┌────────────────────────────────────────────────────────┐
  │ 1. THE MUSTERING-YARD                                  │ (Crossroads Landing, Balrog Scorches, Sentry Camp)
  └───────────────────────────┬────────────────────────────┘
                              │ (30-ft Granite Archway)
                              ▼
  ┌────────────────────────────────────────────────────────┐
  │ 2. THE UPPER GATEHOUSE                                 │ (Adamant Blast-Doors, Fallback Redoubt, Keystone Trap)
  └───────────────────────────┬────────────────────────────┘
                              │
                              │ (Sloping 20-Foot Granite Ramp Descent)
                              ▼
  ====================================================================================== [LEVEL 3B: MIDDLE ARSENAL TIER]
  ┌────────────────────────────────────────────────────────┐
  │ 3. THE FIRST ARMOURY                                   │ (Despoiled Weapon Racks, Orc Scrap-Traps, Black Venom)
  └───────────────────────────┬────────────────────────────┘
                              │ (Pillared Archway)
                              ▼
  ┌────────────────────────────────────────────────────────┐
  │ 4. THE BROKEN HALL                                     │ (Desecrated Museum, 12-ft Balrog Idol, Royal Cartouche)
  └───────────────────────────┬────────────────────────────┘
                              │ (Bronze-Bound Cedar Double Doors)
                              ▼
  ┌────────────────────────────────────────────────────────┐
  │ 5. THE SECOND ARMOURY (Siege Workshop)                 │ (Grond-Ram, Torsion Ballistas, Counterweight Cranes)
  └─────────────┬────────────────────────────┬─────────────┘
                │                            │
  (West Ramp)   │                            │ (South Ramp)
  ┌─────────────┴────────────┐               │
  │ [LEVEL 3B-minus: BASIN]  │               │
  │ 7. THE POISONED HALLS    │               │
  │    • Toxic Balrog Miasma │               │
  │    • Scribe Frár's Slate │               │
  └─────────────┬────────────┘               │
                │ (Bronze Pressure Door)     │
                ▼                            │
  ┌──────────────────────────┐               │
  │ 8. THE UPPER ARMOURY     │               │
  │    • Garrison Salvage    │               │
  │      (50 Suits of Mail)  │               │
  │    • Liquid Fire Chest   │               │
  └─────────────┬────────────┘               │
                │ (Reinforced Flue)          │
                └─────────────┬──────────────┘
                              │
                              ▼
  ====================================================================================== [LEVEL 3C: DEEP SANCTUARY TIER]
  ┌────────────────────────────────────────────────────────┐
  │ 6. THE HALL OF THE MAULER (Drill Arena)                │ (Lair of the Armoured Cave-Troll AL 10, Catwalks)
  └───────────────────────────┬────────────────────────────┘
                              │ (Grand Colonnade Avenue)
                              ▼
  ┌────────────────────────────────────────────────────────┐
  │ 9. THE KING'S DOOR                                     │ (Star-Iron Portal, Ithildin Runes, Grimnar Ambush Parapet)
  └───────────────────────────┬────────────────────────────┘
                              │ (Dual Runic Master Locks / Craft Bypass)
                              ▼
  ┌────────────────────────────────────────────────────────┐
  │ 10. THE LOWER ARMOURY (The Royal Vault)                │ (Dais of Durin, DURIN'S AXE, 3 Relic Coffers, Mithril)
  └────────────────────────────────────────────────────────┘
========================================================================================================================
```

---

## 2. Spatial Connection Matrix & Doorway Types

```
========================================================================================================================
                                      SPATIAL CONNECTION & DOORWAY MATRIX
========================================================================================================================
 FROM ROOM             TO ROOM               PASSAGE / DOOR TYPE             LOCK / TRAP / MECHANISM
------------------------------------------------------------------------------------------------------------------------
 Safe Haven (Thrym)    1. Mustering-Yard     Paved Westward Drift (Gallery)  Ancient transverse military road
 1. Mustering-Yard     2. Upper Gatehouse    30-ft Open Granite Archway      None (Open transit)
 1. Mustering-Yard     3. First Armoury      East Smuggler's Crawl-Vent      Concealed stone grating (**EXPLORE roll**)
 2. Upper Gatehouse    3. First Armoury      Sloping 20-ft Granite Ramp      Buckled Adamant Blast-Doors; Keystone Winch
 3. First Armoury      4. Broken Hall        Open Pillared Archway           2 Interlocking Scythe Scrap-Traps (**SCAN roll**)
 3. First Armoury      5. Second Armoury     West Maintenance Duct           Narrow duct (**EXPLORE roll**; bypasses Room 4)
 4. Broken Hall        5. Second Armoury     Heavy Double Cedar Doors        Bronze-bound; barred from inside (**ATHLETICS roll**)
 5. Second Armoury     6. Hall of Mauler     Heavy Bronze Double Doors       Swings outward; target for Grond-ram
 5. Second Armoury     7. Poisoned Halls     Sealed Iron Pressure-Door       Wheel-latch with airtight lead gasket
 7. Poisoned Halls     8. Upper Armoury      Reinforced Bronze Door          Greased pivots; airtight seal
 8. Upper Armoury      9. King's Door        Reinforced Stone Flue           Straight corridor; connects Level 3B to 3C
 6. Hall of Mauler     9. King's Door        Grand Colonnade Avenue          30-ft wide pillared thoroughfare
  9. King's Door        10. Lower Armoury     The King's Door (Star-Iron)     Dual Locks (Mithril & King's Keys + King's Invocation)
 10. Lower Armoury     10. (Inner Redoubt)   Vault Blast Barricade           Adamant portcullis bolted from inside
========================================================================================================================
```

---

## 3. Tactical Room Floorplans

---

### 3.1 Location 1: The Mustering-Yard (Level 3A)

```
+--------------------------------------------------------------------------------------+
| [Descent Shaft from Safe Haven] (High Overlook Gallery - 20 ft above floor)          |
|  ══════════════════════════════════════════════════════════════════════════════════  |
|  [Pillar 1]               [Pillar 2]               [Pillar 3]             [Pillar 4] |
|                                                                           (Moria Tent)|
|                    ┌────────────────────────────────────┐                            |
|                    │       THE SCORCHED PAVILION        │                            |
|                    │  • Raised Granite Dais (4 ft high) │                            |
|                    │  • Vitrified Balrog Glass Mark     │                            |
|                    │  • Broken Command Plinth (+1 Cover)│                            |
|                    └────────────────────────────────────┘                            |
|                                                                                      |
|  [Pillar 5]               [Pillar 6]               [Pillar 7]             [Pillar 8] |
|                                                                           (Sentry Camp|
|                                                                            Brazier)   |
|  ══════════════════════════════════════════════════════════════════════════════════  |
|                                         │                                            |
|                                         ▼                                            |
|                          [To Location 2: Upper Gatehouse]                            |
+--------------------------------------------------------------------------------------+
```

---

### 3.2 Location 2: The Upper Gatehouse (Level 3A Redoubt)

```
+--------------------------------------------------------------------------------------+
|                           [From Location 1: Mustering-Yard]                          |
|                                         │                                            |
|                                         ▼                                            |
|  ┌───────────────────┐        OUTER GATEWAY THRESHOLD        ┌────────────────────┐  |
|  │ LEFT GUARD TURRET │                                       │ RIGHT GUARD TURRET │  |
|  │ • Arrow Slits     │     [BUCKLED ADAMANT BLAST-DOORS]     │ • Winch Room       │  |
|  │ • Murder-Holes    │       (Warped outward at 45 deg)      │ • KEYSTONE LEVER   │  |
|  └─────────┬─────────┘                                       └─────────┬──────────┘  |
|            │                  ┌────────────────────────┐               │             |
|            │                  │  FORTIFIED REDOUBT     │               │             |
|            │                  │  • Cedar balks & iron  │               │             |
|            │                  │  • Safe Fallback Point │               │             |
|            │                  └────────────────────────┘               │             |
|            │                                                           │             |
|            └────────────► ══ [KEYSTONE WINCH CAVE-IN] ══ ◄─────────────┘             |
|                           (Overhead 3-Ton Granite Block)                             |
|                                         │                                            |
|                                         ▼ (Sloping 20-ft Rampway)                    |
|                           [To Location 3: The First Armoury]                         |
+--------------------------------------------------------------------------------------+
```

---

### 3.3 Location 5: The Second Armoury / Siege Workshop (Level 3B)

```
+--------------------------------------------------------------------------------------+
|                           [From Location 4: The Broken Hall]                         |
|                                         │                                            |
|                                         ▼ (Double Cedar Doors)                       |
|  [SPARE TORSION SPRINGS & WORKBENCHES]           [TIMBER CRANE & 2-TON HOIST BLOCK]  |
|                                                                                      |
|  ┌─────────────────────────────────┐           ┌──────────────────────────────────┐  |
|  │ THE GROND-RAM OF THE THIRD DEEP │           │ TWIN-LIMB TORSION BALLISTA       │  |
|  │ • 20-ft Iron-Headed Ram Chassis │           │ • 6-ft Steel Harpoon Bolt        │  |
|  │ • Mounted on greased tracks     │           │ • 360-degree Turntable           │  |
|  └────────────────┬────────────────┘           └──────────────────────────────────┘  |
|                   │ (Aimed South)                                                    |
|                   ▼                                                                  |
|  [WEST PRESSURE-DOOR] ───────────────────────► [SOUTHERN HEAVY BRONZE DOORS]         |
|  (Airtight Wheel-Latch to Room 7)               (Blast-Doors opening into Room 6)    |
+--------------------------------------------------------------------------------------+
```

---

### 3.4 Location 6: The Hall of the Mauler (Level 3C Arena)

```
+--------------------------------------------------------------------------------------+
|                           [From Location 5: The Second Armoury]                      |
|                                         │                                            |
|                                         ▼ (Heavy Bronze Doors)                       |
|  ════════════════════════════ [HIGH IRON CATWALKS] ════════════════════════════════  |
|  (20-ft Elevation; Chain Ladders; High Ground +1d; Sweepable by Maul)                |
|                                                                                      |
|             [STALACTITE 1]                                 [STALACTITE 2]            |
|          (Drop: 20 Direct Dmg)                         (Drop: 20 Direct Dmg)         |
|                                                                                      |
|                         ┌─────────────────────────────┐                              |
|                         │   THE MAULER'S SCRAP-NEST   │ <── The Mauler (AL 10)       |
|                         │   • 10-ft Mountain of Scrap │     Sleeping / Alert         |
|                         │   • Anvils, Shields, Bones  │     (Riddle Duel Stance)     |
|                         └─────────────────────────────┘                              |
|                                                                                      |
|  [IMPROVISED WEAPON HEAPS]                                [EAST COLLAPSED SLAG CHUTE]|
|  (Hurled for 6-Dmg AoE)                                   (Hazardous Chasm Edge)     |
|                                         │                                            |
|                                         ▼ (Grand Colonnade Avenue)                   |
|                            [To Location 9: The King's Door]                          |
+--------------------------------------------------------------------------------------+
```

---

### 3.5 Locations 7 & 8: The Poisoned Halls & Upper Armoury (Level 3B-minus)

```
+--------------------------------------------------------------------------------------+
|                           [From Location 5: The Second Armoury]                      |
|                                         │                                            |
|                                         ▼ (Sealed Iron Pressure-Door)                |
|  ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ BALROG NEUROTOXIC MIASMA POOL ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈ ≈   |
|  (5-ft Emerald-Yellow Fog; Grievous Exposure: 1 check/min; Protected: 1 check/hr)   |
|                                                                                      |
|  [PETRIFIED DWARVEN KNIGHTS]                     [STONE COUNCIL TABLE]               |
|  (Calcified Warriors in Stasis)                  • Slumped Form of Scribe Frár       |
|                                                  • LEAD SCROLL TUBE: Scribe's Slate  |
|                                                                                      |
|  [JAMMED CEILING VENTILATION FLUE]                                                   |
|  (Overhead Iron Damper Lever - Vents room in 2 rounds; +3 Noise Points)              |
|                                         │                                            |
|                                         ▼ (Reinforced Bronze Door)                   |
|  ┌────────────────────────────────────────────────────────────────────────────────┐  |
|  │ LOCATION 8: THE UPPER ARMOURY (Garrison Salvage Cache)                         │  |
|  │ • 40 Suits of Dwarf Mail-Shirts  • 30 Heavy Tower-Shields  • 50 War-Mattocks   │  |
|  │ • Lead-Sealed Munitions Chest (6 Flasks Liquid Fire) • Officer's Locker        │  |
|  └──────────────────────────────────────┬─────────────────────────────────────────┘  |
|                                         │ (Reinforced Stone Flue)                    |
|                                         ▼                                            |
|                            [To Location 9: The King's Door]                          |
+--------------------------------------------------------------------------------------+
```

---

### 3.6 Locations 9 & 10: The King's Door & The Royal Vault (Level 3C)

```
+--------------------------------------------------------------------------------------+
|            [From Location 6: Colonnade]  /  [From Location 8: Stone Flue]            |
|                                         │                                            |
|                                         ▼                                            |
|  ═════════════════════════ [OVERHEAD DEFENSIVE PARAPET] ═══════════════════════════  |
|  (15-ft Elevation; Murder-Holes; Grimnar the Disgraced & 4 Moria Orc Stalkers Ambush!)|
|                                                                                      |
|                         ┌─────────────────────────────┐                              |
|                         │       THE KING'S DOOR       │                              |
|                         │ • Star-Iron & Black Granite │                              |
|                         │ • Pulsing Ithildin Runes    │                              |
|                         │ • Dual Runic Master Locks   │                              |
|                         └──────────────┬──────────────┘                              |
|                                        │ (Unlocked via Mithril & King's Keys / Endeavour) |
|                                        ▼                                             |
|  ═══════════════════════════ [VAULT BLAST BARRICADE] ═══════════════════════════════  |
|  (Inner Adamant Portcullis — Boltable from inside; 30-Minute Safe Rest Redoubt)      |
|                                                                                      |
|                         ┌─────────────────────────────┐                              |
|                         │      THE DAIS OF DURIN      │                              |
|                         │ • Hexagonal Black Marble    │                              |
|                         │ • DURIN'S AXE (Artifact)    │ <── +4 Eye Awareness Spike!  |
|                         │ • Radiant Stasis Halo       │                              |
|                         └─────────────────────────────┘                              |
|                                                                                      |
|  [COFFER 1: Deep Gate Shield]   [COFFER 2: Moria-Silver Mattock]   [COFFER 3: Stone Mail]
|  [CHESTS OF REFINED MITHRIL INGOTS (5 Bars)]   [ROYAL REGALIA OF THE THIRD DEEP]      |
+--------------------------------------------------------------------------------------+
```

---

## 4. Secret Bypass Ducts & Maintenance Flues

```
========================================================================================================================
                                     SECRET BYPASS DUCTS & FLUES
========================================================================================================================
 FLUE NAME             CONNECTS                  DETECTION & TRANSIT TEST     TACTICAL UTILITY
------------------------------------------------------------------------------------------------------------------------
 1. Smuggler's Vent    Loc 1 -> Loc 3            **EXPLORE roll (Favoured)**  Completely bypasses the sentries and 
                                                                              buckled blast-doors of Location 2.
------------------------------------------------------------------------------------------------------------------------
 2. Maintenance Flue   Loc 3 -> Loc 5            **EXPLORE roll (+1d)**       Bypasses the Balrog Idol in Location 4,
                                                 (Invoking *Burglary*)        avoiding all Dread tests!
------------------------------------------------------------------------------------------------------------------------
 3. Arsenal Flue       Loc 8 -> Loc 9            **SCAN roll**                Direct connecting chute from Upper Armoury
                                                                              to King's Door; bypasses Location 6 Troll!
========================================================================================================================
```

---

## 5. Tactical Fighting Withdrawal Flowchart

```
========================================================================================================================
                                  TACTICAL FIGHTING WITHDRAWAL FLOWCHART
========================================================================================================================

  [TRIGGER: LIFTING DURIN'S AXE] ──► +4 Strategic Eye Awareness ──► ALERT TIER 3 (Drums in the Deep!)
                                                                                │
                                                                                ▼
  [LOCATION 10: VAULT REDOUBT]   ──► Brief Rest (30 min) / Distribute Relics / Pack 50 Suits of Mail (Heavy Burden)
                                                                                │
                                                                                ▼
  [LOCATION 9: KING'S DOOR EXIT] ──► Rearguard deploys / Band Clash vs Orc Advance Squad (Band WAR 3d vs Band TN 15)
                                                                                │
                                                                                ▼
  [LOCATION 6 / 8 TRANSIT]       ──► Heavy Porters escorted through Flue / Mauler arena bypassed or bottled
                                                                                │
                                                                                ▼
  [LOCATION 5: SIEGE WORKSHOP]   ──► Deploy pre-built cedar barricades / Drop counterweight crane on pursuers
                                                                                │
                                                                                ▼
  [LOCATION 4 & 3 TRANSIT]       ──► Rear-facing scythe scrap-traps trigger on Orc vanguard (Delays pursuit 2 rds)
                                                                                │
                                                                                ▼
  [LOCATION 2: GATEHOUSE STAND]  ──► Rearguard (Bláin & Fáin) covers retreat (+2 Readiness)
                                     PULL KEYSTONE WINCH LEVER! ──► 30 CRUSHING DAMAGE CAVE-IN!
                                     Southern rampway permanently sealed with 10 tons of rubble!
                                                                                │
                                                                                ▼
   [LOCATION 1: MUSTERING-YARD]   ──► Secure heavy salvage onto wheel-sledges & litters; overwatch covers portals
                                                                                 │
                                                                                 ▼
   [THE WESTWARD DRIFT (RETREAT)] ──► Phalanx rearguard falls back along Transverse Gallery to Thistlebeard Haven
                                                                                 │
                                                                                 ▼
   [SAFE HAVEN & EAST-GATE CAMP]  ──► Extraction Complete! 40 Mail-shirts, 30 Shields, 50 Mattocks/Axes (+2 Readiness,
                                     50 Treasure Points in tribute, and Royal Renown for King Dáin Ironfoot!)
========================================================================================================================
```



<!-- PAGE BREAK: Appendix B -->

---

## Appendix B: 1-Page Rapid GM Cheat Sheet


# Rapid GM Cheat Sheet & Reference Dashboard
### *The Armouries of the Third Deep* — Quick-Reference Screen

---

## 1. Hero & Band Quick-Reference Dashboard

```
+========================================================================================================+
|                                    HERO & BAND OPERATIONAL REFERENCE                                    |
+========================================================================================================+
| PLAYER-HERO ATTRIBUTE TNS (20 - Attribute Rating):                                                     |
| • Torvir Hammerstone : STR 7 (TN 13) | HRT 2 (TN 18) | WIT 5 (TN 15) | Parry 15 | Mail 5d | Axe 3d (8/20)|
| • Einar son of Anar  : STR 6 (TN 14) | HRT 3 (TN 17) | WIT 5 (TN 15) | Parry 20 | Mail 3d | Swd 3d (4/16)|
| • Khoril Hornblower  : STR 7 (TN 13) | HRT 4 (TN 16) | WIT 4 (TN 16) | Parry 17 | Mail 3d | Axe 3d (6/18)|
|                                                                                                        |
| BAND READINESS & DISPOSITIONS:                                                                         |
| • Band Readiness: 5 (Band TN 15 = 20 - 5) | Band Hope: 12 / 12 | Band Shadow: 1 / 12                   |
| • Dispositions: WAR: 3d | VIGILANCE: 2d | MANOEUVRE: 2d | EXPERTISE: 2d | RALLY: 1d                     |
+========================================================================================================+
```

---

## 2. 10-Room Operational Summary Matrix

```
========================================================================================================================
                                             ROOM OPERATIONAL MATRIX
========================================================================================================================
 LOC NAME & ELEVATION     LIGHT & ATMOSPHERE   HAZARDS & ADVERSARIES        KEY SKILL CHECKS                 SOUND & NOISE
------------------------------------------------------------------------------------------------------------------------
  1  The Mustering-Yard   Pitch dark; green    2 Moria Sniffers behind P4;  STEALTH roll to bypass;          +0 (Shadow-crawl)
     (Level 3A)           lichen; cold drafts  Sentry Camp & Brazier        BATTLE roll for ambush;          +1 (1-rd kill)
                                                                            SCAN roll (Favoured);            +3 (Horn alarm)
                                                                            SCAN/AWE roll to interrogate.
------------------------------------------------------------------------------------------------------------------------
  2  The Upper Gatehouse  Deep shadow; roaring Buckled adamant doors;       CRAFT roll to fortify;           +1 (Shore timber)
     (Level 3A)           drafts; muffled talk Keystone Winch Trap (30 Dmg) AWARENESS roll vs Slag-Worm;     +4 (Trigger Cave-In)
                                                                            CRAFT/BATTLE roll to rig.
------------------------------------------------------------------------------------------------------------------------
  3  The First Armoury    Pitch dark; stagnant Scythe scrap-traps; venom    SCAN roll (Favoured);            +0 (Disarm silently)
     (Level 3B)           air; rot & venom     vats (Injury 16 + Poison)    CRAFT roll (+1d);                +2 (Spring trap)
                                                                            HEALING roll to harvest venom.   +1 (Search scrap)
------------------------------------------------------------------------------------------------------------------------
  4  The Broken Hall      Pitch dark; warm     12-ft Balrog Idol; Dread     VALOUR roll vs Dread;            +0 (Sneak/Lurk)
     (Level 3B)           downdraft; char      Aura; Defaced murals         LORE/RIDDLE roll Cartouche;      +3 (Topple idol)
                                                                            SONG/ENHEARTEN roll (+1d).       +1 (Muffle topple)
------------------------------------------------------------------------------------------------------------------------
  5  The Second Armoury   Gloom; cedar & pine  Siege Engines; Grond-ram;    CRAFT roll to prime siege;       +0 (Inspect gears)
     (Level 3B)           pitch; muffled steps Torsion Ballistas; Patrol    ATHLETICS roll to aim ram;       +3 (Ballista fire)
                                                                            BATTLE roll for ambush zone.     +4 (Grond-ram)
------------------------------------------------------------------------------------------------------------------------
  6  Hall of the Mauler   Dim moss; foul breath; The Mauler (AL 10 Troll,   STEALTH roll across scrap;       +0 (Catwalk sneak)
     (Level 3C)           floor-shaking snore  Scrap Armour 5d, Might 2)    Forward stance RIDDLE roll       +2 (Combat/round)
                                                                            (-1 Hate/success, -1 Hate / 6).  +3 (Stalactite/Roar)
------------------------------------------------------------------------------------------------------------------------
  7  The Poisoned Halls   Emerald phosphores-  Balrog Neurotoxic Miasma;    PROTECTION roll (gas);           +0 (Silent gas move)
     (Level 3B-minus)     cence; sulfur/almond Petrified Dwarf Scribe       CRAFT roll for respirators;      +3 (Flue lever)
                                               (Handout #1: Scribe Slate)   HEALING roll for remedies.       +1 (Drop tool)
------------------------------------------------------------------------------------------------------------------------
  8  The Upper Armoury    Sterile dry cold;    Garrison Salvage (50 Mail);  EXPLORE roll for storage;        +1 (Quiet packing)
     (Level 3B)           gleaming steel       Lead-sealed Munitions Chest  CRAFT roll for liquid fire;      +3 (Rushed clatter)
                                               (6 Flasks Liquid Fire)       ATHLETICS roll to haul.          +0 (Open bronze door)
------------------------------------------------------------------------------------------------------------------------
  9  The King's Door      Pulsing Ithildin blue; Dual Runic Master Locks;   AWARENESS roll for ambush;       +0 (Dual Keys)
     (Level 3C)           sacred cool hush     Grimnar's Parapet Ambush     Skill Endeavour (Resistance 6,   +1 (Lockpick/turn)
                                               (Grimnar AL 6 + 4 Stalkers)  3 Turns: SCAN/CRAFT/RIDDLE).     +2 (Skirmish/round)
------------------------------------------------------------------------------------------------------------------------
 10  The Lower Armoury    Radiant gold/silver; Durin's Axe (Artifact);      AWE/SONG roll;                   Special: +4 STRATEGIC
     (Level 3C)           sacred pure mountain 3 Relic Coffers; Hoard       VALOUR roll (Greed flaw);        EYE AWARENESS &
                          air; snow & cedar    Vault Blast Barricade        CRAFT roll for stasis.           INSTANT ALERT 3!
========================================================================================================================
```

---

## 3. Adversary Combat Quick-Stats

```
========================================================================================================================
                                      ADVERSARY COMBAT REFERENCE
========================================================================================================================
 ADVERSARY           AL  END  MIGHT HATE PARRY ARMOUR  MAIN ATTACK        DMG/INJ  SPECIAL DAMAGE & FELL ABILITIES
------------------------------------------------------------------------------------------------------------------------
 The Mauler          10  80     2    10    —     5d    Maul / Club 3d     8 / 16   Break Shield; Crush 3d (6/12, Seize);
 (Armoured Troll)                                      Scrap Shrapnel 2d  6 / 12   Dull-Witted (Forward stance RIDDLE roll);
                                                                                   Hideous Tough (Reset 40); Strike Fear
------------------------------------------------------------------------------------------------------------------------
 Grimnar the          6  36     2     6   +2     3d    Heavy Scimitar 3d  5 / 16   Break Shield; Stolen Dagger 3d (4/14, Keen);
 Disgraced (Stalker)                                   Broad Spear 2d     5 / 16   Hatred (Durin's Folk, Favoured); Snake-Speed
                                                                                   (Ill-favoured); Vengeful Strike; Hideous Tough
------------------------------------------------------------------------------------------------------------------------
 Moria Sniffer        4  16     1     4    —     3d    Iron-shod Staff 3d Dmg 4, Inj 14 (Heavy); Blowdart 2d (2/12,
 (Goblin Hunter)                                                                       Black Venom); Heartless; Keen Scent (+2d Aware)
------------------------------------------------------------------------------------------------------------------------
 Orc Guard            4  16     1     4   +2     3d    Heavy Scimitar 3d  4 / 16   Pierce; Heavy Spear 3d (4/14); Thick Armour;
 (Moria Sentry)                                                                    Denizen of Dark (Favoured); Hate Sunlight
------------------------------------------------------------------------------------------------------------------------
 Orc Soldier          3  12     1     3   +1     2d    Orc-axe 2d         3 / 18   Break Shield; Short Bow 2d (3/14); Craven;
 (Moria Minion)                                                                    Denizen of Dark; Hate Sunlight
------------------------------------------------------------------------------------------------------------------------
 Orc Drummer          3  12     1     3   +1     2d    Curved Knife 2d    3 / 14   Pierce; Drums in Deep (1 Hate = +3 Eye!)
------------------------------------------------------------------------------------------------------------------------
 Black Uruk           5  20     1     5   +2     3d    Broadsword 3d      4 / 16   Pierce; Bow of Horn 3d (3/14); Horrible
 (Mordor Shock-Troop)                                                              Strength (Ill-fav Protection); Thick Armour
------------------------------------------------------------------------------------------------------------------------
 Black Uruk Captain   6  24     2     6   +3     4d    Great Scimitar 3d  5 / 16   Break Shield; Pierce; Horrible Strength;
 (Vanguard Leader)                                                                 Yell of Triumph (1 Hate = +1 Hate to allies)
========================================================================================================================
```

---

## 4. 4-Stage Alert Tracker & Sound Economy

```
========================================================================================================================
                                     ALERT TRACKER & SOUND ECONOMY
========================================================================================================================
 ALERT STAGE           NOISE POINTS   AMBIENT POSTURE              MECHANICAL MODIFIERS & GM TRIGGERS
------------------------------------------------------------------------------------------------------------------------
 Alert 0: Quiet        0–3 Noise      Tomb-like quiet; loose       +1d to all STEALTH & EXPLORE tests;
 Shadows (Unwary)                     sentries gambling/sleeping   Company gains automatic Surprise Round
------------------------------------------------------------------------------------------------------------------------
 Alert 1: Unease &     4–7 Noise      Rhythmic wall tapping;       Standard dice pools; Roaming Moria Sniffer patrol
 Scent (Suspicious)                   burnt tallow stench          enters chamber on Feat Die Eye of Sauron (S)
------------------------------------------------------------------------------------------------------------------------
 Alert 2: Hunted &     8–11 Noise     Sulfur smoke; iron horns;    -1d on AWARENESS to avoid ambushes; doors barred;
 Barricaded (Hunted)                  Grimnar actively stalks      Hunt Threshold reduced by 2 (14 -> 12)
------------------------------------------------------------------------------------------------------------------------
 Alert 3: Drums in     12+ Noise      Doom, doom war-drums;        REVELATION EPISODE! Full garrison assault;
 Deep (Overrun)                       exit shafts sealed           6-Round Evacuation Countdown to reach Gatehouse
========================================================================================================================
 NOISE ACTION TABLE:
 • Silent move / whisper / lockpicking: +0 Noise
 • Spoken conversation / opening chests / 1-rd kill: +1 Noise (1/4 Alert step)
 • Loud combat round (shouting, clashing iron, bows): +2 Noise (1/2 Alert Tier)
 • Extended combat (4+ rds) / toppling idol / sledgehammer: +3 Noise (+1 Full Alert Tier, +1 Eye Awareness)
 • Firing Ballista / Grond-ram / triggering cave-in: +4 Noise (+1 Full Alert Tier, +1 Eye Awareness)
 • Sounding a Great War-Horn of the Realm: +5 Noise (INSTANT ALERT 3! +2 Eye Awareness)
 • Claiming Durin's Axe from Dais: Special (+4 STRATEGIC EYE AWARENESS & INSTANT ALERT 3!)
========================================================================================================================
```

---

## 5. Toxic Gas Hazard & Subterranean Water Perils

```
========================================================================================================================
                                      HAZARD QUICK-REFERENCE MATRIX
========================================================================================================================
 BALROG TOXIC MIASMA (Rooms 7 & 8):
 • Unprotected (Raw breathing) : Roll **PROTECTION test** EVERY TURN (Ill-favoured). 1-10: Lose End; Eye: Dying!
 • Protected (Vinegar cloth/herb): Roll **PROTECTION test** EVERY HOUR. 1-6: Lose End; Eye: Severe Poison.
 • Masterwork Mask (**CRAFT roll**): COMPLETE IMMUNITY for 4 hours (up to 10 characters).
 • Unjam Flue Damper (**CRAFT roll** / **ATHLETICS roll**): Vents room in 3 rounds (+3 Noise Points).
------------------------------------------------------------------------------------------------------------------------
 WATER PERILS TABLE (Roll Feat Die on drinking untested water):
 • Gandalf Rune (G): Pristine Ancient Spring (Restores +2 Hope, clears 1 tier of Fatigue).
 • 9–10: Icy Snowmelt (Clean and refreshing).
 • 7–8: Orc-Filth (Moderate Poison: lose 4 End, cleared by rest).
 • 5–6: Bitter Water (**VALOUR roll** to swallow; failure = -1d on physical tests for 1 hour).
 • 3–4: Acidic Mine Runoff (Severe Poison: lose 8 End; requires First Aid).
 • 1–2: Malice of Durin's Bane (2 Shadow [Sorcery]; hallucinations).
 • Eye of Sauron (S): The Lurker's Pool (Clean water, but triggers concealed Moria sentry ambush!).
========================================================================================================================
```

---

## 6. Band Mechanics & Tactical Squad Reference

```
========================================================================================================================
                                       BAND QUICK-REFERENCE DASHBOARD
========================================================================================================================
 BAND READINESS: 5 (Band TN 15)  |  BURDEN: Medium -> Heavy (Salvaging)  |  HOPE: 12  |  SHADOW: 1
 DISPOSITIONS:   WAR: 3d  |  VIGILANCE: 2d  |  MANOEUVRE: 2d  |  EXPERTISE: 2d  |  RALLY: 1d
------------------------------------------------------------------------------------------------------------------------
 TACTICAL SQUADS:
 1. Forward Scout Screen (Austri & Bróga): Rolls Vigilance (2d) vs Band TN 15. Prevents surprise; scout screen safety.
 2. Shield-Wall Phalanx (Dúrmer, Dolg, Bláin): Rolls War (3d) vs Band TN 15. Dolg intercepts 1 attack/rd; anti-swarm.
 3. Rearguard Choke Point (Bláin & Fáin): Held at Location 2 Gatehouse. Guarantees escape route; +2 Band Readiness.
 4. Heavy Salvage Porters (Hjoldring & Dúrmer): Hauls 50 mail suits for Balin's colony. Shifts Band to Heavy Burden.
------------------------------------------------------------------------------------------------------------------------
 CRITICAL CONDITIONS & RULES:
 • Band Weary: Triggered when >= 4 companions are incapacitated (Severe/Grievous/Spent/Collapsed). 1, 2, 3 on Success dice = 0!
 • Desperate Stand: Failed critical test -> Re-roll Favoured & Inspired. Gandalf (G) = survives (Moderate); else Slain (+2 Shadow).
========================================================================================================================
```



<!-- PAGE BREAK: Appendix C -->

---

## Appendix C: Dwarf Vanguard Band Worksheet


# Moria Band Management Worksheet
### *The Armouries of the Third Deep* — Player & GM Tactical Squad Tracker

---

## 1. Expedition Overview & Band Statistical Profile

```
========================================================================================================================
                                             EXPEDITION HEADER
========================================================================================================================
 EXPEDITION FORCE:      Balin's Vanguard — Third Deep Infiltration Force (Year 2989 TA)
 EXPEDITION LEADERS & ATTRIBUTE TN MATRIX (20 - Attribute Rating):
 • Torvir Hammerstone (Champion)      : STR 7 (TN 13) | HRT 2 (TN 18) | WIT 5 (TN 15) | Parry 15 | Mail 5d
 • Einar son of Anar (Treasure Hunter): STR 6 (TN 14) | HRT 3 (TN 17) | WIT 5 (TN 15) | Parry 20 | Mail 3d
 • Khoril Hornblower (Captain)        : STR 7 (TN 13) | HRT 4 (TN 16) | WIT 4 (TN 16) | Parry 17 | Mail 3d
 SAFE HAVEN BASE:       The Caves of Thrym Thistlebeard (Third Deep Stratum)
 PRIMARY MISSION:       Salvage 50 Suits of Garrison Wargear & Recover Durin's Axe for King Dáin
========================================================================================================================
```

```
========================================================================================================================
                                         BAND STATISTICAL DASHBOARD
========================================================================================================================
 BAND READINESS RATING:  [ 5 ]  (Base 4 + 1 for Hardened Veteran Dúrmer)
 BAND READINESS TN:      [ 15 ] (Calculated as 20 - Band Readiness 5)
 ACTIVE BAND SIZE:       [ 7 ]  Veteran Dwarf Companions
 CURRENT BAND BURDEN:    [ ] Light    [X] Medium (Ingress)    [ ] Heavy (Salvaging)    [ ] Overburdened
 
 BAND HOPE POOL (Max 12):    [■] [■] [■] [■] [■] [■] [■] [■] [■] [■] [■] [■]   (Current: 12 / 12)
 BAND SHADOW (Cleansed 1):   [■] [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ]   (Current:  1 / 12)
 FELLOWSHIP POOL:            [■] [■] [■] [■]                                   (Current:  4 / 4)
 
 BAND WEARY CONDITION:       [ ] ACTIVE (Triggered when >= 4 companions are Severe+, Spent, or Collapsed)
                             *When Weary: Success dice showing 1, 2, or 3 count as 0!*
========================================================================================================================
```

```
========================================================================================================================
                                          THE FIVE BAND DISPOSITIONS
========================================================================================================================
  [ WAR: 3d6 ]           [ VIGILANCE: 2d6 ]      [ MANOEUVRE: 2d6 ]      [ EXPERTISE: 2d6 ]      [ RALLY: 1d6 ]
  • Melee clashes        • Ambush detection      • Silent marching       • Trap disarming        • Morale recovery
  • Shield-wall charges  • Sentry perimeter      • Tactical withdrawal   • Lock picking          • Resisting Dread
  • Forcing stone        • Acoustic scouting     • Difficult terrain     • Forge / Repair        • Fatigue tests
========================================================================================================================
```

---

## 2. The Seven Companion Dwarves Roster

```
========================================================================================================================
                                          COMPANION TRACKING ROSTER
========================================================================================================================
 #  NAME & SPECIALTY     ENDURANCE   INJURY STATUS TRACKER           FATIGUE STATUS TRACKER          ACTIVE SQUAD / ROLE
------------------------------------------------------------------------------------------------------------------------
 1  BLÁIN THE CLEAVER    [10 / 18]   [ ] Fleeting   [X] Moderate     [ ] Fatigued   [ ] Weary        [ ] Forward Screen
    • Gift: Goblin-Slayer            [ ] Severe     [ ] Grievous     [ ] Spent      [ ] Collapsed    [X] Shield-Wall / Rear
      (+1d vs Orcs/Goblins)          [ ] Lingering                                                   [ ] Salvage Porter
------------------------------------------------------------------------------------------------------------------------
 2  FÁIN IRON-VISOR      [15 / 18]   [ ] Fleeting   [ ] Moderate     [ ] Fatigued   [ ] Weary        [ ] Forward Screen
    • Gift: Dead-Eye                 [ ] Severe     [ ] Grievous     [ ] Spent      [ ] Collapsed    [X] Rearguard Overwatch
      (+1d Ranged from Rear)         [ ] Lingering                                                   [ ] Salvage Porter
------------------------------------------------------------------------------------------------------------------------
 3  DÚRMER IRON-SHOULDER [22 / 22]   [ ] Fleeting   [ ] Moderate     [ ] Fatigued   [ ] Weary        [ ] Forward Screen
    • Status: Hardened Veteran       [ ] Severe     [ ] Grievous     [ ] Spent      [ ] Collapsed    [X] Phalanx Anchor
    • Gift: Mighty (+1d Labor)       [ ] Lingering                                                   [X] Salvage Porter Lead
------------------------------------------------------------------------------------------------------------------------
 4  HJOLDRING ARMOURER   [18 / 18]   [ ] Fleeting   [ ] Moderate     [ ] Fatigued   [ ] Weary        [ ] Forward Screen
    • Gift: Smith (+1d Forge,        [ ] Severe     [ ] Grievous     [ ] Spent      [ ] Collapsed    [ ] Shield-Wall Phalanx
      Repair & Salvage)              [ ] Lingering                                                   [X] Heavy Salvage Lead
------------------------------------------------------------------------------------------------------------------------
 5  BRÓGA VAULTBREAKER   [12 / 12]   [ ] Fleeting   [ ] Moderate     [ ] Fatigued   [ ] Weary        [X] Forward Screen
    • Gift: Vaultbreaker             [ ] Severe     [ ] Grievous     [ ] Spent      [ ] Collapsed    [ ] Lockbreaker Support
      (+1d Locks & Traps)            [ ] Lingering                                                   [ ] Salvage Porter
------------------------------------------------------------------------------------------------------------------------
 6  AUSTRI WAYFINDER     [10 / 18]   [ ] Fleeting   [ ] Moderate     [ ] Fatigued   [ ] Weary        [X] Point Scout Screen
    • Gift: Scout (+1d Flues,        [ ] Severe     [ ] Grievous     [ ] Spent      [ ] Collapsed    [ ] Overwatch
      Bypasses & Stealth)            [ ] Lingering                                                   [ ] Salvage Porter
------------------------------------------------------------------------------------------------------------------------
 7  DOLG THE BULWARK     [18 / 18]   [ ] Fleeting   [ ] Moderate     [ ] Fatigued   [ ] Weary        [ ] Forward Screen
    • Gift: Shield-Bearer            [ ] Severe     [ ] Grievous     [ ] Spent      [ ] Collapsed    [X] Shield-Wall Center
      (Intercepts 1 blow/rd)         [ ] Lingering                                                   [ ] Salvage Porter
========================================================================================================================
```

---

## 3. Tactical Squad Deployment Assignments

Fill in the checkboxes and assign companion names to active operational squads:

```
========================================================================================================================
                                      ACTIVE SQUAD ASSIGNMENT TRACKER
========================================================================================================================

  [ ] 1. FORWARD SCOUT SCREEN (2 Companions)
      • Assigned: Austri the Wayfinder & Bróga Vaultbreaker
      • Tactical Function: Advances 50 paces ahead. Rolls Band Vigilance (2d6) or Manoeuvre (2d6) vs Band TN 15.
      • Benefits: Prevents surprise; spots sentries/traps; allows scouts to make SCAN rolls (Favoured) safely.

  [ ] 2. SHIELD-WALL PHALANX (3 Companions)
      • Assigned: Dúrmer the Iron-Shouldered, Dolg the Bulwark, Bláin the Cleaver
      • Tactical Function: Locks heavy tower-shields across choke points. Rolls Band War (3d6) vs Band TN 15.
      • Benefits: Dolg intercepts 1 melee strike/round; blocks enemy flanking; spending 6s pushes enemy lines back 10 paces.

  [ ] 3. REARGUARD CHOKE POINT DEFENSE (2 Companions stationed at Location 2 Gatehouse)
      • Assigned: Bláin the Cleaver & Fáin Iron-Visor
      • Tactical Function: Fortifies the Upper Gatehouse to secure the Westward Drift escape route to Thrym's Safe Haven.
      • Benefits: Guarantees extraction route; +2 Band Readiness on Fighting Withdrawal; rigs 30-Dmg Keystone Collapse Trap!

  [ ] 4. HEAVY SALVAGE PORTER SQUAD (4 Companions)
      • Assigned: Hjoldring the Armourer, Dúrmer the Iron-Shouldered, Dolg the Bulwark, Bróga Vaultbreaker
      • Tactical Function: Hauls 50 suits of gromril-mail, 30 shields, and weapons from Location 8 for Balin's colony.
      • Drawbacks: Band shifts to Heavy Burden (-1d Manoeuvre, -1d Fatigue, +1 Noise Point per hall traversed).
========================================================================================================================
```

---

## 4. Band Clash & Mass Skirmish Worksheet

Use this section during combat encounters against organized Orc warbands:

```
========================================================================================================================
                                        BAND CLASH RESOLUTION SHEET
========================================================================================================================
 STEP 1: CHOOSE BAND STANCE
 [ ] AGGRESSIVE : Ill-favoured Clash roll; automatically deals +1 bonus Resistance damage on any success.
 [ ] BALANCED   : Standard Clash roll; full tactical versatility.
 [ ] GUARDED    : Favoured Clash roll; ignores the first Special Damage trigger rolled by the enemy.
 [ ] FLEEING    : Band Manoeuvre roll vs Band TN 15 + Enemy Might to disengage and retreat to previous room.
------------------------------------------------------------------------------------------------------------------------
 STEP 2: HERO LEADER ACTIONS (Player Choice)
 • Leader / Captain  : [ ] Command (BATTLE roll -> +1d to Clash)
                       [ ] Inspire (ENHEARTEN roll -> +1 Hope / clear Weary)
 • Frontline Champion: [ ] Fight (Attack roll vs Adversary Parry)
                       [ ] Duel (Single combat vs Enemy Champion, negates Might penalty)
 • Scout / Support   : [ ] Flank / Support (Attack roll or BATTLE roll -> +1d to allies)
                       [ ] Infiltration / Utility (SCAN roll or CRAFT roll, +1d if invoking Burglary)
------------------------------------------------------------------------------------------------------------------------
 STEP 3: RESOLVE CLASH ROLL
 • Roll Band War (3d6) vs Target Number: [ Band TN 15 + Enemy Might: _____ ] = Total TN: [ _____ ]
 • Feat Die Result: [ _____ ]   Success Dice (6s): [ _____ ] [ _____ ] [ _____ ]   Success? [ YES / NO ]
------------------------------------------------------------------------------------------------------------------------
 STEP 4: ENEMY RESISTANCE TRACKER
 • Patrol   (Might 0 | Threat: Bothersome) : [ 3 ]  [ ] [ ] [ ] (Broken at 0)
 • Pack     (Might 1 | Threat: Painful)    : [ 6 ]  [ ] [ ] [ ] [ ] [ ] [ ] (Broken at 0)
 • Warband  (Might 2 | Threat: Vicious)    : [ 9 ]  [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] (Broken at 0)
 • Horde    (Might 3 | Threat: Dreadful)   : [ 12 ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] [ ] (Broken at 0)
========================================================================================================================
```

---

## 5. The Desperate Stand Resolution Flowchart

When a Player-Hero or companion faces a lethal blow or when a critical escape roll fails:

```
========================================================================================================================
                                      DESPERATE STAND QUICK GUIDE
========================================================================================================================
 1. DECLARATION   : A companion (e.g. Dolg or Dúrmer) heroically throws themselves into the breach.
 2. RE-ROLL       : The failed test is immediately re-rolled FAVOURED (roll 2 Feat dice, take best) 
                    and INSPIRED (Success dice total is doubled!).
 3. SURVIVAL CHECK: Examine the Feat Die of the re-roll:
    • GANDALF RUNE (G) : The companion miraculously survives, suffering only a Moderate Injury!
    • ANY OTHER RESULT : The companion is SLAIN or LOST in the dark.
                         The Company immediately gains +2 BAND SHADOW (Dread).
========================================================================================================================
```



<!-- PAGE BREAK: Appendix D -->

---

## Appendix D: Player Handout: Dying Scribe's Slate


# Handout: The Dying Scribe's Slate

> *"To whoever walks these halls in the light of later days: let not the heritage of Durin die in the suffocating dark."*  
> — *Frár son of Frerin, Scribe of the Third Deep (Anno 1981 TA)*

---

## 1. Physical Description & Table Prop Notes

```
========================================================================================
                                 PROP SPECIFICATION
========================================================================================
 OBJECT:         A slab of dark grey basalt slate, roughly 14 inches long and 9 inches 
                 wide, fractured diagonally across its lower left corner.
 INSCRIPTION:    Etched with an iron-tipped chisel in formal Angerthas Moria (Cirth Runes), 
                 hurriedly carved into the stone with trembling strokes.
 PHYSICAL CUES:  The grooves of the runes are filled with dried, blackened Dwarf-blood and 
                 a crust of sulfurous yellow soot from the Balrog's toxic miasma.
 LOCATION FOUND: Clutched in the petrified, mail-clad skeletal hands of Scribe Frár, seated 
                 against the council dais in Keyed Location 7 (The Poisoned Halls).
========================================================================================
```

---

## 2. In-World Inscription: The Slate of Scribe Frár

```
========================================================================================
               ᚠᚱᚨᚱ ᛋᛟᚾ ᛟᚠ ᚠᚱᛖᚱᛁᚾ — ᛏᚺᛖ ᛚᚨᛋᛏ ᚹᛟᚱᛞᛋ ᛟᚠ ᛏᚺᛖ ᛏᚺᛁᚱᛞ ᛞᛖᛖᛈ
========================================================================================
 
 [ CIRTH RUNIC ENGRAVING — ARCHAIC TRANSLATION RECORDED FOR TABLE DISPLAY ]

 I AM FRÁR, SON OF FRERIN, SCRIBE OF THE THIRD DEEP. THE SHADOW HAS AWOKEN. FROM THE BLACK 
 PITS BENEATH THE ROOTS OF CARADHRAS IT CAME, CLAD IN SMOKE AND RED FIRE. OUR MIGHT IS 
 BROKEN. KING DURIN IS FALLEN; KING NÁIN IS STRUCK DOWN BY THE SHADOW. THE MOUNTAIN SHAKES, 
 AND CHOKING ASH POURS THROUGH THE LOWER FLUES.
 
 BY ORDER OF LORD MARSHAL THRAIN, WE HAVE RETREATED TO THE TWELFTH AND FOURTEENTH HALLS. 
 THE HIGH HEIRLOOMS OF OUR ANCESTORS SHALL NOT BE DEFILED BY THE FLAMES. WE HAVE BORNE 
 DURIN'S AXE AND THE ROYAL CROWNS INTO THE DEEPEST ROCK OF THE LOWER ARMOURY.
 
 THE KING'S DOOR IS SEALED.
 
 WE DROPPED THE ADAMANT BOLTS AND TURNED THE DUAL RINGS OF GROMRIL. THE WARDING WILL 
 HOLD THOUGH THE MOUNTAIN SHAKE. BUT WE COULD NOT PRESERVE BOTH KEYS IN ONE BASIN. 
 
 THE KING'S KEY WAS BORNE UPWARD BY LORD NÁIN’S COURIERS, FLEEING TOWARD THE LAST REDOUBT 
 IN THE FIFTH HALL. SEEK THE KING'S KEY IN THAT FINAL REFUGE, IF ANY OF DURIN'S FOLK SURVIVE.
 
 THE MITHRIL KEY WAS CARRIED BY MARSHAL THRAIN AS HE FLED TOWARD THE SECRET PILLAR DOOR 
 BY THE DIMRILL STAIRS. BUT THE SHADOW OVERTOOK THEM, AND THE KEY IS LOST NEAR THE PILLAR. 
 I FEAR ANY INVADERS WHO ENTER MANSIONS IN LATER YEARS WILL CLAIM IT FOR ITS BRIGHT METAL, 
 THOUGH THEY SHALL NEVER KNOW IT LEADS TO THE TREASURE CHAMBER.
 
 NOW THE BREATH OF THE DEMON RISES THROUGH THE VENTILATION CHUTES. IT IS NOT MERE SMOKE. 
 IT IS A FOUL, EMERALD-YELLOW MIASMA THAT TURNS THE BLOOD TO SLAG AND DRIES THE LUNGS TO 
 STONE. WE HAVE NO HERBS LEFT TO CHEW. MY HANDS GROW COLD AS IRON. THE CAPTAINS LIE 
 SLEEPING BESIDE ME, THEIR VISORS UNLIFTED.
 
 IF A TRUE SON OF DURIN EVER CLEANSES THESE MANSIONS, SEEK THE MITHRIL KEY AND FIND THE 
 LAST REDOUBT TO RECLAIM THE KING'S KEY, OR SET THY MASTER SMITHS TO THE PORTAL. 
 
 LIFT ONCE MORE THE AXE OF DURIN. 
 LET ITS FLAME SHINE IN THE DARK. 
 
 BARUK KHAZÂD! KHAZÂD AI-MÊNU!
 
 [ The inscription ends in jagged, unfinished strokes where the chisel slipped ]
========================================================================================
```

---

## 3. Visual Handout Layout (For GM Table Delivery)

```
+--------------------------------------------------------------------------------------+
|  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  |
| /  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \ |
|/                                                                                    \|
|   "I am Frár son of Frerin, Scribe of the Third Deep...                              |
|                                                                                      |
|   The Shadow has awoken. From the black pits beneath the roots of                    |
|   Caradhras it came, clad in smoke and red fire. King Durin is fallen;               |
|   King Náin is struck down by the Shadow.                                            |
|                                                                                      |
|   By order of the Lord Marshal, we have retreated to the Fourteenth Hall.            |
|   Durin's Axe rests in the sealed Lower Armoury.                                     |
|                                                                                      |
|   THE KING'S DOOR IS SEALED.                                                         |
|                                                                                      |
|   The King's Key was borne upward to the Last Redoubt in the Fifth Hall.             |
|   The Mithril Key was lost near the Dimrill Stairs pillar door when Marshal          |
|   Thrain was overtaken by the Shadow. Later invaders may claim it, though they       |
|   will not know it leads here.                                                       |
|                                                                                      |
|   The breath of the demon rises. It turns the lungs to stone. We have no             |
|   vinegar or herbs left. My fingers freeze.                                          |
|                                                                                      |
|   Find the Last Redoubt for the King's Key, and seek the Mithril Key.                |
|   Lift once more the Axe of Durin.                                                   |
|                                                                                      |
|   BARUK KHAZÂD! KHAZÂD AI-MÊNU!"                                                     |
|                                                                                      |
|\                                                                                    /|
| \  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  /\  / |
|  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  \/  |
+--------------------------------------------------------------------------------------+
```

---

## 4. Runemaster's Translation & Linguistic Notes

When a player-hero or companion examines the stone slab, the GM may share the following detailed observations based on standard skill tests:

```
========================================================================================
                              SKILL REVELATIONS & LORE CUES
========================================================================================
 SKILL TEST & MODIFIERS        RUNIC INSIGHT & OPERATIONAL BENEFIT
----------------------------------------------------------------------------------------
 LORE roll or SCAN roll        • Confirms the date of writing: 1981 TA (The Fall of Moria).
 (Favoured / +1d if invoking   • Identifies the key mechanism: The King's Door uses a 
  relevant Traits or tools)      synchronized double-cylinder tumbler requiring both 
                                 the King's Key and the Mithril Key, or a Craft bypass.
----------------------------------------------------------------------------------------
 CRAFT roll or HEALING roll    • Analyzes the yellow crust on the slate: Confirms it is 
 (+1d if invoking Smith or       residue of the Balrog's neurotoxic miasma.
  Herbal lore)                 • Reveals the remedy: Vinegar-soaked linen and crushed 
                                 charcoal will filter the vapor (see CRAFT roll 
                                 respirator masks).
----------------------------------------------------------------------------------------
 RIDDLE roll                   • Deciphers the marginal scratches at the bottom of the 
 (+1d if invoking ancient        slab: Scribe Frár scratched the tumbler layout—3 left, 
  Dwarven lore)                  2 right, 1 center—granting +2d on the Turn 1 roll of 
                                 the Lockbreaker Skill Endeavour (see Chapter 6 §4.3).
========================================================================================
```

---

## 5. GM Table Facilitation & Narrative Cues

### When to Present This Handout
* **Location**: Present this handout when the Company explores **Keyed Location 7: The Poisoned Halls** and searches the council dais where Scribe Frár's body lies preserved in stasis.
* **Sensory Staging**: Describe the heavy chill of the hall, the emerald mist swirling at knee height, and the eerie, untouched dignity of the fallen Dwarven captains. The slate is cold as mountain ice and leaves a faint smudge of sulfur on the finder's fingers.
* **Tactical Value to the Party**:
  1. **Solves the Vault Mystery**: Explicitly tells the players that *Durin's Axe* is inside Location 10 and that Location 9 (The King's Door) is currently sealed.
  2. **Points to Key Locations**: Directs the heroes to seek the *Mithril Key* among the Mordor Orc patrol (Grimnar / Grashnak's scout troop) and find the *Last Redoubt* in the Fifth Hall to retrieve the *King's Key*.
  3. **Unlocks the Craft Bypass Option**: Grants tactical advantages (+2d on Tumbler Alignment) if the players decide to pick the adamant lock via the Skill Endeavour.
